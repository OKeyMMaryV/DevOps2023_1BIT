...
Функция ПодготовитьРасчетАмортизацииОС(ТаблицаОС, ТаблицаРеквизиты, Отказ, РежимСправкиРасчета = Ложь) Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицуИзМассива(ТаблицаСчетовНачисленияАмортизации, Запрос.Параметры.СчетаНачисленияАмортизацииБУНачалоПериода, "Счет");
	...
	Пока ВыборкаПоОС.Следующий() Цикл
		...
		Если ВыборкаПоОС.ПорядокПогашенияСтоимости = Перечисления.ПорядокПогашенияСтоимостиОС.НачислениеАмортизации
			...
			ОстаточнаяСтоимость = ВыборкаПоОС.СтоимостьНачальныйОстаток - ВыборкаПоОС.АмортизацияНачальныйОстаток;
			...
			Если НЕ Пропустить Тогда
				...
				Если ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.Линейный Тогда
					...
					Если ПоддержкаФСБУ6 Тогда
						...
						СуммаАмортизации = (ОстаточнаяСтоимость - ЛиквидационнаяСтоимость) / ОстатокСрокаПолезногоИспользования;
					Иначе
				...
				ИначеЕсли ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.ПропорциональноОбъемуПродукции Тогда
					...
					Иначе
						Если ПоддержкаФСБУ6 Тогда
							...
							ЛиквидационнаяСтоимость = ВыборкаПоОС.ЛиквидационнаяСтоимость;
							СуммаАмортизации = (ОстаточнаяСтоимость - ЛиквидационнаяСтоимость) * ВыборкаПоОС.Количество / ОстатокОбъемаПродукции;
						Иначе
				...
				ИначеЕсли ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.УменьшаемогоОстатка Тогда
					...
					Если ПоддержкаФСБУ6 Тогда
						ЛиквидационнаяСтоимость = ВыборкаПоОС.ЛиквидационнаяСтоимость;
						СтоимостьИтоговая = ОстаточнаяСтоимость - ЛиквидационнаяСтоимость;
				...
				ИначеЕсли ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.ПоСуммеЧиселЛетИспользования Тогда
					...
					Иначе
						...
						Если ПоддержкаФСБУ6 Тогда
							ЛиквидационнаяСтоимость = ВыборкаПоОС.ЛиквидационнаяСтоимость;
							СтоимостьДляВычисленияАмортизации = ОстаточнаяСтоимость - ЛиквидационнаяСтоимость;
						Иначе
...
Процедура ПроверитьВозможностьПереводаВМалоценноеОборудование(ТаблицаОС, ТаблицаРеквизиты, Отказ) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаУчетаОС,
		БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ОсновныеСредства));
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаУчетаОС,
		БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ДоходныеВложенияВ_МЦ));
	...
	|	Остатки.Субконто1 КАК ОсновноеСредство,
	|	Остатки.Счет КАК Счет,
	|	Остатки.СуммаНУОстатокДт КАК СуммаНУОстатокДт
	|ПОМЕСТИТЬ ОстаткиБУ
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&Период, Счет В (&СчетаУчетаОС), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства), Организация = &Организация) КАК Остатки
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство,
	|	Счет
...
Функция НовыйТаблицаОстаточнойСтоимостиОС() Экспорт
	...
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-10-12 (#Аренда)
	ТаблицаСтоимости.Колонки.Добавить("Субконто3", Метаданные.ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Тип);
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-10-12 (#Аренда)
...
Функция ПодготовитьТаблицуОстаточнойСтоимости(ТаблицаОС, ТаблицаРеквизиты, ПараметрыВыбытия, Отказ) Экспорт
	...
	Если ТаблицаПараметрыСписания.Количество() > 0 Тогда
		...
		|	ТаблицаПараметрыСписания.ОсновноеСредство КАК ОсновноеСредство,
		|	ТаблицаПараметрыСписания.ТолькоЛинейный КАК ТолькоЛинейный,
		|	ТаблицаПараметрыСписания.СтоимостьОС + ТаблицаПараметрыСписания.ЗатратыНаСписаниеОС - ТаблицаПараметрыСписания.АмортизацияОС КАК СтоимостьБУ,
		...
		|	ТаблицаПараметрыСписания.СтоимостьОСВР + ТаблицаПараметрыСписания.ЗатратыНаСписаниеОСВР + ТаблицаПараметрыСписания.КорректировкаСтоимостиВР - ТаблицаПараметрыСписания.АмортизацияОСВР КАК СтоимостьВР,
		|	ТаблицаПараметрыСписания.ЗатратыНаСписаниеОС КАК ЗатратыНаСписаниеОС,
		|	ВЫБОР
...
Функция ПодготовитьПараметрыОстаточнойСтоимости(ТаблицаОС, ТаблицаРеквизиты)
	...
	//ОК+ Аверьянова #2538 от 21.06.16 //заполнение функции(3-е с/к) в 91 счете
	+ "Субконто3,"         // <Характеристика.ВидыСубконтоХозрасчетные> - ЦФО
	//ОК-	
...
Процедура СформироватьДвиженияСписаниеОстаточнойСтоимостиОС(ТаблицаСтоимости, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаТаблицы Из  Параметры.ТаблицаСтоимости Цикл
		...
		Если СуммаПроводки > 0
			...
			//izhtc-Kir 18.08.2015 (
			//ОК+ Аверьянова С.  - заявка Redmine №776 от 05.05.14
			Субконто3 = Неопределено;
			Попытка
				Если ТипЗнч(ТаблицаРеквизиты[0].Регистратор) = Тип("ДокументСсылка.СписаниеОС") Тогда
					Субконто3 = ТаблицаРеквизиты[0].Регистратор.Субконто3;
				КонецЕсли;
			Исключение
			КонецПопытки;
			
			Попытка
				Если ТипЗнч(ТаблицаРеквизиты[0].Регистратор) = Тип("ДокументСсылка.ПередачаОС") Тогда
					Субконто3 = СтрокаТаблицы.Субконто3;
				КонецЕсли;
			Исключение
			КонецПопытки;
...
Функция ПодготовитьПараметрыСписаниеОстаточнойСтоимостиОС(ТаблицаСтоимости, ТаблицаРеквизиты)
	...
	//izhtc-Kir 18.08.2015 (
	Если ТипЗнч(ТаблицаСтоимости) = Тип("ТаблицаЗначений") И ТаблицаСтоимости.Колонки.Найти("Субконто3") <> Неопределено Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",Субконто3"
	КонецЕсли;
	//izhtc-Kir 18.08.2015 )
...
Функция ПодготовитьПараметрыКорректировкаПредметыАрендыПереходНаФСБУ25(ТаблицаОС, ТаблицаРасчетАмортизации, ТаблицаАмортизации, ТаблицаРеквизиты)
	...
	+ "Период,"                    // <Дата> - период движений - дата документа
	+ "Организация,"               // <СправочникСсылка.Организация> - организация-арендатор
	+ "Подразделение";             // <СправочникСсылка.Организация> - подразделение документа
...
Функция ПодготовитьТаблицуАрендныхПлатежей(ТаблицаОС, Реквизиты, Отказ)
	...
	МассивСчетовАрендныеПлатежи = Новый Массив;
	МассивСчетовАрендныеПлатежи.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСПоставщикамиИПодрядчиками);     // 60
	МассивСчетовАрендныеПлатежи.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСРазнымиДебиторамиИКредиторами); // 76
	...
	Запрос.УстановитьПараметр("МассивСчетовАрендныеПлатежи", МассивСчетовАрендныеПлатежи);
	...
	|					ИЗ
	|						НеСнятыеСУчета),
	|			НЕ Корсчет В (&СчетаАрендныхОбязательств)
	...
	|ГДЕ
	|	Хозрасчетный.Ссылка В ИЕРАРХИИ(&МассивСчетовАрендныеПлатежи)
	|	И НЕ Хозрасчетный.Ссылка В (&СчетаНДСПоАренднымОбязательствам)
	|	И НЕ Хозрасчетный.Ссылка В (&СчетаПроцентовПоАренде)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Хозрасчетный.Ссылка
	|;
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(&НачалоПериода, &КонецПериодаГраница, , , Счет В (&СчетаАрендныхОбязательств), &ВидыСубконтоКонтрагентыДоговоры, Организация = &Организация) КАК ХозрасчетныйОстаткиИОбороты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|					ИЗ
	|						НеСнятыеСУчета)
	|				И СпособПоступления В (ЗНАЧЕНИЕ(Перечисление.СпособыПоступленияАктивов.ПоДоговоруЛизинга), ЗНАЧЕНИЕ(Перечисление.СпособыПоступленияАктивов.ПоДоговоруАренды))) КАК ПервоначальныеСведенияОСБухгалтерскийУчет
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СпособыОтраженияРасходовПоАренднымПлатежамОСНалоговыйУчет.ОсновноеСредство КАК ОсновноеСредство,
	|	СпособыОтраженияРасходовПоАренднымПлатежамОСНалоговыйУчет.СпособОтраженияРасходовПоАренднымПлатежам КАК СпособОтраженияРасходовПоАренднымПлатежам
...
Процедура СформироватьДвиженияРасходыПоАренднымПлатежамНУ(ТаблицаРасходыПоАренднымПлатежамНУ, 
	...
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаРасходыПоАренднымПлатежамНУ Цикл
		...
		Если СвойстваСчетаКт.УчетПоПодразделениям Тогда
			Проводка.ПодразделениеКт = СтрокаТаблицы.ПодразделениеРасходовПоАренднымПлатежам;
		КонецЕсли;
...
Процедура ПодготовитьРасчетАмортизацииОСПоФСБУ6(ПараметрыПроведения, Отказ) Экспорт
	...
	ТаблицаАмортизации = ПолучитьПустуюТаблицуПереходНаФСБУ6();
	...
	|	И СостоянияОСПринятоКУчету.ДатаПринятияКУчету < ДОБАВИТЬКДАТЕ(&Период, МЕСЯЦ, -1)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство";
	Запрос.Выполнить();
	...
	|	ПРЕДСТАВЛЕНИЕ(СписокОС.ОсновноеСредство) КАК ОсновноеСредствоПредставление,
	|	СписокОС.ОсновноеСредство.Код КАК Код,
	|	СписокОС.ИстекшийСрокПолезногоИспользования КАК ИстекшийСрокПолезногоИспользования,
	|	СписокОС.СрокПолезногоИспользования КАК СрокПолезногоИспользования,
	...
	Пока ВыборкаПоОС.Следующий() Цикл
		...
		Если ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.Линейный Тогда
			...
			АмортизацияФСБУ6 = Окр(Стоимость /ВыборкаПоОС.СрокПолезногоИспользованияФСБУ6 * ВыборкаПоОС.ИстекшийСрокПолезногоИспользования, 2);
...
Функция ПолучитьПустуюТаблицуПереходНаФСБУ6()
