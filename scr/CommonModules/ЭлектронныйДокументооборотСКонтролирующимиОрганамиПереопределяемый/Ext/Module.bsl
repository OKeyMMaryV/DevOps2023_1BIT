...
Процедура ПодписьРуководителя(Организация, Подпись) Экспорт
	...
	ДанныеПодписи = Справочники.Организации.ДвоичныеДанныеКартинкиОрганизации(Организация, "ФайлПодписьРуководителя");
...
Процедура ПолучитьЗначенияКонстантИнтеграции(ФлагиУчета = Неопределено) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	""ИнтеграцияСБанком"" КАК Название,
	|	ФлагиУчета.Значение КАК Значение
	|ИЗ
	|	Константа.ИнтерфейсИнтеграцииСБанком КАК ФлагиУчета
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	""УчетИмпортныхТоваров"",
	|	ФлагиУчета.Значение
	|ИЗ
	...
	|
	|ВЫБРАТЬ
	|	""Использовать1СОтчетностьПриИнтеграцииСБанком"",
	|	ФлагиУчета.Значение
	|ИЗ
	|	Константа.Использовать1СОтчетностьПриИнтеграцииСБанком КАК ФлагиУчета
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
