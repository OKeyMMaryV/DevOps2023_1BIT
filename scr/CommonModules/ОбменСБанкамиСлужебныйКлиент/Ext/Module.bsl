...
//
Процедура АутентификацияПоСертификатуСбербанк(Оповещение, ИдентификаторВК, ДанныеСертификата, НастройкаОбмена, ЭтоПолучениеНастроек = Ложь) Экспорт
...
//
Процедура ПодписатьЭДСбербанк(Оповещение, НастройкаОбмена, МассивСообщенийОбмена, ОграничениеПоСертификатам = Неопределено, ПринудительнаяАутентификация = Ложь) Экспорт
...
Процедура ПродолжитьПолучениеВыпискиПослеУстановкиСоединенияЧерезДопОбработку(АутентификацияВыполнена, Параметры) Экспорт
...
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена.
//
Процедура ПолучитьНовыеДокументыВК(Оповещение, ПодключаемыйМодуль, НастройкаОбмена, СоздаватьОперацииВыписки = Истина) Экспорт
...
Процедура ПровестиАутентификациюПослеПолученияФродПараметровСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.ПодключаемыйМодуль1С.НачатьВызовАутентификация(Оповещение,
		ДополнительныеПараметры.ИдентификаторСессии, ПодписьСоли, ДополнительныеПараметры.СтрокаФрод, ИдентификаторСессии,
		КодВозврата);
...
Процедура ОтправитьДокументыВБанк(ДанныеАутентификации, Параметры) Экспорт
	...
	Если НЕ Параметры.ТребуетсяАвторизация ИЛИ ЗначениеЗаполнено(ДанныеАутентификации) Тогда
		...
		ИначеЕсли РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.ОбменЧерезВК") Тогда
			...
			ПараметрыЗапроса = ОбменСБанкамиКлиентСервер.ПараметрыПолученияВыпискиБанка();
			ПодготовитьЗапросыВыписокДляОтправкиЧерезВК(Оповещение, Параметры.НастройкаОбмена, ПараметрыЗапроса, Истина);
...
Процедура ВключитьЖурналированиеПослеУточненияКаталогаЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Строка") Тогда
		...
		ДополнительныеПараметры.ПодключаемыйМодуль.НачатьВызовНачатьЖурналирование(
			ДополнительныеПараметры.ОповещениеПослеВключенияЖурналирования, Результат);
	Иначе
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеВключенияЖурналирования, Истина);
	КонецЕсли;
...
//  ОповещениеЗапускаДлительнойОперации - ОписаниеОповещение - вызывается после запуска длительной операции. Используется для прерывания процесса по инициативе пользователя.
//     * ИдентификаторЗадания - УникальныйИдентификатор - идентификатор длительной операции
//     * ДополнительныеПараметры - Структура - дополнительные параметры.
//
Процедура ПолучитьНовыеДокументыИзБанка(Оповещение, НастройкаОбмена, ИдентификаторСессии, ВыводитьОкноОжидания = Ложь, ОповещениеЗапускаДлительнойОперации = Неопределено)
...
Процедура ОбработатьЭД(Знач МассивСсылокНаОбъект, Действие, Знач СообщениеОбмена = Неопределено) Экспорт
	...
	ДополнительныеПараметры = Новый Структура("Действие, СообщениеОбмена", Действие, СообщениеОбмена);

	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действие, "Сформировать") Тогда
		СтандартнаяОбработка = Истина;
		ОбработчикОповещения = Новый ОписаниеОповещения(
...
Процедура ПродолжитьОбработкуДокументовПослеПереопределения(МассивСсылокНаОбъект, ДополнительныеПараметры) Экспорт
	...
	Если (ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действие, "Подписать")
		...
		Если ТребуетсяКриптография Тогда
			Оповещение = Новый ОписаниеОповещения("ПослеПолученияОтпечатковОбработатьЭД", ЭтотОбъект, ДополнительныеПараметры);
	...
	ПослеПолученияОтпечатковОбработатьЭД(Неопределено, ДополнительныеПараметры)
...
Процедура ПослеПолученияОтпечатковОбработатьЭД(ОтпечаткиСертификатов, Параметры) Экспорт
	...
	ПараметрыОбработки.Вставить("СообщениеОбмена", СообщениеОбмена);
	Результат = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияПоОбработкеЭлектронныхДокументов(ПараметрыОбработки);
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
	ПараметрыОжидания.ВыводитьСообщения = Истина;
...
Процедура ВывестиИнформациюОбОбработанныхЭД(ТекстЗаголовка, КоличествоСформированных, КоличествоПодписанных, КоличествоПодготовленных, КоличествоОтправленных, КоличествоПодтвержденных, ПодтверждениеПлатежейВБанке = Неопределено)
