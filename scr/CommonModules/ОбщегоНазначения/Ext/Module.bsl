...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	...
	// BIT Amerkulov 15042014 /Исправление для работы с периметрами консолидации {
	Если ТипЗнч(Ссылка) <> Тип("СправочникОбъект.бит_му_ПериметрыКонсолидации") И Реквизиты = "ЮрФизЛицо" Тогда
	...
	// Ок Видяйкин 20180209 (Задание в Омни AT-1641261) +
	// Если документ "ОК_СписаниеКорректировокНаСтоимостьЗапасов", там табличная часть "Таблица", совпадает с переменной в запросе. В таком случае запрос вываливался в ошибку.
	ФлагОК_СписаниеКорректировокНаСтоимостьЗапасов=0;
	Попытка
		Если ТипЗнч(Ссылка)=ТИП("ДокументСсылка.ОК_СписаниеКорректировокНаСтоимостьЗапасов") Тогда
			ФлагОК_СписаниеКорректировокНаСтоимостьЗапасов=1;
		КонецЕсли;
	Исключение
	КонецПопытки;
	// Ок Видяйкин 20180209 (Задание в Омни AT-1641261) -	
	...
	// Ок Видяйкин 20180209 (Задание в Омни AT-1641261) +
	Если ФлагОК_СписаниеКорректировокНаСтоимостьЗапасов=1 Тогда
		ТекстЗапроса = 
		"ВЫБРАТЬ " + ?(ВыбратьРазрешенные, "РАЗРЕШЕННЫЕ", "") + "
		|" + ТекстЗапросаПолей + "
		|ИЗ
		|	" + ПолноеИмяОбъектаМетаданных + " КАК Таблица_Внутренняя
		|ГДЕ
		|	Таблица_Внутренняя.Ссылка = &Ссылка
		|";
	КонецЕсли;	
	// Ок Видяйкин 20180209 (Задание в Омни AT-1641261) -	
...
Функция ЗначениеРеквизитаОбъекта(Ссылка, ИмяРеквизита, ВыбратьРазрешенные = Ложь) Экспорт
	...
	// BIT Amerkulov 15042014 /Исправление для работы с периметрами консолидации {
	Если ТипЗнч(Ссылка) <> Тип("СправочникОбъект.бит_му_ПериметрыКонсолидации") И ИмяРеквизита = "ЮрФизЛицо" Тогда
		Возврат Новый Структура;		
	КонецЕсли;
	// }
...
Функция РедакцияКонфигурации() Экспорт
	...
	Если ПустаяСтрока(Результат) Тогда
		Результат = бит_ОбщегоНазначения.МетаданныеВерсия();
	КонецЕсли;
