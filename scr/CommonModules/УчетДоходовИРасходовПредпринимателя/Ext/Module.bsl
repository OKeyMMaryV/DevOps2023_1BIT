...
Функция ПодготовитьТаблицуВключенияАвансовВПрочиеДоходы(ТаблицаРеквизитов, Отказ) Экспорт
	Перем ТаблицаАвансов, ДатаИзменения;
	...
	ПлательщикНДФЛ = УчетнаяПолитика.ПлательщикНДФЛ(Реквизиты.Организация, КонецМесяца(Реквизиты.Период), ДатаИзменения);
	Если Не ПлательщикНДФЛ И ДатаИзменения < НачалоМесяца(Реквизиты.Период) Тогда
		Возврат ТаблицаАвансов; // Прекращена уплата НДФЛ в прошлых периодах
	КонецЕсли;
	...
	Если Не ((Месяц(Реквизиты.Период) = 12) Или ПрекращенаУплатаНДФЛ) Тогда
		Возврат ТаблицаАвансов;
	КонецЕсли;
	...
	СтатьяДоходов = Справочники.ПрочиеДоходыИРасходы.ПредопределенныйЭлемент("ДоходыПоАвансамПрошлогоНалоговогоПериодаИП");
	...
	Иначе
		КонецПериода = НалоговыйПериод.Конец;
	КонецЕсли;
...
Функция ПодготовитьТаблицуРасходовНаУплатуСтраховыхВзносов(ТаблицаРеквизитов) Экспорт
	...
	ЕНВДУменьшаетсяНаФиксированныеВзносы = (ПлательщикЕНВД Или ПлательщикЕНВДвПрошлом)
	...
	Для каждого СтраховойВзнос Из ТаблицаРасходов Цикл
		...
		Если ВестиУчетПоВидамДеятельностиИП И (ПлательщикЕНВД Или ПлательщикЕНВДвПрошлом) Тогда
			
			// Сумма взносов, учтенная в расчете ЕНВД
			СтраховойВзносЕНВД = Окр(СтраховойВзнос.Сумма * КоэффициентЕНВД, 2);
		...
		ИначеЕсли ВестиУчетПоВидамДеятельностиИП И Не (ПлательщикЕНВД Или ПлательщикЕНВДвПрошлом) Тогда
...
Функция СуммаДоходаДляРасчетаСтраховыхВзносов(Организация, ДатаНачала, ДатаОкончания) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Если ДатаНачала >= УчетСтраховыхВзносовИП.ДатаНачалаУчетаРасходовПриРасчетеСтраховыхВзносовИП() Тогда
...
Процедура ЗаполнитьОтчет3НДФЛ(ИДРедакцииОтчета, Знач ПараметрыОтчета, Контейнер) Экспорт
	...
	ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2018кв1"
		...
		Или ИДРедакцииОтчета = "ФормаОтчета2020кв1" Тогда
...
Процедура ЗаполнитьРаздел63НДФЛ(Контейнер, ПараметрыОтчета)
	...
	Налог = Окр((ИтогоДоходов - ИтогоРасходов) * НалоговаяСтавка(), 0);
...
// Возвращает ставку НДФЛ, указанную в п. 1 ст. 224 НК РФ.
//
// Возвращаемое значение:
//  Число - ставка НДФЛ.
//
Функция НалоговаяСтавка(Знач Период = Неопределено) Экспорт
