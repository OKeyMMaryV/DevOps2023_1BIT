...
Процедура СоздатьСальдоВременныхРазниц(МенеджерВременныхТаблиц, ОтчетнаяДата, Организации, ВариантУчетаОтложенногоНалога)
	...
	|	ПланГруппировки.ИсключитьСубконто1 КАК ИсключитьСубконто1,
	|	ПланГруппировки.ИсключитьСубконто2 КАК ИсключитьСубконто2,
	|	ПланГруппировки.ИсключитьСубконто3 КАК ИсключитьСубконто3,
	...
	НастроитьРасчетВременнойРазницы(Запрос.Текст, ВариантУчетаОтложенногоНалога);
...
Функция ИспользоватьСуммыРазниц(Организация, Период) Экспорт
	...
	Возврат ПолучитьФункциональнуюОпцию("ПоддержкаПБУ18", ПараметрыФункциональнойОпции);
...
//
// Параметры:
//  ПоляРазниц     - Массив - поля формы, отображающие разницы, в том числе - заголовки строк
...
//
Процедура НастроитьПоляУчетаРазниц(ПоляРазниц, ПоляЗаголовков, Период, Организация = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(Организация) Тогда
		ИспользоватьСуммыРазниц = Ложь;
	Иначе
		ИспользоватьСуммыРазниц = НалогНаПрибыльБухгалтерскийУчет.ИспользоватьСуммыРазниц(Организация, Период);
	КонецЕсли;
	
	Для Каждого Поле Из ПоляРазниц Цикл
		Поле.Видимость = ИспользоватьСуммыРазниц;
	КонецЦикла;
