...
Процедура ЗаписатьЗначенияПоУмолчанию(ФизическоеЛицоСсылка, ГражданствоПоУмолчанию) Экспорт
	...
	Если ГражданствоПоУмолчанию Тогда
		...
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.ГражданствоФизическихЛиц");
		ЭлементБлокировкиДанных.УстановитьЗначение("ФизическоеЛицо", ФизическоеЛицоСсылка);
...
Функция ПодобратьСписокФизЛиц(ФизическоеЛицоСсылка, Знач Фамилия, Знач Имя, Знач Отчество, Знач Уточнение = Неопределено) Экспорт
	...
	Если МассивФизлиц.Количество() > 0 Тогда
		...
		Если МассивДоступныхФизическихЛиц.Количество() > 0 Тогда
			
			СтруктураВозврат.ДанныеФизическихЛицДоступны = МассивДоступныхФизическихЛиц.Количество() = МассивФизлиц.Количество();
			
			ДанныеФизическихЛиц = КадровыйУчет.КадровыеДанныеФизическихЛиц(
				Истина,
				МассивДоступныхФизическихЛиц,
		...
		Если Не СтруктураВозврат.ДанныеФизическихЛицДоступны Тогда
			...
			КадровыйУчет.СоздатьНаДатуВТКадровыеДанныеФизическихЛиц(
				...
				Ложь,
				ОбщегоНазначенияКлиентСервер.РазностьМассивов(МассивФизлиц, МассивДоступныхФизическихЛиц),
...
Функция ПохожиеФизическиеЛица(Знач ФизическоеЛицоСсылка, Знач Фамилия, Знач Имя, Знач Отчество, Знач Уточнение = "", Знач ФИОПолностью = Ложь, ТочноеСовпадение = Ложь)
	...
	// Дополним запрос по архиву физических лиц
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.АрхивФизическихЛиц") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("АрхивФизическихЛиц");
		Модуль.ДополнитьЗапросПохожиеФизическиеЛица(Запрос, ЧастиИмени.Фамилия, ЧастиИмени.Имя, ЧастиИмени.Отчество, ТочноеСовпадение);
	КонецЕсли;
