...
Процедура ПроверитьДоступностьСервисов(РезультатыДиагностикиСоединения, ПроверяемыеСервисы)
	...
	Для каждого Сервис Из ПроверяемыеСервисы Цикл
		...
		ОбщийРезультат = ОбщийРезультат И ОписаниеСервера.Результат;
...
Функция ИнтернетРесурсДоступен(АдресРесурса)
	...
	Попытка
		...
		Соединение = Новый HTTPСоединение(СтруктураURI.Хост,,,,, 7,
...
Процедура ДиагностикаКриптографии(РезультатДиагностики, ПараметрыВыполненияДиагностики, Контекст)
	...
	ОпределитьНеполученныеСертификаты(РезультатДиагностики);
...
Процедура ОпределитьНепривязанныеСертификатыИНаличиеСертификатов(РезультатДиагностики, ПараметрыВыполненияДиагностики)
	...
	Для каждого СтрокаТЗ Из ПривязанныеКУчетнымЗаписямСертификаты Цикл
	...
	Если ПривязанныеКУчетнымЗаписямСертификаты.Количество() = 0 Тогда
	...
	Для каждого КлючИЗначение Из СвойстваСертификатов Цикл
		...
		КонецЕсли;
	КонецЦикла;
	РезультатКриптография.ЕстьНеоблачныеСертификаты = РезультатКриптография.ОблачныеСертификаты.Количество() > 0;
	...
	Если (ЕстьКриптографияНаСервере И ЕстьМенеджерКриптографииНаСервере)
		...
		РезультатКриптография.Результаты.НаличиеСертификатов.Результат = ПривязанныеКУчетнымЗаписямСертификаты.Количество() > 0;
		...
		КонецЕсли;
	КонецЕсли;
	РезультатКриптография.Результаты.НаличиеСертификатов.Сертификаты = ПривязанныеКУчетнымЗаписямСертификаты.ВыгрузитьКолонку("Ссылка");
...
Процедура ОпределитьНеполученныеСертификаты(РезультатДиагностики)
...
Функция УчетныеЗаписиИзОтбора(ЗначениеОтбора) 
	...
	Если ТипЗнч(ЗначениеОтбора[0]) = Тип("РегистрСведенийКлючЗаписи.УчетныеЗаписиЭДО") Тогда
