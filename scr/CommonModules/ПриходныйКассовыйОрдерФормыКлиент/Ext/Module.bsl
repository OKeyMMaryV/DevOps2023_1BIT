...
Процедура ТекстНеобходимоЗаполнитьПатентОбработкаНавигационнойСсылки(Форма, Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка) Экспорт
...
Функция ПроверитьЗаполнениеДляФормированияЧекаНПД(Форма) Экспорт
	...
	КонтрагентЗаполнен = ЗначениеЗаполнено(Форма.Объект.Контрагент)
		Или Форма.Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.РозничнаяВыручка");
	...
	УслугаНПДЗаполнена = ЗначениеЗаполнено(Форма.Объект.УслугаНПД);
	Если Не УслугаНПДЗаполнена Тогда
	...
	СуммаДокументаЗаполнена = ЗначениеЗаполнено(Форма.Объект.СуммаДокумента);
