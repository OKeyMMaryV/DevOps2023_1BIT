...
Функция ПодготовитьТаблицуСтатусовОтгрузкиПоСчетам(ТаблицаТоварыУслуги, ТаблицаВозвратнаяТара, ТаблицаРеквизитов) Экспорт
	...
	Если Не ЗначениеЗаполнено(Реквизиты.СчетНаОплатуПокупателю) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ТекущиеСтатусы = РегистрыСведений.СтатусыДокументов.ПолучитьСтатусыДокумента(
		Реквизиты.СчетНаОплатуПокупателю, Реквизиты.Организация);
	ТекущийСтатусОтгрузки = ТекущиеСтатусы.ДополнительныйСтатус;
	Если ЗначениеЗаполнено(ТекущийСтатусОтгрузки)
	   И АвтоматическиИзменяемыеСтатусыОтгрузки().Найти(ТекущийСтатусОтгрузки) = Неопределено Тогда
	...
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.СтатусыДокументов");
	ЭлементБлокировки.УстановитьЗначение("Организация", Реквизиты.Организация);
	ЭлементБлокировки.УстановитьЗначение("Документ",    Реквизиты.СчетНаОплатуПокупателю);
	Блокировка.Заблокировать();
	
	РеализацииПоСчету = Документы.РеализацияТоваровУслуг.РеализацииПоСчету(Реквизиты.СчетНаОплатуПокупателю);
	...
	ТоварыУслугиКРеализации = Документы.РеализацияТоваровУслуг.ТоварыУслугиКРеализацииПоСчету(
		Реквизиты.СчетНаОплатуПокупателю, РеализацииПоСчету);
	...
	НоваяСтрока = ТаблицаСтатусов.Добавить();
	НоваяСтрока.Документ = Реквизиты.СчетНаОплатуПокупателю;
...
// Рассчитывает статус поступления и статус счета-фактуры перед записью документа поступления.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - Форма объекта, запись которого выполняется.
//   Отказ - Булево - флаг отказа.
//   ТекущийОбъект - ДокументОбъект - объект записываемого документа.
//   ПараметрыЗаписи - Структура - Структура, содержащая параметры записи.
//
Процедура ПоступленияПередЗаписьюНаСервере(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	ЕстьСчетФактура = ЗначениеЗаполнено(Форма.СчетФактура) ИЛИ ЗначениеЗаполнено(Форма.НомерСчетаФактурыПолученного);
