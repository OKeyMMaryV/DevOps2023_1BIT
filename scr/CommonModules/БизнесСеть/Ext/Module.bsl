...
Функция ВыполнитьКомандуСервиса(ПараметрыКоманды, Отказ, ТекстОшибки = Неопределено) Экспорт
	...
	Если ПараметрыСоединения.Аутентификация Тогда
		КлючТикета = ПолучитьКлючТикета(Соединение, ПараметрыСоединения, ПараметрыКоманды, Отказ, ТекстОшибки);
	...
	Результат = ОбработатьОтветСервиса(ПараметрыСоединения, ПараметрыКоманды, КодСостояния, Данные, Отказ);
...
//  ТребуетсяОбновитьИнтерфейс   - Булево - возвращаемый признак. Истина возвращается, когда константа ИспользоватьОбменБизнесСеть
//                                          выключена и необходимо вызвать функцию ОбновитьИнтерфейс().
//  ТекстОшибки                  - Строка - возвращается текст ошибки при выполнении.
//  ПоказыватьОшибкиПользователю - Булево - выводит сообщение пользователю при выполнении команды сервиса
//                                          и вызывает исключение, в случае ошибки разбора данных с сервиса.
//
Процедура ЗарегистрироватьОрганизации(СписокОрганизаций, Отказ, ТребуетсяОбновитьИнтерфейс = Неопределено,
		ТекстОшибки = Неопределено, ПоказыватьОшибкиПользователю = Истина) Экспорт
	
	Если Не ПравоНастройкиОбменаДокументами() Тогда
		Отказ = Истина;
		ТекстОшибки = НСтр("ru= 'Недостаточно прав для регистрации организации.
			|Обратитесь к администратору системы.'");
		
		Если ПоказыватьОшибкиПользователю Тогда
	...
	УстановитьСвойствоПараметраКомандыБлокировкиСообщенияОбОшибках(ПараметрыКоманды, ПоказыватьОшибкиПользователю);
	ДанныеСервиса = ВыполнитьКомандуСервиса(ПараметрыКоманды, Отказ, ТекстОшибки);
	...
	Если ДанныеСервиса = Неопределено
		...
		УстановитьСвойствоПараметраКомандыБлокировкиСообщенияОбОшибках(ПараметрыМетода, ПоказыватьОшибкиПользователю);
		ТекущийАбонент = ВыполнитьКомандуСервиса(ПараметрыМетода, Отказ, ТекстОшибки);
		Если Отказ Тогда
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Если ТипЗнч(ДанныеСервиса) = Тип("Массив") Тогда
			Если ДанныеСервиса.Количество() = 0 Тогда
				Отказ = Истина;
				ТекстОшибки = НСтр("ru = 'Абонент сервиса 1С:Бизнес-сеть не найден.'");
				Если ПоказыватьОшибкиПользователю Тогда
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;
			Иначе
	...
	Если ТекущийАбонент = Неопределено Тогда
		ТекстОшибки =  НСтр("ru = 'Отсутствуют данные об администраторе абонента.'");
		Если ПоказыватьОшибкиПользователю Тогда
			ВызватьИсключение ТекстОшибки;
		Иначе
			Возврат;
	...
	Для каждого СтрокаОрганизаций Из ТаблицаОрганизаций Цикл
		...
		Если ТипЗнч(ДанныеСервиса) = Тип("Массив") И ДанныеСервиса[0] = Истина Тогда
			...
			УстановитьСвойствоПараметраКомандыБлокировкиСообщенияОбОшибках(ПараметрыКоманды, ПоказыватьОшибкиПользователю);
			ВыполнитьКомандуСервиса(ПараметрыКоманды, Отказ, ТекстОшибки);
...
//
Функция ПолучитьКлючТикета(Соединение, ПараметрыСоединения, ПараметрыКоманды, Отказ, ТекстСообщения)
	...
	Если Тикет = Неопределено ИЛИ ТекущаяУниверсальнаяДатаВМиллисекундах() > Тикет.СрокЖизни Тогда
		...
		ПараметрыТикета = ПараметрыКомандыПолучениеКлюча(ПараметрыМетода, Отказ, ТекстСообщения);
		...
		Тикет = ПолучитьНовыйТикет(СписокТикетов, Соединение, ПараметрыСоединения, ПараметрыТикета, Отказ, ТекстСообщения);
		Если Отказ Тогда
			Возврат Неопределено;
		КонецЕсли;
...
//
Функция ПолучитьНовыйТикет(СписокТикетов, Соединение, ПараметрыСоединения, ПараметрыТикета, Отказ, ТекстОшибки)
	...
	Если ЗначениеЗаполнено(Тикет) Тогда
		...
		Исключение
			Отказ = Истина;
			ВидОперации = НСтр("ru = 'Получение токена'");
	...
	Если Ответ.КодСостояния <> 200 Тогда
		...
		ПодробныйТекстОшибки = ТекстОшибки + Символы.ПС
			+ "POST" + " "
			+ АдресСоединенияURL(ПараметрыСоединения.Сервер, ПараметрыСоединения.Порт,
				АдресРесурса, ПараметрыСоединения.ЗащищенноеСоединение <> Неопределено)
			+ Символы.ПС
...
Функция ПараметрыКомандыПолучениеКлюча(ПараметрыМетода, Отказ = Неопределено, ТекстОшибки = Неопределено)
...
Функция ПараметрыКомандыРегистрацияАбонента(ПараметрыМетода, Отказ)
	...
	ПараметрыКоманды = ОписаниеПараметровКомандыСервиса();
	ПараметрыКоманды.Наименование = НСтр("ru = 'Регистрация абонента'");
	...
	ПараметрыКоманды.Ошибки.Вставить(404, НСтр("ru = 'Ошибка регистрации абонента в сервисе.'"));
	ПараметрыКоманды.Ошибки.Вставить(403, НСтр("ru = 'Недостаточно прав для регистрации абонента в сервисе.'"));
...
Функция УстановитьСвойствоПараметраКомандыБлокировкиСообщенияОбОшибках(ПараметрыКоманды, ПоказыватьОшибкиПользователю)
	Если НЕ ПоказыватьОшибкиПользователю Тогда
...
//     * СписокПользователей    - Массив из Структура - пользователи синхронизации, где:
...
Процедура СформироватьДанныеСинхронизацииПользователя(ПараметрыСинхронизации, ОбновлятьИдентификаторыДоступа)
	...
	Для каждого СтрокаТаблицы Из ПользователиСервиса Цикл
		...
		УчетныеДанные.Вставить("personAccounts", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПользовательСистемы));
		
		ДанныеПользователя = Новый Структура("Ссылка, Идентификатор, Пароль");
	...
	ПараметрыСинхронизации.СписокПользователей    = СписокПользователей;
...
Процедура СохранитьИдентификаторыПользователей(СписокПользователей, РежимУдаления = Ложь)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СписокПользователей) Тогда
			Для каждого ЭлементКоллекции Из СписокПользователей Цикл
				НоваяСтрока = ТаблицаПользователей.Добавить();
				НоваяСтрока.Ссылка = ЭлементКоллекции.Ссылка;
			КонецЦикла;
			ЭлементБлокировки.ИсточникДанных = ТаблицаПользователей;
		...
		Если ЗначениеЗаполнено(СписокПользователей) Тогда
			...
			Запрос.УстановитьПараметр("МассивСсылок", ТаблицаПользователей.ВыгрузитьКолонку("Ссылка"));
