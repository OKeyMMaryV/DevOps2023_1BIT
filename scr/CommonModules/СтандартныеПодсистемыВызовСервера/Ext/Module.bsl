...
//1c-izhtc spawn 10.08.15 (
//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-10-22 (#3926)
//Процедура УстановитьПараметрСеансаТекущийИнициатор(DomainName) Экспорт 
Процедура УстановитьПараметрСеансаТекущийИнициатор(DomainName = Неопределено) Экспорт 
//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-10-22 (#3926)
	УстановитьПривилегированныйРежим(Истина);
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-10-22 (#3926)
	...
	Если ТипЗнч(ТекущийПользователь) = Тип("СправочникСсылка.Пользователи")
		...
		Запрос.Текст = 
			...
			|	И НЕ бит_БК_Инициаторы.ПометкаУдаления";
		...
		Если ВыборкаДетальныеЗаписи.Следующий() Тогда
			ПараметрыСеанса.бит_БК_ТекущийИнициатор = ВыборкаДетальныеЗаписи.Инициатор;
		Иначе
			ПараметрыСеанса.бит_БК_ТекущийИнициатор = ПредопределенноеЗначение("Справочник.бит_БК_Инициаторы.ПустаяСсылка");
		КонецЕсли;
