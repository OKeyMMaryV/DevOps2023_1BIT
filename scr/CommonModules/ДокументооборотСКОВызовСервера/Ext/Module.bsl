	...
	Пока ИнициализироватьКонтекст Цикл
		...
		Если ДвоичныеДанныеОбработки <> Неопределено Тогда
			...
			Попытка
				...
				ИмяОбработки_ДокументооборотСКО = МенеджерВнешнихОбработок.Подключить(Адрес, "Обработка_ДокументооборотСКО", Ложь, ОписаниеЗащиты);	
			Исключение
...
Функция ПолучитьОбработкуЭДО(ТекстСообщения = "") Экспорт
	...
	Если ПодключатьВнешнююОбработкуЭДО() И НЕ ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы() Тогда
