...
//
Процедура УстановитьВидимостьПредварительногоПросмотра(Форма) Экспорт
...
Процедура ПересчитатьИтоги(МестоРасхождения, АдресРезультата) Экспорт
	...
	Если СтрНачинаетсяС(МестоРасхождения.ИмяОтчета, "бит_") Тогда
		РегистрСИтогами = РегистрыБухгалтерии.бит_Дополнительный_2;
	КонецЕсли; 
...
// Возвращает структуру настроек отчета, которые допустимо отправить в Пульт.
//
// Параметры:
...
//
// Возвращаемое значение:
//   Структура - структура настроек отчета, которые нужно отправить в Пульт.
//
Функция ПоказателиОтчетаРуководителяДляСтатистики(ПараметрыФормированияОтчета) Экспорт
	...
	Если ПараметрыФормированияОтчета.Свойство("ДополнительныеПоля") Тогда
		...
		ПоказателиОтчета.Вставить("ДополнительныеПоля", Новый Массив);
		
		Для Каждого СтрокаТаблицы Из ПараметрыФормированияОтчета.ДополнительныеПоля Цикл
			Если СтрокаТаблицы.Использование Тогда
				ПоказателиОтчета.ДополнительныеПоля.Добавить(СтрокаТаблицы.Поле);
			КонецЕсли;
		КонецЦикла;
	...
	ПоказателиОтчета.Вставить("ПользовательИБ", РеквизитыПользователя.ИдентификаторПользователяИБ);
	ПоказателиОтчета.Вставить("ПользовательСервиса", РеквизитыПользователя.ИдентификаторПользователяСервиса);
	
	ПоказателиОтчета.Вставить("ДатаСобытия", НачалоЧаса(ТекущаяДатаСеанса()));
...
Процедура ПодготовитьСхемуКомпоновкиДанныхДляИзменения(ПараметрыОтчета, Схема, КомпоновщикНастроек)
	...
	Схема = Отчет.ПолучитьМакет("СхемаКомпоновкиДанных");
	
	Отчет.НастроитьСхемуКомпоновкиДанных(ПараметрыОтчета, Схема);
