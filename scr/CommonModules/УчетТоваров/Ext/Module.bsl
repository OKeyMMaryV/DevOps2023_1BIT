...
Процедура СформироватьДвиженияРасходыПоПутевомуЛисту(ПриобретениеТоплива, ТаблицаРеквизиты, Движения) Экспорт
	...
	СодержаниеТопливнаяКарта = НСтр("ru='Приобретено топливо для %1 по Топливной карте №%2'", КодОсновногоЯзыка);
	СодержаниеКассовыйЧек    = НСтр("ru='Приобретено топливо для %1 по Кассовому чеку №%2 от %3'", КодОсновногоЯзыка);
	...
	Для Каждого ТекущаяСтрока Из СтруктураТаблиц.КупленноеТопливо Цикл
		...
		Если ТекущаяСтрока.ВидВходящегоДокумента = ТипКассовыйЧек Тогда
			...
			СодержаниеПроводки = СтрШаблон(СодержаниеКассовыйЧек,
		...
		Иначе
			...
			СодержаниеПроводки = СтрШаблон(СодержаниеТопливнаяКарта,
				Реквизиты.ТранспортноеСредство,
				ТекущаяСтрока.НомерВходящегоДокумента);
	...
	СодержаниеПроводки   = НСтр("ru='Расход топлива по %1'", КодОсновногоЯзыка);
...
Процедура СформироватьДвиженияКорректировкиПоступленияТоваров(РеквизитыДокумента, ТаблицаПоТоварам, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаТовары Из ТаблицаПоТоварам Цикл
		...
		Если Год(Реквизиты.ДокументПоступленияДата) < Год(Реквизиты.Период)
			И РеквизитыДокумента.Колонки.Найти("ок_КорректировкаТекущимГодом") <> Неопределено
			И Реквизиты.ок_КорректировкаТекущимГодом Тогда
			
			ПоступлениеТекущегоМесяца = Ложь;
			ПоступлениеТекущегоГода = Истина;
		КонецЕсли;
...
Функция ПолучитьРеквизитыКорректировкиПоступленияТоваров(РеквизитыДокумента)
	...
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2019-11-17 (#3526)
	Если ТипЗнч(РеквизитыДокумента) = Тип("ТаблицаЗначений")
		И РеквизитыДокумента.Колонки.Найти("ок_КорректировкаТекущимГодом") <> Неопределено Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",ок_КорректировкаТекущимГодом";
	КонецЕсли;
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2019-11-17 (#3526)
...
Процедура СформироватьДвиженияКорректировкиПоступленияУслуг(РеквизитыДокумента, ТаблицаПоУслугам, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаУслуги Из ТаблицаПоУслугам Цикл
		...
		Если Год(Реквизиты.ДокументПоступленияДата) < Год(Реквизиты.Период) 
			И (ТипЗнч(Реквизиты) = Тип("СтрокаТаблицыЗначений")
				И ТипЗнч(Реквизиты.Владелец()) = Тип("ТаблицаЗначений")
				И Реквизиты.Владелец().Колонки.Найти("ок_КорректировкаТекущимГодом") <> Неопределено
				И НЕ Реквизиты.ок_КорректировкаТекущимГодом) Тогда
		//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2019-11-17 (#3526)
		...
		ИначеЕсли (Месяц(Реквизиты.ДокументПоступленияДата) < Месяц(Реквизиты.Период)
			И Реквизиты.СпособОтраженияУслугТекГода = 0)
			ИЛИ (Год(Реквизиты.ДокументПоступленияДата) < Год(Реквизиты.Период) И Реквизиты.СпособОтраженияУслугТекГода = 0
			      И ТипЗнч(Реквизиты) = Тип("СтрокаТаблицыЗначений") И ТипЗнч(Реквизиты.Владелец()) = Тип("ТаблицаЗначений")
				  И Реквизиты.Владелец().Колонки.Найти("ок_КорректировкаТекущимГодом") <> Неопределено И Реквизиты.ок_КорректировкаТекущимГодом) Тогда
		//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2019-11-17 (#3526)	
		...
		Если Реквизиты.РаздельныйУчетНДСНаСчете19 Тогда
			...
			ИначеЕсли (Месяц(Реквизиты.ДокументПоступленияДата) < Месяц(Реквизиты.Период)
				И Реквизиты.СпособОтраженияУслугТекГода = 0 )
				ИЛИ (Год(Реквизиты.ДокументПоступленияДата) < Год(Реквизиты.Период) И ТипЗнч(Реквизиты) = Тип("СтрокаТаблицыЗначений")
				И ТипЗнч(Реквизиты.Владелец()) = Тип("ТаблицаЗначений")И Реквизиты.Владелец().Колонки.Найти("ок_КорректировкаТекущимГодом") <> Неопределено
				И Реквизиты.ок_КорректировкаТекущимГодом) Тогда
			//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2019-11-17 (#3526)
