...
Функция НачатьОтправкуДекларацииИспользуяДокументооборотСКОВФоне(СсылкаНаОтчет, ПовторнаяОтправка, СообщениеОбОшибке) Экспорт
	...
	СохраненныйОтчет = ДесериализоватьИПроверитьСсылкуНаРегламентированныйОтчет(СсылкаНаОтчет, СообщениеОбОшибке);
...
Функция ПреобразоватьДанныеРеглОтчетаБРОДляМП(ИсточникОтчета, ИмяФормыОтчета, ДанныеБРО)
	...
	ИначеЕсли ИсточникОтчета = "РегламентированныйОтчет6НДФЛ" Тогда
		Если ИмяФормыОтчета = "ФормаОтчета2016Кв1"
			ИЛИ ИмяФормыОтчета = "ФормаОтчета2017Кв4" Тогда
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
		...
		Для Каждого ИмяПечатнойФормы Из МассивПечатныхФорм Цикл
			...
			Если ИмяПечатнойФормы = "СчетФактура"
				...
				Если ПлательщикНДС ИЛИ ИмяПечатнойФормы = "СчетФактура" Тогда
					РеквизитыСФ = УчетНДСВызовСервера.СоздатьСчетФактуруВыданныйНаОсновании(ДокСсылка);
...
Процедура РазобратьПолученноеСообщение(СтруктураПараметров, СообщениеОбменаСтрокаXML, СообщениеОбОшибке)
	...
	Пока ЧтениеXML.URIПространстваИмен = URIПространстваИмен Цикл
		...
		ИначеЕсли ОбъектXDTO.Тип().Имя = "Организация" Тогда
			...
			ЗагрузитьОрганизацию(ОбъектXDTO, СтруктураПараметров.УзелОбмена, Организация, РегистрацииВНалоговомОргане);
		...
		ИначеЕсли ОбъектXDTO.Тип().Имя = "УдалениеОбъекта" Тогда
			...
			ЗагрузитьУдалениеОбъекта(ОбъектXDTO, СтруктураПараметров.УзелОбмена, ВидыОбменаСБанком);
	...
	Если ИзмененияНастроек.Количество() > 0 Тогда
		...
		Если НовыеЗначенияНастроекПриложения.Количество() >0
			...
			Если НовыеЗначенияНастроекПриложения.Количество() > 0 Тогда
				...
				Настройка = УзелОбъект.НастройкиПриложения.Найти(Организация, "Организация");;
		...
		Если ИзмененияНастроек.Свойство("Организация") И НЕ ЗначениеЗаполнено(СтруктураПараметров.Организация) Тогда
		...
		Если ИзмененияНастроек.Свойство("ДатаНачалаРаботы") Тогда
			...
			ПроверитьУстановитьДатуВводаНачальныхОстатков(ДатаВводаНачальныхОстатков, Организация, СтруктураПараметров.УзелОбмена);
		
		КонецЕсли;
		
		Если ИзмененияНастроек.Свойство("ИПИспользуетТрудНаемныхРаботников") Тогда
			МобильнаяБухгалтерияЗарплата.ПроверитьУстановитьНастройкиЗарплаты(Организация);
		КонецЕсли;
		...
		Если ИзмененияНастроек.Свойство("НастройкиУчетнойПолитики") Тогда
			...
			Если СтруктураПараметров.ВерсияСхемы < 5 Тогда
				УдалитьНастройкиУчетнойПолитики(Организация);
			...
			УстановитьУчетнуюПолитику(Организация, НастройкиУчетнойПолитики);
		Иначе
			...
			Если ЕстьИзмененияВлияющиеНаУчет Тогда
				...
				ПроверитьУстановитьУчетнуюПолитикуПоУмолчанию(Организация, ДобавитьМесяц(ДатаНачалаРаботы, -12));
		...
		Если ПерезаполнитьВесьСписокЗадач Тогда
			...
			ОбновитьЗадачи(Организация, 90, Ложь);
	...
	Для Каждого ДанныеРегистрацииВНалоговомОргане ИЗ РегистрацииВНалоговомОргане Цикл
		...
		ЗаписатьРегистрациюВНалоговомОргане(РегистрацияВНО, Организация, ДанныеРегистрацииВНалоговомОргане);
...
Процедура ЗагрузитьЧекиНПД(ОбъектXDTO, УзелОбмена) Экспорт
	...
	Для Каждого Запись ИЗ ОбъектXDTO.Записи Цикл
		...
		НоваяЗапись.ПричинаАннулирования  = Запись.ПричинаАннулирования;
...
Процедура ЗагрузитьПлатежноеПоручение(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	ОжидаетПолученияНомера = Ложь;
	Если ПустаяСтрока(ОбъектXDTO.Номер) Тогда
		Объект.УстановитьНовыйНомер();
		ОжидаетПолученияНомера = Истина;
	Иначе
		Объект.Номер = ОбъектXDTO.Номер;
	КонецЕсли;
	...
	Если Объект.Проведен ИЛИ ОбъектXDTO.Проведен Тогда
		...
		НоваяСтрока.Удаление = ОбъектXDTO.ПометкаУдаления;
		НоваяСтрока.ЗаполнитьСчета = Ложь;
		НоваяСтрока.ОжидаетПолученияНомера = ОжидаетПолученияНомера;
	Иначе
		Объект.ПометкаУдаления = ОбъектXDTO.ПометкаУдаления;
	КонецЕсли;
...
Процедура ЗагрузитьПоступлениеНаРасчетныйСчет(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	Объект.УслугаНПД = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.УслугаНПД, "Номенклатура");
...
Процедура ЗагрузитьПоступлениеТоваровУслуг(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	Если ЗначениеЗаполнено(ОбъектXDTO.ДатаСчетаФактуры)
		...
		Если НЕ ЗначениеЗаполнено(СчетФактура) Тогда
			УчетНДСВызовСервера.СоздатьСчетФактуруПолученныйНаОсновании(Объект.Ссылка, ОбъектXDTO.НомерСчетаФактуры, ОбъектXDTO.ДатаСчетаФактуры);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗагрузитьСведенияОЗастрахованныхЛицахСЗВ_М(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	Если Объект.Проведен ИЛИ (НЕ ОбъектXDTO.ПометкаУдаления) Тогда
		...
		НоваяСтрока.Ссылка   = ?(Объект.Ссылка.Пустая(), Объект.ПолучитьСсылкуНового(), Объект.Ссылка);
		НоваяСтрока.Дата     = ОбъектXDTO.Дата;
		НоваяСтрока.Провести = НЕ ОбъектXDTO.ПометкаУдаления;
		НоваяСтрока.ЗаполнитьСчета = Ложь;
	Иначе
		Объект.ПометкаУдаления  = ОбъектXDTO.ПометкаУдаления;
	КонецЕсли;
...
Процедура ЗагрузитьУдалениеОбъекта(ОбъектXDTO, УзелОбмена, ВидыОбменаСБанком)
	
	Ссылка = "";
	УдалитьДвижения = Ложь;
	
	// Справочники
	Если ОбъектXDTO.ТипОбъекта = "Банк" Тогда
		Ссылка = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.Ссылка, "Банки");
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "БанковскийСчет" Тогда
		Ссылка = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.Ссылка, "БанковскиеСчета");
		ВидОбменаСБанком = ВидыОбменаСБанком.Добавить();
		ВидОбменаСБанком.БанковскийСчет = Ссылка;
		ВидОбменаСБанком.Удалить = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ВидДеятельностиЕНВД" Тогда
		Ссылка = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.Ссылка, "ВидыДеятельностиЕНВД");
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ДоговорКонтрагента" Тогда
		Ссылка = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.Ссылка, "ДоговорыКонтрагентов");
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "Должности" Тогда
		Ссылка = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.Ссылка, "Должности");
	...
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "Сотрудники" Тогда
		Ссылка = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.Ссылка, "Сотрудники");
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "СтранаМира" Тогда
		Ссылка = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.Ссылка, "СтраныМира");
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ФизическиеЛица" Тогда
		Ссылка = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.Ссылка, "ФизическиеЛица");

	// Документы
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "АктСверкиВзаиморасчетов" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "АктСверкиВзаиморасчетов");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ВедомостьНаВыплатуЗарплатыВБанк" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ВедомостьНаВыплатуЗарплатыВБанк");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ВедомостьНаВыплатуЗарплатыВКассу" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ВедомостьНаВыплатуЗарплатыВКассу");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ВводНачальныхОстатков" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ВводНачальныхОстатков");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ЗаписьКУДиР" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ЗаписьКУДиР");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ЗаявлениеАбонентаСпецоператораСвязи" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ЗаявлениеАбонентаСпецоператораСвязи");
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "НачислениеЗарплаты" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "НачислениеЗарплаты");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ПлатежноеПоручение" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ПлатежноеПоручение");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ПоступлениеНаРасчетныйСчет" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ПоступлениеНаРасчетныйСчет");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ПоступлениеТоваровУслуг" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ПоступлениеТоваровУслуг");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "ПриходныйКассовыйОрдер" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ПриходныйКассовыйОрдер");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "РасходныйКассовыйОрдер" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "РасходныйКассовыйОрдер");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "РасходыНаОнлайнКассы" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "РасходыНаОнлайнКассыУменьшающиеНалоги");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "РеализацияТоваровУслуг" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "РеализацияТоваровУслуг");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "РегламентированныйОтчет" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "РегламентированныйОтчет");
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "РозничнаяПродажа" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ОтчетОРозничныхПродажах");
		УдалитьДвижения = Истина;
	ИначеЕсли ОбъектXDTO.ТипОбъекта = "СведенияОЗастрахованныхЛицахСЗВ_М" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "СведенияОЗастрахованныхЛицахСЗВ_М");
		УдалитьДвижения = Истина;
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		Если Объект <> Неопределено Тогда
			...
			Если ДатыЗапретаИзменения.ИзменениеЗапрещено(Объект) Тогда
...
Функция ПодготовитьСообщениеВыгрузки(СтруктураПараметров, Отбор = Неопределено, ПолноеИмяФайлаОбмена = "")
	...
	ВерсияСхемыУзлаОбмена = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураПараметров.УзелОбмена, "ВерсияСхемы");
	Если СтруктураПараметров.ВерсияСхемы > ВерсияСхемыУзлаОбмена Тогда
		ПриИзмененииВерсииСхемы(ВерсияСхемыУзлаОбмена, СтруктураПараметров);
	КонецЕсли;
	...
	Если НЕ СтруктураПараметров.ПервичнаяИнициализацияВыполнена Тогда
		...
		// Передадим признак интеграции с банком
		ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, ВыгрузитьИнтеграцияСБанком(СтруктураПараметров));
...
Функция ПолучитьОбъектXDTO(Данные, СтруктураПараметров)
	...
	Если ТипЗнч(Данные) = Тип("КонстантаМенеджерЗначения.ИспользоватьНесколькоСкладов") Тогда
		...
		ПередаваемыйОбъект = ВыгрузитьИспользоватьНесколькоСкладов();
		...
		ИначеЕсли ТипЗнч(ОтчетСсылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если ОбщегоНазначения.СсылкаСуществует(ОтчетСсылка) Тогда
				...
				Попытка
					...
					ЗапрещеноРедактироватьВМобильном = НЕ (ОперацииСРеглОтчетом.ВыгрузкаНаСервере
...
Функция ВыгрузитьИнтеграцияСБанком(СтруктураПараметров)
	
	ПередаваемыйОбъект = СоздатьОбъектXDTO("ИнтеграцияСБанком");
	ПередаваемыйОбъект.ИнтеграцияСБанкомЗначение = Справочники.НастройкиИнтеграцииСБанками.ИнтеграцияВИнформационнойБазеВключена();
	Возврат ПередаваемыйОбъект;
...
Функция ВыгрузитьИспользоватьНесколькоСкладов()
...
Функция ВыгрузитьЧекиНПД(Данные, СтруктураПараметров)
	...
	Для каждого ДанныеЗаписи Из Данные Цикл
		...
		ПередаваемаяЗапись.ПричинаАннулирования  = ДанныеЗаписи.ПричинаАннулирования;
...
Функция ВыгрузитьПоступлениеНаРасчетныйСчет(Данные, СтруктураПараметров)
	...
	Если СтруктураПараметров.ВерсияСхемы >= 6 Тогда
		ПередаваемыйОбъект.УслугаНПД = СериализоватьСсылку(Данные.УслугаНПД);
	КонецЕсли;
...
Функция ВыгрузитьОрганизацииПрисоединенныеФайлы(Данные, СтруктураПараметров)
	...
	ПередаваемыйОбъект.ХранимыйФайл    = Base64Строка(РаботаСФайлами.ДвоичныеДанныеФайла(Данные.Ссылка));
...
Процедура ПриИзмененииВерсииСхемы(ТекущаяВерсияСхемы, СтруктураПараметров)
	
	Если НЕ СтруктураПараметров.ПерваяСинхронизация И ТекущаяВерсияСхемы < 5 И СтруктураПараметров.ВерсияСхемы >= 5 Тогда
...
Процедура ЗаполнитьРегламентированныеОтчеты(СтруктураПараметров, ЗаполнениеОтчетности, СообщениеОбОшибке)
	...
	Для каждого СтрокаЗаполнения Из ЗаполнениеОтчетности Цикл
		...
		ОбъектОтчета.ОбменДанными.Отправитель = СтруктураПараметров.УзелОбмена;
...
Функция ДесериализоватьИПроверитьСсылкуНаРегламентированныйОтчет(СсылкаНаОтчет, СообщениеОбОшибке)
	...
	Если НЕ ЗначениеЗаполнено(СохраненныйОтчет) ИЛИ НЕ ОбщегоНазначения.СсылкаСуществует(СохраненныйОтчет) Тогда
		ЗарегистрироватьОшибкуОбмена(
			НСтр("ru = 'При отправке декларации на e-mail из мобильной бухгалтерии передана некорректная ссылка на сохраненный регламентированный отчет'"),
			НСтр("ru = 'Отправка декларации через 1С-Отчетность из мобильной бухгалтерии'"));
		
		СообщениеОбОшибке = НСтр("ru = 'Не удалось выгрузить декларацию из мобильного приложения.'");
...
Функция СформироватьОтчет(СтруктураПараметров, СообщениеОбОшибке) Экспорт
	...
	Если ИмяОтчета = "КнигаУчетаДоходовИРасходов" Тогда
		...
		Организация = ДесериализоватьСсылкуНаСправочник(ПараметрыОтчета.Организация, "Организации");
