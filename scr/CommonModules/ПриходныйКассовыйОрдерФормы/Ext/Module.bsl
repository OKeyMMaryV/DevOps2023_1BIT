...
Процедура ПодготовитьФормуНаСервере(Форма) Экспорт
	...
	Иначе
		...
		Форма.ТекстНеобходимоЗаполнитьПатент = УчетПСН.ТекстНеобходимоЗаполнитьПатент();
	...
	Если Объект.БезЗакрывающихДокументов Тогда
		ПараметрыОбъекта      = ПриходныйКассовыйОрдерФормыКлиентСервер.ТекущиеПараметрыОбъекта(Форма);
		Форма.СвойстваПлатежа = ПриходныйКассовыйОрдерФормыКлиентСервер.СвойстваСтрокРасшифровкиПлатежа(ПараметрыОбъекта, Истина);
...
Процедура УстановитьВидимостьСтраницНаСервере(Форма) 
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПКО.РозничнаяВыручка Тогда
		...
		Иначе
			...
			ТекущаяГруппа = ?(РасшифровкаНедоступна, Неопределено, Элементы.ГруппаРозничнаяВыручка);
		КонецЕсли;
...
Процедура УправлениеФормой(Форма) Экспорт
	...
	Элементы.ТекстНеобходимоЗаполнитьПатент.Видимость
		= (Объект.ВидОперации = Перечисления.ВидыОперацийПКО.РозничнаяВыручка)
			И Объект.ВыручкаСНТТ И Форма.УчетВПРодажныхЦенах
...
Процедура НастроитьЭлементыДляНПД(Форма)
	...
	ВидОперацииОтражениеДохода = Объект.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя
...
Процедура ОтобразитьГиперссылкуПерейтиВТаблицу(Форма)
	...
	Если КоличествоСтрок = 1 Тогда
		...
		Если Форма.ПлательщикНПД Или НетДопАналитикиВБезЗакрывающих Тогда
			ВключитьВидимостьСсылкиНаПлатеж(Элементы, Ложь, Ложь);
		Иначе
