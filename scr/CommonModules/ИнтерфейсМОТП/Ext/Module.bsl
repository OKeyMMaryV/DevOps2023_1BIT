...
// 	ВариантПолученияМРЦ - Строка, Неопределено - Вариант получения МРЦ ("ВычислениеИЗапрос" Или "ВычислениеИЗапросСПроверкойНеобходимости").
...
Функция ДеревоУпаковок(Значение, Детализация = Неопределено, ВариантПолученияМРЦ = "ВычислениеИЗапрос") Экспорт
	...
	Если ТипЗнч(Значение) = Тип("Строка") Тогда
		...
		Иначе
			...
			КодМаркировкиБезСкобок   = ШтрихкодированиеИСМПКлиентСервер.КодМаркировкиБезСкобок(Значение);
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("Массив") Тогда
		...
		Для Каждого КодМаркировки Из Значение Цикл
			...
			Иначе
				ЭтоЛогистическаяУпаковка = Истина;
				КодМаркировкиБезСкобок = ШтрихкодированиеИСМПКлиентСервер.КодМаркировкиБезСкобок(КодМаркировки);
		...
		Для Каждого КодМаркировки Из ПакетКодовМаркировки Цикл
			...
			Если ЭтоКодМаркировкиТабачнойПачки Тогда
				...
				Если Не ЗначениеЗаполнено(СтрокаДерева.МРЦ)
					И (ВариантПолученияМРЦ = "ВычислениеИЗапрос" Или ВариантПолученияМРЦ = "ВычислениеИЗапросСПроверкойНеобходимости") Тогда
			...
			Иначе
				...
				Если ЭтоКодМаркировкиБлока Тогда
					...
					ДанныеСтатуса            = СтатусыКодовМаркировки[КодМаркировки];
					КодМаркировкиДляПроверки = КодМаркировки;
				...
				Иначе
					...
					Если ЭтоПроверкаПарыКодов Тогда
						...
						Если ДанныеСтатусаСоСкобками.Статус <> Перечисления.СтатусыКодовМаркировкиМОТП.Неопределен Тогда
							...
							КодМаркировкиДляПроверки = КодМаркировкиСоСкобками;
						ИначеЕсли ДанныеСтатусаБезСкобок.Статус <> Перечисления.СтатусыКодовМаркировкиМОТП.Неопределен Тогда
							...
							КодМаркировкиДляПроверки = КодМаркировкиБезСкобок;
						Иначе
							ДанныеСтатуса            = ДанныеСтатусаБезСкобок;
							КодМаркировкиДляПроверки = КодМаркировкиСоСкобками;
						КонецЕсли;
				...
				Если ДанныеСтатуса.Статус <> Перечисления.СтатусыКодовМаркировкиМОТП.Неопределен Тогда
					...
					Результат = ЗапроситьДанныеОбАгрегацииКодовМаркировки(КодМаркировкиДляПроверки);
					Если Результат.ДанныеОбАгрегации <> Неопределено
						...
						Если Не ЗначениеЗаполнено(Детализация)
							...
							ЗаполнитьДеревоУпаковокРекурсивно(
								...
								Результат.ДанныеОбАгрегации, Кеш, Детализация, ВариантПолученияМРЦ);
...
Функция ДанныеПродукцииПоШтрихкодуEAN(Значение) Экспорт
	...
	Если РезультатЗапросаСпискаПродукции.ПродукцияОрганизации <> Неопределено Тогда
		...
		Для Каждого КлючИЗначение Из ДанныеПродукцииПоШтрихкодуEAN Цикл
			...
			НайденнаяСтрока = ТаблицаПродукции.Найти(ШтрихкодированиеИСКлиентСервер.GTINПоШтрихкодуEAN(ШтрихкодEAN), "GTIN");
...
Функция ЗапроситьСтатусыКодовМаркировки(КодыМаркировки, СтатусыКодовМаркировкиКеш = Неопределено) Экспорт
	...
	Для Каждого КодМаркировки Из КодыМаркировки Цикл
		НормализованныйКодМаркировки = ШтрихкодированиеМОТПКлиентСервер.НормализоватьКодМаркировки(КодМаркировки, Ложь);
		НормализованныеКодыМаркировки.Добавить(НормализованныйКодМаркировки);
		СоответствиеКодовМаркировки.Вставить(НормализованныйКодМаркировки, КодМаркировки);
	КонецЦикла;
	
	ТелоЗапроса = Новый Структура;
	ТелоЗапроса.Вставить("ids", НормализованныеКодыМаркировки);
	
	URLЗапроса = "private-office-api/private/v2/cis/status";
				...
				Если ТипЗнч(ДанныеОбработки) = Тип("Массив") Тогда
					...
					Для Каждого Элемент Из ДанныеОбработки Цикл
						...
						ВозвращаемоеЗначение.СтатусыКодовМаркировки.Вставить(
							СоответствиеКодовМаркировки[Элемент.cis], ПараметрыКодаМаркировки);
...
Функция ЗапроситьМРЦДляКодаМаркировки(КодМаркировки) Экспорт
	...
	URLЗапроса = СтрШаблон(
		"private-office-api/private/v2/cis/mrp?cis=%1",
...
Функция СоздатьЗаказНаФормированиеСпискаКодовМаркировкиУчастника(Организация, ТипУпаковки = "box") Экспорт
	...
	URLЗапроса = "private-office-api/api/v2/cis/my";
	РезультатЗапроса = ИнтеграцияИСМП.ОтправитьДанныеВСервис(URLЗапроса, ТелоЗапроса, КлючСессии, "POST", ИнтерфейсМОТПКлиентСервер.ПараметрыОтправкиHTTPЗапросов());
...
Функция ЗапроситьСтатусРегистрацииУчастника(ИНН) Экспорт
	...
	URLЗапроса = СтрШаблон(
		"private-office-api/private/v2/participants/%1/status",
		ИНН);
	РезультатЗапроса = ИнтеграцияИСМП.ПолучитьДанныеИзСервиса(URLЗапроса, КлючСессии, ИнтерфейсМОТПКлиентСервер.ПараметрыОтправкиHTTPЗапросов());
...
Функция ЗапроситьСписокКонтрагентовУчастника(Организация) Экспорт
	...
	URLЗапроса = "private-office-api/private/v2/partner/";
	РезультатЗапроса = ИнтеграцияИСМП.ПолучитьДанныеИзСервиса(URLЗапроса, КлючСессии, ИнтерфейсМОТПКлиентСервер.ПараметрыОтправкиHTTPЗапросов());
...
Функция ЗапроситьСписокПродукции() Экспорт
	...
	URLЗапроса = "private-office-api/private/v2/products/";
	РезультатЗапроса = ИнтеграцияИСМП.ПолучитьДанныеИзСервиса(URLЗапроса, КлючСессии, ИнтерфейсМОТПКлиентСервер.ПараметрыОтправкиHTTPЗапросов());
...
Функция ЗапроситьКарточкуПродукции(Идентификатор) Экспорт
	...
	URLЗапроса = СтрШаблон(
		"private-office-api/private/v2/products/%1",
		Идентификатор);
	РезультатЗапроса = ИнтеграцияИСМП.ПолучитьДанныеИзСервиса(URLЗапроса, КлючСессии, ИнтерфейсМОТПКлиентСервер.ПараметрыОтправкиHTTPЗапросов());
...
Функция ЗапроситьДокументПоИдентификатору(Идентификатор, ИмяВременногоФайла) Экспорт
	...
	URLЗапроса = СтрШаблон(
		"private-office-api/api/v2/documents/my/%1/body",
		Идентификатор);
...
Функция ЗапроситьСтатусЗаказа(ИдентификаторЗаказа) Экспорт
	...
	URLЗапроса = СтрШаблон(
		"private-office-api/api/v2/orders/%1/status",
		ИдентификаторЗаказа);
	РезультатЗапроса = ИнтеграцияИСМП.ПолучитьДанныеИзСервиса(URLЗапроса, КлючСессии, ИнтерфейсМОТПКлиентСервер.ПараметрыОтправкиHTTPЗапросов());
...
Функция ЗапроситьРезультатЗаказа(ИдентификаторЗаказа, ИмяВременногоФайла) Экспорт
	...
	URLЗапроса = СтрШаблон(
		"private-office-api/api/v2/orders/%1/result",
		ИдентификаторЗаказа);
...
Функция ЗапроситьИсториюДвиженияКодаМаркировки(КодМаркировки) Экспорт
	...
	URLЗапроса = СтрШаблон(
		"private-office-api/api/v2/cis/history?cis=%1",
		КодироватьСтроку(КодМаркировки, СпособКодированияСтроки.КодировкаURL));
	РезультатЗапроса = ИнтеграцияИСМП.ПолучитьДанныеИзСервиса(URLЗапроса, КлючСессии, ИнтерфейсМОТПКлиентСервер.ПараметрыОтправкиHTTPЗапросов());
...
Процедура ЗаполнитьДеревоУпаковокРекурсивно(ДеревоУпаковок, ДанныеВерхнегоУровня, КодыМаркировкиУпаковок, Кеш, Детализация, ВариантПолученияМРЦ)
	...
	Для Каждого КлючИЗначение Из КодыМаркировкиУпаковок Цикл
		...
		Если ТипЗнч(КлючИЗначение) <> Тип("Строка") Тогда // Логистическая упаковка или блок
			...
			Если Детализация = Неопределено
				...
				ИнтерфейсИСМПСлужебный.ПроверитьИсправитьДублиСтрокДереваУпаковок(КодМаркировкиУпаковки, Кеш.КодыМаркировки);
				...
				ЗаполнитьДеревоУпаковокРекурсивно(СтрокаДерева, СтрокаДерева, ВложенныеКодыМаркировки, Кеш, Детализация, ВариантПолученияМРЦ);
				...
				Кеш.КодыМаркировки.Вставить(КодМаркировкиУпаковки, СтрокаДерева);
				ОбновитьКешGTIN(СтрокаДерева, Кеш.GTIN);
			...
			Иначе
				...
				ДанныеСтрокиВместоДанныеВерхнегоУровня.Вставить("GTIN",             ?(ЭтоКодМаркировкиБлока, GTIN, ""));
				ДанныеСтрокиВместоДанныеВерхнегоУровня.Вставить("МРЦ",              ?(ЭтоКодМаркировкиБлока, МРЦ,  0));
				...
				ЗаполнитьДеревоУпаковокРекурсивно(ДеревоУпаковок, ДанныеСтрокиВместоДанныеВерхнегоУровня, ВложенныеКодыМаркировки, Кеш, Детализация, ВариантПолученияМРЦ);
		...
		Иначе // Табачная пачка
			...
			ИнтерфейсИСМПСлужебный.ПроверитьИсправитьДублиСтрокДереваУпаковок(КлючИЗначение, Кеш.КодыМаркировки);
			...
			Если Детализация = Неопределено
				...
				Кеш.КодыМаркировки.Вставить(КлючИЗначение, СтрокаДерева);
				ОбновитьКешGTIN(СтрокаДерева, Кеш.GTIN);
