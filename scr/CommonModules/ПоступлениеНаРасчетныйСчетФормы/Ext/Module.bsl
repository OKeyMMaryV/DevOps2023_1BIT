...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	бит_Казначейство.ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка);
...
Процедура ПодготовитьФормуНаСервере(Форма) Экспорт
	...
	// бит_Финанс изменения кода. Начало.
	бит_Казначейство.ПодготовитьФормуНаСервере(Форма);
	// бит_Финанс изменения кода. Конец.
...
Процедура НастроитьЭлементыДляНПД(Форма)
	...
	ВидОперацииОтражениеДохода = Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя;
	
	ВидимостьЭлементовНПД = Форма.ПлательщикНПД И ВидОперацииОтражениеДохода;
	
	Элементы.ГруппаЧекНПД.Видимость     = ВидимостьЭлементовНПД;
	Элементы.УслугаНПД.Видимость        = ВидимостьЭлементовНПД;
	...
	Если ВидимостьЭлементовНПД Тогда
		...
		Элементы.УслугаНПД.Видимость = ЧекСформирован Или ДоступнаИнтеграция;
...
Функция ТекущиеПараметрыОбъекта(Форма)
	...
	ПараметрыОбъекта = Новый Структура(
		...
		|ДоговорКонтрагента, СтатьяДвиженияДенежныхСредств,
	...
	Если Форма.СвойстваПлатежа <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыОбъекта, Форма.СвойстваПлатежа,
			"ДоговорКонтрагента, СтатьяДвиженияДенежныхСредств, СуммаПлатежа, СуммаУслуг");
	КонецЕсли;
...
Функция СвойстваСтрокРасшифровкиПлатежаСервер(Знач ПараметрыПлатежа, Знач ПолучатьДоговор) Экспорт
	...
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2021-07-27 (#4099)
	пСпособПогашенияЗадолженности = Неопределено;
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2021-07-27 (#4099)
	
	// +СБ. Широков Николай. 2014-12-04. ТЗ "Казначейство" 2.2.10.2 
	Если (ПараметрыПлатежа.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя)
		...
		Если СписокОрганизаций_НеПогашатьЗадолженность <> Неопределено 
			И СписокОрганизаций_НеПогашатьЗадолженность.НайтиПоЗначению(ПараметрыПлатежа.Организация) <> Неопределено Тогда
			
			//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2021-07-27 (#4099)
			//СвойстваПлатежа.СпособПогашенияЗадолженности = Перечисления.СпособыПогашенияЗадолженности.НеПогашать;
			пСпособПогашенияЗадолженности = Перечисления.СпособыПогашенияЗадолженности.НеПогашать
			//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2021-07-27 (#4099)
	...
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2021-07-27 (#4099)
	////ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2020-06-10 (#3704)
	//Если СвойстваПлатежа.СпособПогашенияЗадолженности = ПредопределенноеЗначение("Перечисление.СпособыПогашенияЗадолженности.Автоматически")
	//	ИЛИ СвойстваПлатежа.СпособПогашенияЗадолженности = ПредопределенноеЗначение("Перечисление.СпособыПогашенияЗадолженности.ПустаяСсылка") Тогда
	//	
	...
	//	//ЭтоОрганизацияОКЕЙ 		= ПараметрыПлатежа.Организация = Организация_ОКЕЙ ИЛИ ПараметрыПлатежа.Организация = Организация_Лоджистикс;
	//	#КонецОбласти
	//	#Область НовыйКод
	//	Организация_Лоджистикс 	= бит_БК_Общий.ПолучитьЗначениеНастройкиМеханизмаИмпортаДанных("Организации", "Организация Лоджистикс");
	//	ЭтоОрганизацияОКЕЙ 		= ПараметрыПлатежа.Организация = Организация_Лоджистикс;
	//	#КонецОбласти
	//	//ОКЕЙ Михайлова М.В.(ПервыйБИТ) Конец  2021-06-18 (#НТП_МП_02)
	//	
	//	Если НЕ ЭтоОрганизацияОКЕЙ Тогда
	//		СпособПогашенияЗадолженностиПоДоговору = Справочники.ДоговорыКонтрагентов.ОпределитьСпособПогашенияЗадолженностиПоДоговору(ПараметрыПлатежа.ДоговорКонтрагента);
	//		Если СпособПогашенияЗадолженностиПоДоговору <> Неопределено
	//			И СвойстваПлатежа.СпособПогашенияЗадолженности <> СпособПогашенияЗадолженностиПоДоговору Тогда
	//			СвойстваПлатежа.СпособПогашенияЗадолженности = СпособПогашенияЗадолженностиПоДоговору;
	//		КонецЕсли; 
	//	КонецЕсли;
	//
	//КонецЕсли; 
	////ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2020-06-10 (#3704) 
		
	Если пСпособПогашенияЗадолженности = Неопределено Тогда
		пСпособПогашенияЗадолженности = Справочники.ДоговорыКонтрагентов.ОпределитьСпособПогашенияЗадолженностиПоДоговору(ПараметрыПлатежа.ДоговорКонтрагента);
	КонецЕсли;
	
	Если пСпособПогашенияЗадолженности <> Неопределено
		И СвойстваПлатежа.СпособПогашенияЗадолженности <> пСпособПогашенияЗадолженности Тогда
		СвойстваПлатежа.СпособПогашенияЗадолженности = пСпособПогашенияЗадолженности;
	КонецЕсли;
	//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2021-07-27 (#4099)
