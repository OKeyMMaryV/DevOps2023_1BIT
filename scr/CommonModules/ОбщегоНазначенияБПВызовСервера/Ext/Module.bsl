...
Процедура ИзменитьАктивностьПоРегистратору(ДокументОбъект, Отказ,ВключитьАктивность = Истина) Экспорт
	...
	бит_МассивКУдалению = бит_КонтрольЗакрытогоПериода.ИзменитьАктивностьПоРегистраторуПереопределяемый(ДокументОбъект, Отказ);
	// бит_Финанс добавление кода конец }}  
	
	Для Каждого Набор ИЗ ДокументОбъект.Движения Цикл
		
		// бит_Финанс добавление кода. Начало: 21.11.2016 BF-321 {{
		Если НЕ бит_МассивКУдалению.Найти(Набор.Метаданные().Имя) = Неопределено Тогда
			Продолжить;
		КонецЕсли; 
		// бит_Финанс добавление кода конец }}  
...
Функция УстановитьСтатусыДокументов(СписокДокументов, СтатусДокумента) Экспорт
	...
	СтатусИзменен = РегистрыСведений.СтатусыДокументов.УстановитьСтатусыДокументов(
		СписокДокументов.ВыгрузитьЗначения(), СтатусДокумента);
