...
Процедура ФормаОтчетаПриСозданииНаСервере(Форма) Экспорт
	...
	БухгалтерскиеОтчеты.УстановитьВидимостьПредварительногоПросмотра(Форма);
...
// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-09-08 (#4347)
Процедура ДополнитьИнформациейОРНПТ(ДополнительнаяИнформация, СтруктураПараметров, Запрос, ПоискРНПТПоОснованию = Ложь) Экспорт
	...
	УчетНДСБП.ДополнитьИнформациейОРНПТ(ДополнительнаяИнформация, СтруктураПараметров, Запрос, ПоискРНПТПоОснованию);
...
Процедура ДополнитьПокупкиИнформациейОНомерахДекларацийНаТовары(ДополнительнаяИнформация, СтруктураПараметров, Запрос) Экспорт
	...
	УчетНДСБП.ДополнитьПокупкиИнформациейОНомерахДекларацийНаТовары(ДополнительнаяИнформация, СтруктураПараметров, Запрос);

КонецПроцедуры

Процедура ДополнитьИнформациейОПосредникахИПлатежах(ДополнительнаяИнформация, СтруктураПараметров, Запрос) Экспорт

	УчетНДСБП.ДополнитьИнформациейОПосредникахИПлатежах(ДополнительнаяИнформация, СтруктураПараметров, Запрос);

КонецПроцедуры

Процедура ДополнитьИнформациейОТНВЭД(ДополнительнаяИнформация, Запрос) Экспорт

	УчетНДСБП.ДополнитьИнформациейОТНВЭД(ДополнительнаяИнформация, Запрос);
...
Процедура ДополнитьПродажиИнформациейОНомерахДекларацийНаТовары(ДополнительнаяИнформация, СтруктураПараметров, Запрос) Экспорт
	...
	УчетНДСБП.ДополнитьПродажиИнформациейОНомерахДекларацийНаТовары(ДополнительнаяИнформация, СтруктураПараметров, Запрос);

КонецПроцедуры

Процедура ПреобразоватьПоказателиРНПТКСторнирующейЗаписи(МассивСвРегНом) Экспорт
	
	УчетНДСБП.ПреобразоватьПоказателиРНПТКСторнирующейЗаписи(МассивСвРегНом);
	
КонецПроцедуры
// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-09-08 (#4347)
...
Функция РеквизитыДляНадписиОСчетеФактуреПолученном(Знач Документ, СтруктураОтбора = Неопределено) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция РеквизитыДляНадписиОСчетеФактуреВыданном(Знач Документ, СтруктураОтбора = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СчетФактура) Тогда
		...
		РеквизитыСФ = Новый Структура(
			...
			| НомерИсправления,ДатаИсправления,Корректировка,Проведен",
			СчетФактура, "");
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СчетФактура,
			...
			|Представление,Проведен");
			
		ЗаполнитьЗначенияСвойств(РеквизитыСФ, ЗначенияРеквизитов, "Исправление,НомерИсправления,Представление,Проведен");
...
Функция НайтиПодчиненныйСчетФактуруПолученный(
	...
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		Возврат ДокументОснование;	
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Конец 2021-05-17 (#НТП-МП-03)
	КонецЕсли;
...
Функция НайтиПодчиненныйСчетФактуруВыданныйНаРеализацию(ДокументОснование, ИсключаемыйСФ = Неопределено, ПометкаУдаления = Ложь, СтруктураОтбора = Неопределено) Экспорт
	...
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Начало 2021-05-17 (#НТП-МП-03)
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.бит_ок_ОперацияАксапты") Тогда
		Возврат ДокументОснование;	
	//ОКЕЙ Лещенко Д.Ю. (БИТ) Конец 2021-05-17 (#НТП-МП-03)
	КонецЕсли;
...
Функция ДобавитьОснованиеВСчетФактуруПолученный(Основание, НомерСчетаФактурыПолученного, ДатаСчетаФактурыПолученного, ОбновлятьСтатусСчетаФактурыПоДокументу = Истина) Экспорт
...
// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-08 (#4208)
...
КонецФункции
// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-08 (#4208)
...
Функция СоздатьСчетФактуруПолученныйНаОсновании(Основание, НомерСчетаФактурыПолученного, ДатаСчетаФактурыПолученного, Продавец = Неопределено, ОбновлятьСтатусСчетаФактурыПоДокументу = Истина) Экспорт
	
	Возврат Документы.СчетФактураПолученный.СоздатьДокументНаОсновании(Основание, НомерСчетаФактурыПолученного, ДатаСчетаФактурыПолученного, Продавец, ОбновлятьСтатусСчетаФактурыПоДокументу);
...
Функция ПараметрыСчетаФактуры(СчетФактура) Экспорт
	...
	Результат = Новый Структура(
		...
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
		|ЕстьПрослеживаемыеТовары,
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-08-30 (#4214)
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-08-30 (#4214) 
	...
	Иначе
		...
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
		ЗаполнятьПризнакПослеживаемости = РезультатЗапроса.Колонки.Найти("ЕстьПрослеживаемыеТовары") <> Неопределено;
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
		Корректировка = Ложь;
		...
		Если Корректировка Тогда
			...
			Пока ВыборкаДокументы.Следующий() Цикл
				...
				Если ПерваяСтрока Тогда
					...
					+ ?(ЕстьКолонкаПродавец, ",Продавец", "") + ?(ЗаполнятьПризнакПослеживаемости, ",ЕстьПрослеживаемыеТовары", "");
					//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
		...
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				...
				Иначе
					//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
					Если ЗаполнятьПризнакПослеживаемости 
						И Не ЗначениеЗаполнено(Результат.ЕстьПрослеживаемыеТовары) Тогда
						Результат.ЕстьПрослеживаемыеТовары = Выборка.ЕстьПрослеживаемыеТовары;
					КонецЕсли;
					//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
...
Функция ТекстЗапросаВозвратТоваровЧерезКомиссионера()
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
	|	,МАКСИМУМ(ВложенныйЗапрос.ЕстьПрослеживаемыеТовары) КАК ЕстьПрослеживаемыеТовары
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
	|ПОМЕСТИТЬ ВТ_Суммы
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		КОНЕЦ КАК ЕстьНДС
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
	|		,ОтчетКомиссионераОПродажахТоварыВозвращенные.ПрослеживаемыйТовар КАК ЕстьПрослеживаемыеТовары
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
	|	ИЗ
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
	|	,МАКСИМУМ(ВТ_Суммы.ЕстьПрослеживаемыеТовары) КАК ЕстьПрослеживаемыеТовары
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
	|ИЗ
...
Функция ТекстЗапросаРеализацияОтгруженныхТоваров()
	...
	|		КОНЕЦ) КАК ЕстьНДС
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
	|	,МАКСИМУМ(РеализацияТоваровУслугТовары.ПрослеживаемыйТовар) КАК ЕстьПрослеживаемыеТовары
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
	|	,МАКСИМУМ(ВТ_ТаблицыНДС.ЕстьПрослеживаемыеТовары) КАК ЕстьПрослеживаемыеТовары
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
	|ИЗ
	|	ВТ_РеализацияОтгруженныхТоваров КАК ВТ_РеализацияОтгруженныхТоваров
...
// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-10 (#4208)
//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-08-30 (#4214) 
...
//	"ВЫБРАТЬ
//	|	ВЫБОР
//	|		КОГДА СчетФактураВыданныйДокументыОснования.ДокументОснование ССЫЛКА Документ.ОтчетКомиссионераОПродажах
...
//	|		ИНАЧЕ 0
//	|	КОНЕЦ КАК СуммаНДСДокументаКомиссия,
//	|	СчетФактураВыданный.ИсходныйДокумент КАК ИсходныйДокумент
//	|ИЗ
...
//	|		ПО Товары.Ссылка.ДокументРеализации = СчетФактураВыданный.ДокументОснование
//	|ГДЕ
...
//	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СчетФактураВыданный КАК СчетФактураВыданный
//	|		ПО АгентскиеУслуги.Ссылка.ДокументРеализации = СчетФактураВыданный.ДокументОснование
//	|ГДЕ
...
Функция ТекстЗапросаКорректировкаРеализации(ТолькоПредварительнуюТаблицу = Ложь)
	...
	Иначе
		...
		|	ВЫБОР
		|		КОГДА НЕ &КорректировочныйСчетФактура
		|				ИЛИ Товары.Сумма - Товары.СуммаДоИзменения <> 0
		|				ИЛИ Товары.СуммаНДС - Товары.СуммаНДСДоИзменения <> 0
		|			ТОГДА Товары.ПрослеживаемыйТовар
		|		ИНАЧЕ ЛОЖЬ
		...
		|	СУММА(СуммаДокументаКомиссия),
		|	СУММА(СуммаНДСДокументаКомиссия),
		|	МАКСИМУМ(ЕстьПрослеживаемыеТовары)
		|ПО
		|	ОБЩИЕ,
		|	Документ,
		|	ИсходныйДокумент";
...
Функция ТекстЗапросаВыданныеСчетаФактурыПеревыставленныеКомитенту()
	...
	|		КОНЕЦ) КАК ЕстьНДС
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-10-01 (#4377)
	//|	,МАКСИМУМ(ОтчетКомитентуОПродажахТовары.ПрослеживаемыйТовар) КАК ЕстьПрослеживаемыеТовары
	|	,МАКСИМУМ(ЛОЖЬ) КАК ЕстьПрослеживаемыеТовары
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-10-01 (#4377)
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
	|ПОМЕСТИТЬ ВТ_Суммы
	|ИЗ
...
Функция ТекстЗапросаВыданныеСчетаФактурыПокупателямПоОтчетуКомиссионера()
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
	|		,ОтчетКомиссионераОПродажахТовары.ПрослеживаемыйТовар КАК ЕстьПрослеживаемыеТовары
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
	|	ИЗ
...
Функция ТекстЗапросаАвансовыйОтчет()
	...
	|	КОНЕЦ КАК ЕстьНДС
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
	|	,ЛОЖЬ КАК ЕстьПрослеживаемыеТовар
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
	|ИЗ
...
Функция ТекстЗапросаВозвратОтПокупателя()
	...
	|	СчетФактураВыданныйДокументыОснования.Ссылка КАК ИсходныйДокумент
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-10-01 (#4381) 
	//|	,ВозвратТоваровОтПокупателяТовары.ПрослеживаемыйТовар КАК ЕстьПрослеживаемыеТовары
	|	,ЛОЖЬ КАК ЕстьПрослеживаемыеТовары
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-10-01 (#4381) 
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
	|ИЗ
...
Функция ТекстЗапросаВозвратПоставщику()
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-16 (#4214)
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-10-01 (#4381)
	//|	,Товары.ПрослеживаемыйТовар КАК ЕстьПрослеживаемыеТовары
	|	,ЛОЖЬ КАК ЕстьПрослеживаемыеТовары
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-10-01 (#4381)
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-16 (#4214) 
	|ИЗ
...
Функция ТекстЗапросаПередачаОС()
	...
	|	ПередачаОСОС.Ссылка.Контрагент КАК Контрагент,
	|	ПередачаОСОС.Ссылка.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ЕСТЬNULL(ДоговорыКонтрагентов.ГосударственныйКонтракт.Код, """") КАК ИдентификаторГосКонтракта,
	|	СУММА(ПередачаОСОС.Сумма + ВЫБОР
	|			КОГДА ПередачаОСОС.Ссылка.СуммаВключаетНДС
	|				ТОГДА 0
	|			ИНАЧЕ ПередачаОСОС.СуммаНДС
	|		КОНЕЦ) КАК СуммаДокумента,
	|	ПередачаОСОС.Ссылка.ВалютаДокумента КАК ВалютаДокумента,
	|	СУММА(ПередачаОСОС.СуммаНДС) КАК СуммаНДСДокумента,
	|	0 КАК СуммаДокументаКомиссия,
	|	0 КАК СуммаНДСДокументаКомиссия,
	|	СУММА(ВЫБОР
	|			КОГДА ПередачаОСОС.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
	...
	|			ИНАЧЕ 1
	|		КОНЕЦ) КАК ЕстьНДС,
	|	МАКСИМУМ(ПередачаОСОС.ПрослеживаемыйТовар) КАК ЕстьПрослеживаемыеТовары
	|ИЗ
	|	Документ.ПередачаОС.ОС КАК ПередачаОСОС
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|		ПО ПередачаОСОС.Ссылка.ДоговорКонтрагента = ДоговорыКонтрагентов.Ссылка
	|ГДЕ
	|	ПередачаОСОС.Ссылка В(&ДокументыОснования)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПередачаОСОС.Ссылка,
...
Функция ТекстЗапросаРеализацияТоваровУслуг()
	...
	|	СУММА(Таблица.СуммаНДС) КАК СуммаНДСДокумента,
	|	СУММА(ВЫБОР
	|			КОГДА Таблица.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыНаСкладе)
	|					ИЛИ Таблица.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыПереданныеНаКомиссию)
	|				ТОГДА Таблица.Сумма + ВЫБОР
	|						КОГДА Таблица.Ссылка.СуммаВключаетНДС
	|							ТОГДА 0
	...
	|			ИНАЧЕ 1
	|		КОНЕЦ) КАК ЕстьНДС,
	|	МАКСИМУМ(Таблица.ПрослеживаемыйТовар) КАК ЕстьПрослеживаемыеТовары
	|ИЗ
	...
	|	СУММА(Таблица.СуммаНДС),
	|	ВЫБОР
	|		КОГДА ДоговорыКонтрагентовКомиссия.УчетАгентскогоНДС
	|				И Таблица.Ссылка.КурсВзаиморасчетов > 1
	|			ТОГДА СУММА(Таблица.Сумма * (Таблица.Ссылка.КурсВзаиморасчетов / ВЫБОР
	...
	|						КОГДА Таблица.Ссылка.СуммаВключаетНДС
	|							ТОГДА 0
	|						ИНАЧЕ Таблица.СуммаНДС * (Таблица.Ссылка.КурсВзаиморасчетов / ВЫБОР
	|								КОГДА Таблица.Ссылка.КратностьВзаиморасчетов = 0
	|									ТОГДА 1
	|								ИНАЧЕ Таблица.Ссылка.КратностьВзаиморасчетов
	|							КОНЕЦ)
	|					КОНЕЦ)
	|		ИНАЧЕ СУММА(Таблица.Сумма + ВЫБОР
	|					КОГДА Таблица.Ссылка.СуммаВключаетНДС
	|						ТОГДА 0
	|					ИНАЧЕ Таблица.СуммаНДС
	|				КОНЕЦ)
	...
	|		КОГДА ДоговорыКонтрагентовКомиссия.УчетАгентскогоНДС
	|				И Таблица.Ссылка.КурсВзаиморасчетов > 1
	|			ТОГДА СУММА(Таблица.СуммаНДС * (Таблица.Ссылка.КурсВзаиморасчетов / ВЫБОР
	|						КОГДА Таблица.Ссылка.КратностьВзаиморасчетов = 0
	|							ТОГДА 1
	|						ИНАЧЕ Таблица.Ссылка.КратностьВзаиморасчетов
	|					КОНЕЦ))
	|		ИНАЧЕ СУММА(Таблица.СуммаНДС)
	|	КОНЕЦ,
	|	СУММА(ВЫБОР
	|			КОГДА Таблица.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
	|				ТОГДА 0
	...
	|	Таблица.Ссылка.ДоговорКонтрагента,
	|	ЕСТЬNULL(ДоговорыКонтрагентов.ГосударственныйКонтракт.Код, """"),
	|	ДоговорыКонтрагентовКомиссия.УчетАгентскогоНДС,
	|	Таблица.Ссылка.ВалютаДокумента,
	|	Таблица.Ссылка.КурсВзаиморасчетов
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ТекстЗапросаПоступлениеТоваровУслуг()
	...
	|	ТаблицыПоступлениеТоваровУслуг.СуммаДокумента КАК СуммаДокумента,
	|	ТаблицыПоступлениеТоваровУслуг.ВалютаДокумента КАК ВалютаДокумента,
	|	ТаблицыПоступлениеТоваровУслуг.СуммаНДСДокумента КАК СуммаНДСДокумента,
	|	ВЫБОР
	|		КОГДА ТаблицыПоступлениеТоваровУслуг.СуммаДокументаКомиссия <> 0
	|			ТОГДА ТаблицыПоступлениеТоваровУслуг.СуммаДокументаКомиссия
	...
	|			ТОГДА ТаблицыПоступлениеТоваровУслуг.СуммаНДСДокументаКомиссия
	|		ИНАЧЕ ТаблицыПоступлениеТоваровУслуг.СуммаНДСДокументаКомиссия
	|	КОНЕЦ КАК СуммаНДСДокументаКомиссия,
	|	ТаблицыПоступлениеТоваровУслуг.ЕстьНДС КАК ЕстьНДС,
	|	ТаблицыПоступлениеТоваровУслуг.ЕстьПрослеживаемыеТовары КАК ЕстьПрослеживаемыеТовары
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		МАКСИМУМ(ТаблицаОборудование.ПрослеживаемыйТовар) КАК ЕстьПрослеживаемыеТовары
	|	ИЗ
	...
	|		МАКСИМУМ(ТаблицаТовары.ПрослеживаемыйТовар)
	|	ИЗ
