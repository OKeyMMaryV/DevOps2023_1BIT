...
Процедура РассчитатьСтатусОформления(МетаданныеДокумента, ДокументОснование, РеквизитыДокументаОснования, ЗаписьТребуется) Экспорт
	...
	|	СтатусыОформления.Архивный         КАК Архивный
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		...
		Иначе
			...
			ДанныеЗаписи.Архивный = Выборка.Архивный;
...
Функция СтатусОформления(МетаданныеДокумента, ДокументОснование) Экспорт
	...
	Запрос = ИнтеграцияИС.МенеджерОбъектаПоПолномуИмени(МетаданныеДокумента.ПолноеИмя()).ЗапросСтатусаОформления(ДокументОснование);
...
//   * ЕстьОшибки                                 - Булево - флаг наличия ошибок
//
Функция ПараметрыРезультатаПроверкиДанныхТСД(ПараметрыСканирования)
...
//
// Параметры:
//   МассивСчитанныхДанных - Массив    - загруженные штрихкоды
//   ПараметрыПроверки     - Структура - (См. ПараметрыПроверкиДанныхТСД)
//
// Возвращаемое значение:
//   Структура - (См. ПараметрыРезультатаПроверкиДанныхТСД)
//
Функция РезультатПроверкиДанныхТСД(МассивСчитанныхДанных, ПараметрыПроверки) Экспорт
	...
	ОбрезатьДанныеТСДДоДетализации(МассивСчитанныхДанных, Детализация);
	ЕстьИерархия = УпорядочитьДанныеТСД(МассивСчитанныхДанных);
	
	СоответствиеСчитанныхАкцизныхМарок        = Новый Соответствие;
	СоответствиеСчитанныхДанныхНеАкцизныхМарок = Новый Соответствие;
	
	Результат = ПараметрыРезультатаПроверкиДанныхТСД(ПараметрыСканирования);
	...
	Для Каждого ЭлементМассива Из МассивСчитанныхДанных Цикл
		...
		Если ЭлементМассива.Свойство("ШтрихкодМаркиАлкогольнойПродукции")
			...
			Если ЭлементМассива.Свойство("ШтрихкодУпаковки") Тогда
				ДанныеМаркируемойПродукции.Вставить("Родитель", ЭлементМассива.ШтрихкодУпаковки);
			КонецЕсли;
		...
		ИначеЕсли АкцизныеМаркиВызовСервера.ЭтоШтрихкодАкцизнойМарки(ЭлементМассива.Штрихкод) Тогда
		...
		Иначе
			...
			Если ЭлементМассива.Свойство("ШтрихкодУпаковки") Тогда
				НоваяСтрока.Родитель = ЭлементМассива.ШтрихкодУпаковки;
			КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНеАкцизныеМарки Цикл
		...
		Если СтрокаТаблицы.ЭтоУпаковка Тогда
			...
			Если Детализация = Неопределено Или Детализация = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки Тогда
				Продолжить;
			ИначеЕсли ПараметрыПроверки.ЭтоФормаДокумента Тогда
				Продолжить;
			Иначе
				Результат.Упаковки.Добавить(Новый Структура("Штрихкод,Родитель",СтрокаТаблицы.ШтриховойКод, СтрокаТаблицы.Родитель));
			КонецЕсли;
		...
		ИначеЕсли СтрокаТаблицы.ЭтоИмеющаясяВБазеУпаковка Тогда
			...
			Если Детализация <> Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки Тогда
				Результат.Упаковки.Добавить(Новый Структура("Штрихкод,Родитель",СтрокаТаблицы.ШтриховойКод, СтрокаТаблицы.Родитель));
			КонецЕсли;
		...
		ИначеЕсли СтрокаТаблицы.ЭтоПартионнаяАлкогольнаяПродукция Тогда
			...
			Если ПараметрыПроверки.ЭтоФормаДокумента Тогда
				...
				Результат.ПартионнаяАлкогольнаяПродукция.Добавить(СоответствиеСчитанныхДанныхНеАкцизныхМарок[СтрокаТаблицы.ШтриховойКод]);
	...
	Если Детализация = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Полная Тогда
		Если НовыеУпаковки.Количество() + СуществующиеУпаковки.Количество() > 1 И Не ЕстьИерархия Тогда
			ПроверитьКорректностьСчитанныхТСДУпаковок(Результат, 
				...
				НовыеУпаковки,
				СуществующиеУпаковки,
				ПараметрыПроверки.СоответствиеШтрихкодовСтрокДерева);
			КонецЕсли;
	ИначеЕсли Детализация = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки Тогда
	...
	ПолучитьДанныеПоШтрихКодамТСД(Результат, ПараметрыСканирования, ПараметрыПроверки);
	
	Если Не Результат.ЕстьОшибкиВДеревеУпаковок
		И ПараметрыСканирования.ЗапрашиватьНоменклатуру Тогда
		ПроверитьНеобходимостьУказанияДополнительныхДанныхДляАкцизныхМарок(Результат, ПараметрыСканирования);
	КонецЕсли;
	
	АкцизныеМаркиЕГАИС.ДанныеДокументаОснования(ПараметрыСканирования);
...
// 
// Параметры:
//   Результат                    - Структура - (См. ПараметрыРезультатаПроверкиДанныхТСД)
//   ДетализацияСтруктурыХранения - Неопределено, ПеречислениеСсылка.ДетализацияСтруктурыХраненияАлкогольнойПродукции - 
...
Процедура ПроверитьЗаполнениеДанныхТСД(Результат, ДетализацияСтруктурыХранения, ВсегоУпаковок, ЕстьИерархия)
	...
	Если Всего = 0 Тогда
		...
		Результат.ТекстОшибки = НСтр("ru = 'В считанных штриховых кодах нет данных ни по упаковкам, ни по алкогольной продукции.'");
	КонецЕсли;
	
	Если ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.КоробаСБутылками 
		Или ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Полная Тогда
		...
		Если Марок > 0
			...
			Результат.ЕстьОшибки = Истина;
			Результат.ТекстОшибки = НСтр("ru = 'Одновременно считана маркируемая алкогольная продукция и несколько упаковок. Данные обработаны не будут.'");
		КонецЕсли;
...
// При необходимости переупорядочивает данные поступившие из ТСД по правилу: вложение упаковки всегда должно 
//   быть в массиве после строки самой упаковки
// 
// Параметры:
//   ДанныеТСД - Массив - данные поступившие с ТСД
Функция УпорядочитьДанныеТСД(ДанныеТСД)
	...
	Если Уровень = 1 Тогда
		Возврат Ложь;
	КонецЕсли;
	ТаблицаСоответствия.Сортировать("Уровень Убыв");
...
Процедура ОбрезатьДанныеТСДДоДетализации(ДанныеТСД, Знач Детализация)
	...
	Если Детализация = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Полная Тогда
		Возврат;
	ИначеЕсли Детализация = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.КоробаСБутылками Тогда
		Детализация = Истина;
	Иначе
		Детализация = Ложь;
	КонецЕсли;
...
Процедура ПроверитьНеобходимостьУказанияДополнительныхДанныхДляАкцизныхМарок(ЗагрузкаДанныхТСД, ПараметрыСканирования)
	...
	Для Каждого ЭлементМассива Из ЗагрузкаДанныхТСД.МаркируемаяПродукция Цикл
		
		ТребуетсяУказаниеДопДанных = Ложь;
		
		Если ПараметрыСканирования.ЗапрашиватьНоменклатуру
			И (Не ЗначениеЗаполнено(ЭлементМассива.ДанныеШтрихкода.Номенклатура)
			Или Не ЗначениеЗаполнено(ЭлементМассива.ДанныеШтрихкода.АлкогольнаяПродукция)) Тогда
			ТребуетсяУказаниеДопДанных = Истина;
			ЗагрузкаДанныхТСД.ПараметрыУточненияДанных = Новый Структура("АлкогольнаяПродукция, Номенклатура, Характеристика",
				ЭлементМассива.ДанныеШтрихкода.АлкогольнаяПродукция, ЭлементМассива.ДанныеШтрихкода.Номенклатура, ЭлементМассива.ДанныеШтрихкода.Характеристика);
		КонецЕсли;
	...
	ЗагрузкаДанныхТСД.МаркируемаяПродукция                       = МассивНеТребуетсяУказаниеДанных;
	ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукцияДопУказание = МассивТребуетсяУказаниеДанных;
	
КонецПроцедуры

Процедура ПолучитьДанныеПоШтрихкодамТСД(РезультатПроверки, ПараметрыСканирования, ПараметрыПроверки)
	...
	ДанныеПоШтрихКодам = ШтрихкодированиеИС.ДанныеПоШтрихкодам(МассивШтрихкодов, ПараметрыСканирования);
	...
	Для Каждого ЭлементМассива Из РезультатПроверки.МаркируемаяПродукция Цикл
		...
		Иначе
			...
			Если ПараметрыСканирования.ЗапрашиватьНоменклатуру
				И (Не ЗначениеЗаполнено(ЭлементМассива.ДанныеШтрихкода.Номенклатура)
				Или Не ЗначениеЗаполнено(ЭлементМассива.ДанныеШтрихкода.АлкогольнаяПродукция)) Тогда
				РезультатПроверки.ТребуетсяУточнениеДанных = Истина;
			КонецЕсли;
