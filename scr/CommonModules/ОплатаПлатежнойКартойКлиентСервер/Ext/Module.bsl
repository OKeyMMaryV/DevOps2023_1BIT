...
Процедура СформироватьПредставлениеВидаОплаты(Форма) Экспорт
	...
	Если Форма.ИмяФормы = "Документ.ОплатаПлатежнойКартой.Форма.ФормаДокумента" И Объект.СчетКасса =  ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПереводыВПути") Тогда
		ТекстВидОплаты = "" 
						+ ?(ЗначениеЗаполнено(Объект.ок_ВидыПереводов), Строка(Объект.ок_ВидыПереводов), НСтр("ru = '<Вид переводов>'"))
						+", " + ?(ЗначениеЗаполнено(Объект.ок_СтатьяДвиженияДенежныхСредств), Строка(Объект.ок_СтатьяДвиженияДенежныхСредств), НСтр("ru = '<Статья ДДС>'"));
...
Процедура УправлениеФормой(Форма) Экспорт
	...
	Элементы.ТекстНеобходимоЗаполнитьПатент.Видимость = (ЭтоРозничнаяВыручка И Форма.УчетВПродажныхЦенах)
	...
	Если Элементы.Найти("ГруппаРасшифровкаПлатежа") <> Неопределено Тогда
		Элементы.ГруппаРасшифровкаПлатежа.Видимость =
			Не Объект.БезЗакрывающихДокументов И Не (Форма.ПлательщикНПД И ЭтоРозничнаяВыручка) И КоличествоСтрок = 1;
	КонецЕсли;
	...
	Если Элементы.Найти("ГруппаСтраницы") <> Неопределено Тогда
		Элементы.ГруппаСтраницы.Видимость = Не Объект.БезЗакрывающихДокументов
			И Не (Форма.ПлательщикНПД И ЭтоРозничнаяВыручка);
	КонецЕсли;
