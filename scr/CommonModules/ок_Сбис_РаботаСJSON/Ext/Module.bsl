...
//&НаКлиенте
Функция ПреобразоватьЗначениеВJSON(ЗначениеПреобразовать, Отказ, ФормироватьОбъект=Истина) Экспорт
	...
	СтрокаJSON = "";
	Если ТипЗнч(ЗначениеПреобразовать) = Тип("Структура") Тогда
		СтрокаJSON = "{"+ЗначениеВJSON(ЗначениеПреобразовать)+"}";
	ИначеЕсли ТипЗнч(ЗначениеПреобразовать) = Тип("Массив") Тогда
		СтрокаJSON = "["+ЗначениеВJSON(ЗначениеПреобразовать)+"]";
	Иначе
		СтрокаJSON = ЗначениеВJSON(ЗначениеПреобразовать);
	КонецЕсли;
	Если ФормироватьОбъект	Тогда
		Возврат СтрокаJSON;
	КонецЕсли;
	//Если формируем не объект(строку), то дополнительно экранировать спец. символ и символ экрана"\". 
	//При обратной распаковке, снимутся доп. экраны и останется строка, которую уже можно распаковать в json.
	СтрокаJSON	= """" +	СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрокаJSON,	
							"""", "\"""),//экранировать двойные кавычки
							"\t", "\\t"),//экранировать спец символы
							"\n", "\\n"),
							"\f", "\\f"),
							"\r", "\\r"),
							"\b", "\\b"),
							"\\""", "\\\""")//добавить экранирование для появления экранированного слеша в запросе
							+ """";
	Возврат СтрокаJSON;
...
//&НаКлиенте
Функция ЗначениеВJSON(ЗначениеПреобразовать) Экспорт 
	...
	Если ТипЗнч(ЗначениеПреобразовать)	= Тип("Структура")	Тогда
		Для Каждого Элемент Из ЗначениеПреобразовать Цикл
			Если ТипЗнч(Элемент.Значение) = Тип("Строка") Тогда
				...
				ТекстРезультат = ТекстРезультат+""""+Элемент.Ключ+""":"""+ЗначениеВJSON(Элемент.Значение)+""",";
			ИначеЕсли ТипЗнч(Элемент.Значение) = Тип("Число") Тогда
				ТекстРезультат = ТекстРезультат+""""+Элемент.Ключ+""":"""+строка(Элемент.Значение)+""",";
			ИначеЕсли ТипЗнч(Элемент.Значение) = Тип("Дата") Тогда
				ТекстРезультат = ТекстРезультат+""""+Элемент.Ключ+""":"""+строка(Элемент.Значение)+""",";
			ИначеЕсли ТипЗнч(Элемент.Значение) = Тип("Неопределено") Тогда
				ТекстРезультат = ТекстРезультат+""""+Элемент.Ключ+""":"""",";
			...
			ИначеЕсли ТипЗнч(Элемент.Значение) = Тип("Массив") Тогда
				...
				Для Каждого ЭлементМассива Из Элемент.Значение Цикл
					...
					ТекстРезультат = ТекстРезультат+ЗначениеВJSON(ЭлементМассива);
					ТекстРезультат = ТекстРезультат+"},";
				КонецЦикла;
			...
			ИначеЕсли ТипЗнч(Элемент.Значение) = Тип("Булево") Тогда
				ТекстРезультат = ТекстРезультат+""""+Элемент.Ключ+""":"""+?(Элемент.Значение, "Истина", "Ложь")+""",";
			КонецЕсли;
		КонецЦикла;
		Если Прав(ТекстРезультат, 1) = "," Тогда
			ТекстРезультат = Лев(ТекстРезультат, СтрДлина(ТекстРезультат)-1);
		КонецЕсли;
	ИначеЕсли	 ТипЗнч(ЗначениеПреобразовать)	= Тип("Строка")	Тогда
		ТекстРезультат = СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(ЗначениеПреобразовать, 
