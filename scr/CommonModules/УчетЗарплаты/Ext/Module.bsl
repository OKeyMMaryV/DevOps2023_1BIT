...
Функция ДополнитьТаблицуЗаработнойПлатыДаннымиПоНДФЛ(НачисленнаяЗарплатаИВзносы, НачисленныйНДФЛ, Организация, ПериодРегистрации, Регистратор)
	...
	Если УчетнаяПолитика.ПрименяетсяУСН(Организация, ПериодРегистрации) Тогда
		...
		Запрос.УстановитьПараметр("УчитыватьСтатьюРасходов",              ПолучитьФункциональнуюОпцию("УчетЗарплатыИКадровВоВнешнейПрограмме"));
		...
		|	И РасходыПриУСНОбороты.СчетУчета = &СчетУчетаНДФЛ
...
Процедура ПроверитьЗаполненностьСпособаОтраженияВУчете(ТаблицаЗаработнойПлаты, Реквизиты)
	...
	ВидыОперацийНачисления = Перечисления.ВидыОперацийПоЗарплате.ВидыОперацийПоГруппам("Начисления");
...
Функция ДополнитьТаблицуРезервовАналитикойБухУчета(ТаблицаРезервов)
	...
	|	ТаблицаРезервов.ОблагаетсяЕНВД,
	...
	|	&СтатьяДоходовРезервыПоОтпускам,
...
Процедура СформироватьПроводкиПоВыплатеЗарплаты(ТаблицаВыплатыЗаработнойПлаты, Реквизиты, Движения, Отказ)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВыплаты Цикл
		...
		Если СтрокаТаблицы.СуммаКВыплате <> 0 Тогда
			...
			//++СВВ Сапожников Вадим 11.08.15
			Проводка.Содержание   = "Перечисление ЗП по ведомости №" + СтрокаТаблицы.Ведомость.Номер + " от " + Формат(СтрокаТаблицы.Ведомость.Дата,"ДФ=dd.MM.yy");
			//--СВВ			
...
Функция НалогСДивидендовАкционера(ДатаПолученияДохода, Акционер, Начислено, ВычетПоНДФЛ = 0, КатегорияДивидендов = Неопределено) Экспорт
	
	Возврат УчетНДФЛБазовый.НалогСДивидендовАкционера(ДатаПолученияДохода, Акционер, Начислено, ВычетПоНДФЛ, КатегорияДивидендов);
...
Функция ПодготовитьПараметрыДвиженийОтраженияДивидендов(ТаблицаДивиденды, ТаблицаРеквизиты)
	...
	СписокОбязательныхКолонок = ""
	+ "Период,"					// <Дата>
	+ "Учредитель,"				// <Ссылка на справочник "Физические лица">
...
Процедура ОтразитьДивидендыВУчетеНДФЛ(Реквизиты, ТаблицаДивиденды, Движения, Отказ)
	...
	ТаблицаДивиденды.Колонки.СуммаДохода.Имя ="Начислено";
	ТаблицаДивиденды.Колонки.Учредитель.Имя  ="Акционер";
	ТаблицаДивиденды.Колонки.СуммаНалога.Имя ="НДФЛ";
	ТаблицаДивиденды.Колонки.Добавить("ВычетПоНДФЛ", Новый ОписаниеТипов("Число"));
	
	УчетНДФЛБазовый.СформироватьДоходыИНДФЛСДивидендов(Реквизиты.Регистратор,
		Движения,
		Отказ,
...
Процедура РегистрацияУдержанногоНДФЛ(ТаблицаВыплатаДивидендов, Отказ, Движения, Реквизиты)
	...
	Если НЕ Отказ Тогда
		...
		|	&СуммаВыплаченная КАК СуммаВыплаты,
		...
		Запрос.УстановитьПараметр("Ссылка",           ТаблицаВыплатаДивидендов[0].НачислениеДивидендов);
		Запрос.УстановитьПараметр("СуммаВыплаченная", ТаблицаВыплатаДивидендов[0].СуммаКВыплате);
		Запрос.Выполнить();
		
		// учет фактической даты получения доходов НДФЛ
		УчетФактическиПолученныхДоходов.ЗарегистрироватьНовуюДатуПолученияДохода(
			Реквизиты.Регистратор,
			Движения,
			Запрос.МенеджерВременныхТаблиц,
			Реквизиты.Период,
		...
		УчетНДФЛБазовый.РассчитатьИЗарегистрироватьУдержанныеНалогиСДивидендов(
			Реквизиты.Регистратор, 
			Движения, 
			Отказ, 
			Реквизиты.Организация,
...
Процедура СоздатьПараметрыУчетаЗарплаты(Организация) Экспорт
	...
	НаборЗаписей = РегистрыСведений.ПроизводственныеКалендариОрганизаций.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Организация.Установить(Организация);
	НаборЗаписей.Прочитать();
	...
	Если НаборЗаписей.Количество() = 0 Тогда
		...
		Если НЕ ОсновнойПроизводственныйКалендарь = Неопределено Тогда
			Запись = НаборЗаписей.Добавить();
			Запись.Организация = Организация;
			Запись.ПроизводственныйКалендарь = КалендарныеГрафики.ОсновнойПроизводственныйКалендарь();
			НаборЗаписей.ДополнительныеСвойства.Вставить("ПропуститьПроверкуЗапретаИзменения", Истина);
			НаборЗаписей.Записать();
		КонецЕсли;
...
Функция СпособОтраженияПоУмолчанию()
	...
	Иначе
		...
		ЕНВД = Ложь;
		РегистрыСведений.ПорядокОтраженияЗарплатыВБухУчете.СоздатьПоУмолчанию(СпособОтраженияЗарплатыВБухучете, ЕНВД);
...
Функция ЗарегистрированыКадровыеМероприятия(Организация, Период) Экспорт
	...
	//   2. Зарегистрированы кадровые документы
	//   3. Если простой кадровый учет - заполнены дата приема или увольнения
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ЗаявлениеОПредоставленииСведенийОТрудовойДеятельности.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЗаявлениеОПредоставленииСведенийОТрудовойДеятельности КАК ЗаявлениеОПредоставленииСведенийОТрудовойДеятельности
	|ГДЕ
	|	ЗаявлениеОПредоставленииСведенийОТрудовойДеятельности.Дата МЕЖДУ &НачалоПериода И &ОкончаниеПериода
	|	И ЗаявлениеОПредоставленииСведенийОТрудовойДеятельности.Организация = &Организация
	|	И ЗаявлениеОПредоставленииСведенийОТрудовойДеятельности.Проведен
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|ГДЕ
	|	ПриемНаРаботу.ДатаПриема МЕЖДУ &НачалоПериода И &ОкончаниеПериода
	...
	|ГДЕ
	|	КадровыйПеревод.ДатаНачала МЕЖДУ &НачалоПериода И &ОкончаниеПериода
	...
	|ГДЕ
	|	Увольнение.ДатаУвольнения МЕЖДУ &НачалоПериода И &ОкончаниеПериода
...
Функция РеквизитыПлатежногоПорученияПоДатеНомеру(ДатаПП, НомерПП)
	...
	Возврат СтрШаблон(НСтр("ru = '№ %1 от %2'", ОбщегоНазначения.КодОсновногоЯзыка()), НомерПП, Формат(ДатаПП, "ДФ=дд.ММ.гггг"));
...
Процедура ЗаполнитьПроизводственныйКалендарьПоУмолчанию() Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПроизводственныеКалендариОрганизаций.Организация КАК Организация
	|ПОМЕСТИТЬ ВТ_КалендариОрганизаций
	|ИЗ
	|	РегистрСведений.ПроизводственныеКалендариОрганизаций КАК ПроизводственныеКалендариОрганизаций
	|ГДЕ
	|	ПроизводственныеКалендариОрганизаций.ПроизводственныйКалендарь <> ЗНАЧЕНИЕ(Справочник.ПроизводственныеКалендари.ПустаяСсылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|				(ВЫБРАТЬ
	|					ВТ_КалендариОрганизаций.Организация
	|				ИЗ
	|					ВТ_КалендариОрганизаций КАК ВТ_КалендариОрганизаций)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.Организация               = Выборка.Организация;
			НоваяЗапись.ПроизводственныйКалендарь = ОсновнойПроизводственныйКалендарь;
			ОбновлениеИнформационнойБазы.ЗаписатьНаборЗаписей(НаборЗаписей);
			
		Исключение
			ОтменитьТранзакцию();
			ШаблонСообщения = НСтр("ru = 'Не удалось установить производственный календарь для организации %1
			|%2'");
