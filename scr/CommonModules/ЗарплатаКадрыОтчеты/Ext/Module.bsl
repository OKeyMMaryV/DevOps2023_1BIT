...
Функция СохраненныйМакетКомпоновкиДанныхОтчета(Отчет, КлючВарианта)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.ВерсияКонфигурации = бит_ОбщегоНазначения.МетаданныеВерсия() Тогда
...
Процедура ЗапомнитьМакетКомпоновкиДанных(Отчет, КлючВарианта, МакетКомпоновкиДанных) Экспорт
	...
	Попытка
		...
		Запись.ВерсияКонфигурации = бит_ОбщегоНазначения.МетаданныеВерсия();
...
Процедура ПриКомпоновкеРезультатаРасчетныйЛистокПоФизическимЛицам(ОбъектОтчета, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка, НаАванс = Ложь)
	...
	Попытка
		...
		Иначе
			...
			ЕстьГруппировкаПоОрганизации = МожноГруппировать(НастройкиОтчета.Порядок, "ОрганизацияСортировки");
		...
		Для каждого СтрокаМесяца Из РезультатКомпоновки.ДанныеОтчета.Строки Цикл
			...
			Для Каждого СтрокаГоловногоСотрудника Из СтрокаМесяца.Строки Цикл
				...
				Если ТекущееФизическоеЛицо <> СтрокаГоловногоСотрудника.ФизическоеЛицо
					Или Не УправленческийРасчетныйЛисток И ТекущаяОрганизация <> СтрокаГоловногоСотрудника.ОрганизацияСортировки Тогда
					...
					Если Не УправленческийРасчетныйЛисток
						И СтрокаГоловногоСотрудника.Владелец().Колонки.Найти("ОрганизацияСортировки") <> Неопределено Тогда
						
						ДанныеПоФизическомуЛицу.Вставить("Организация", СтрокаГоловногоСотрудника.ОрганизацияСортировки);
						ТекущаяОрганизация = СтрокаГоловногоСотрудника.ОрганизацияСортировки;
			...
			Для каждого СтрокаКоллекции Из КоллекцияДанных Цикл
				...
				ВывестиСотрудникаРасчетныйЛисток(ВыводимаяКоллекция, ДокументРезультат, ДокументРезультатСтраница, РезультатКомпоновки.МакетПечатнойФормы,
					...
					ВыводимыеГруппировки, НастройкиВывода, НаАванс, СвойстваРегистраторов, ДанныеРасчетныхЛистковСтруктура);
...
Процедура ВывестиСотрудникаРасчетныйЛисток(СтрокиГоловныхСотрудников, ДокументРезультатПромежуточный, ДокументРезультатСтраница, Макет, СоответствиеПользовательскихПолей, ДанныеПоНДФЛИВзносам, ГруппироватьПоИсточникамФинансирования, ВыводимыеГруппировки, НастройкиВывода, НаАванс, СвойстваРегистраторов, ДанныеРасчетныхЛистковСтруктура)
	...
	Если ГруппироватьПоИсточникамФинансирования Тогда
		ИтогиПоГруппам = ВывестиГруппировкиПоСтатьямФинансирования(ТелоРасчетногоЛистка, Макет, НастройкиВывода, СтрокиГоловныхСотрудников, СоответствиеПользовательскихПолей, ДанныеПечатнойФормы, СвойстваРегистраторов, ДанныеРасчетныхЛистков);
	Иначе
		ИтогиПоГруппам = ВывестиТелоРасчетногоЛистка(ТелоРасчетногоЛистка, Макет, НастройкиВывода, СтрокиГоловныхСотрудников, СоответствиеПользовательскихПолей, ДанныеПечатнойФормы, СвойстваРегистраторов, ДанныеРасчетныхЛистков);
	КонецЕсли;
	...
	ДанныеРасчетныхЛистковСтруктура.ОбщиеДанныеРасчетныхЛистков.Вставить(СтрокаШапки.ФизическоеЛицо, ОбщиеДанныеРасчетногоЛиста);
...
Функция ВывестиГруппировкиПоСтатьямФинансирования(ДокументРезультат, Макет, НастройкиВывода, СтрокиГоловныхСотрудников, СоответствиеПользовательскихПолей, ДанныеПечатнойФормы, СвойстваРегистраторов, ДанныеРасчетныхЛистков)
	...
	Для каждого СтатьяФинансирования Из СтатьиФинансирования Цикл
		...
		ИтогиПоСтатье = ВывестиТелоРасчетногоЛистка(ДокументРезультат, Макет, НастройкиВывода, СтрокиПоСтатьям, СоответствиеПользовательскихПолей, ДанныеПечатнойФормы, СвойстваРегистраторов, ДанныеРасчетныхЛистков);
...
Функция ВывестиТелоРасчетногоЛистка(ДокументРезультат, Макет, НастройкиВывода, СтрокиГоловныхСотрудников, СоответствиеПользовательскихПолей, ДанныеПечатнойФормы, СвойстваРегистраторов, ДанныеРасчетныхЛистков)
		...
		Для каждого СтрокаСотрудника Из СтрокаГоловногоСотрудника.Строки Цикл
			...
			Если СтрокаГруппы <> Неопределено Тогда
				...
				Для каждого СтрокаУдержаний Из СтрокаГруппы.Строки Цикл
					...
					ЗаполнитьЗначенияСвойств(ДанныеРасчетныхЛистков.Добавить(), СтрокаКоллекции);
				...
				КоллекцииРазделов.Удержано.ЗаполнитьЗначения(Неопределено, "Сотрудник");
				КоллекцииРазделов.Удержано.Свернуть(СворачиваемыеКолонки, "Сумма");
				...
				Если СтрокаГруппы <> Неопределено Тогда
					...
					Для каждого СтрокаЛьготы Из СтрокаГруппы.Строки Цикл
						...
						Если СтрокаКоллекции = Неопределено Тогда
							...
							СтрокаКоллекции = Новый Структура("ФизическоеЛицо,Группа,Сотрудник,ВидРасчета,ПриоритетВидаРасчета,Сумма");
		...
		ИначеЕсли ИндексРабочегоМеста < КоллекцииРазделов.Начислено.Количество() Тогда
			...
			Иначе
				...
				Если ВывестиЗаголовокРабочегоМеста Тогда
					...
					ПараметрыЗаполнения.ПредставлениеРабочегоМеста = ПредставлениеРабочегоМеста(РабочееМесто.СтрокаСотрудника);
				...
				Иначе
					...
					Если ИндексСтрокиНачислено < РабочееМесто.СтрокиНачислений.Количество() Тогда
						...
						СтрокаДанныхРасчетногоЛистка = ДанныеРасчетныхЛистков.Добавить();
						СтрокаДанныхРасчетногоЛистка.ПредставлениеРабочегоМеста = ПредставлениеРабочегоМеста(РабочееМесто.СтрокаСотрудника);
						ЗаполнитьЗначенияСвойств(СтрокаДанныхРасчетногоЛистка, СтрокаНачислений);
...
Процедура ВывестиТелоСправочнойИнформации(ДокументРезультат, Макет, КоллекцияСправочныхДанных, ДанныеРасчетныхЛистков)
