...
Функция ПериодичностьУплатыФиксированныхСтраховыхВзносов(Организация, Период) Экспорт
	...
	Если Не ЭтоИПНулевка(Организация)
		И (ПрименяетсяУСН ИЛИ (ПлательщикЕНВД И ФиксированныеВзносыУменьшаютЕНВД)) Тогда
...
Функция СтраховыеВзносыСДоходовКУплате(Организация, Период, СтруктураДоходов, СообщатьОбОшибках = Истина) Экспорт
	...
	Если Период >= ДатаНачалаУчетаРасходовПриРасчетеСтраховыхВзносовИП()
		И Год(Период) = Год(ОбщегоНазначения.ТекущаяДатаПользователя())
		И УчетнаяПолитика.ПлательщикНДФЛ(Организация, Период)  Тогда
	...
	КонецЕсли;
	СтраховыеВзносы.СуммаВзносаПФРсДоходов = Макс(СтраховыеВзносы.СуммаВзносаПФРсДоходовВсего - СтраховыеВзносы.СуммаВзносаПФРсДоходовУплачено, 0);
...
Функция ДанныеДляРасчетаСтраховыхВзносовСДоходов(Период, Организация = Неопределено) Экспорт
	...
	ДанныеДляРасчета.Вставить("ВзносыВСумме", Ложь);
	
	ФиксированныеСтраховыеВзносы = РассчитатьФиксированныеСтраховыеВзносы(КонецГода(Период), Организация);
	
	ПредельнаяСуммаВзноса = 8 * РазмерыСтраховыхВзносов(Период).ПФР;
	...
	Если Период >= ДатаНачалаУчетаРасходовПриРасчетеСтраховыхВзносовИП()
		И УчетнаяПолитика.ПлательщикНДФЛ(Организация, Период) Тогда
		ДанныеДляРасчета.ВзносыВСумме = Истина;
	КонецЕсли;
...
Функция РассчитатьФиксированныеСтраховыеВзносы(Знач Период, Организация = Неопределено) Экспорт
	...
	РазмерыСтраховыхВзносов = РазмерыСтраховыхВзносов(Период);
		...
		Если ЗначениеЗаполнено(Организация) Тогда
			...
			Если ЗначениеЗаполнено(ДатаРегистрации) И НачалоМесяца < ДатаРегистрации И ДатаРегистрации <= НачалоДня(КонецМесяца) Тогда
				...
				Если Не УчитыватьДеньРегистрацииПриРасчетеСтраховыхВзносовИП(Период) Тогда
					...
					ПервыйДеньМесяца = ДатаРегистрации + 1;
				КонецЕсли;
			Иначе
				ПервыйДеньМесяца = НачалоМесяца;
			КонецЕсли;
			
			ПлательщикФиксированныхВзносов = УчетнаяПолитика.ПлательщикФиксированныхВзносов(Организация, КонецМесяца, ДатаИзменения);
			Если ПервыйДеньМесяца < ДатаИзменения И ДатаИзменения <= НачалоДня(КонецМесяца) Тогда
				// В течении месяца поменялась настройка системы налогообложения.
				// Сравниваем настройку на конец месяца с настройкой на день, предшествующий изменению.
				ПлательщикФиксированныхВзносовВПрошлом = УчетнаяПолитика.ПлательщикФиксированныхВзносов(Организация, ДатаИзменения - 1);
				Если Не ПлательщикФиксированныхВзносовВПрошлом И ПлательщикФиксированныхВзносов Тогда
					// Плательщик взносов с даты изменения включительно до конца месяца
					ДнейУплатыВзносов = ДнейВМесяце - День(ДатаИзменения) + 1;
				ИначеЕсли ПлательщикФиксированныхВзносовВПрошлом И Не ПлательщикФиксированныхВзносов Тогда
					// Плательщик взносов с начала месяца до даты изменения
					ДнейУплатыВзносов = День(ДатаИзменения) - День(ПервыйДеньМесяца);
				ИначеЕсли ПлательщикФиксированныхВзносов И ПлательщикФиксированныхВзносовВПрошлом Тогда
					// Плательщик взносов как на старой СНО, так и на новой
					ДнейУплатыВзносов = ДнейВМесяце;
				Иначе
					ДнейУплатыВзносов = 0;
				КонецЕсли;
			Иначе
				ДнейУплатыВзносов = ?(ПлательщикФиксированныхВзносов, ДнейВМесяце - День(ПервыйДеньМесяца) + 1, 0);
			КонецЕсли;
...
Функция ГраницыПериодаУплатыСтраховыхВзносов(СтраховойПериод, ВидПлатежаВГосБюджет)
...
Функция РазмерыСтраховыхВзносов(СтраховойПериод)
	
	СтраховыеВзносы = Новый Структура("ПФР, ФФОМС, ФСС", 0, 0, 0);
...
Функция ПериодичностьНачисления(Организация, Период) Экспорт
	...
	Если Период >= ДатаАвтоНачисленияСтраховыхВзносовИП() Тогда
		...
		Если УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходыЗаПериод(Организация, НачалоКвартала(Период), КонецКвартала(Период)) Тогда
			// Выполняем чаще для признания расходов по УСН и уменьшения авансового платежа
			Периодичность = Перечисления.Периодичность.Квартал;
		ИначеЕсли УчетнаяПолитика.ПлательщикНДФЛЗаПериод(Организация, НачалоКвартала(Период), КонецКвартала(Период))
...
Функция РассчитатьСтраховыеВзносыСДоходов(Организация, Период)
	...
	Если Период >= УчетСтраховыхВзносовИП.ДатаНачалаУчетаРасходовПриРасчетеСтраховыхВзносовИП()
		И УчетнаяПолитика.ПлательщикНДФЛ(Организация, Период) Тогда
		// Используется допущение - все уплаченные страховые взносы, при закрытии года будут учтены в составе расходов.
		СтраховыеВзносыУплаченныеЗаПериод = СтраховыеВзносыУплаченныеЗаПериод(Организация, НачалоГода(Период), Период);
		СтруктураДоходов.СуммаДоходаИП = СтруктураДоходов.СуммаДоходаИП - СтраховыеВзносыУплаченныеЗаПериод.Итог("Сумма");
	КонецЕсли;
...
Функция ДатаНачалаУчетаРасходовПриРасчетеСтраховыхВзносовИП() Экспорт
	
	// Постановление Конституционного Суда Российской Федерации от 30.11.2016 № 27-П
	Возврат Дата(2016, 1, 1);
