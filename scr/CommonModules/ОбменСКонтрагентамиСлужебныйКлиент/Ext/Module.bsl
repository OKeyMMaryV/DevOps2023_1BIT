...
Процедура ОткрытьЭДДляПросмотра(СсылкаДляПросмотра, ПараметрыФормы = Неопределено, ВладелецФормы = Неопределено) Экспорт
	...
	Если ТипСсылкиДляПросмотра = Тип("ДокументСсылка.ЭлектронныйДокументИсходящий") Тогда
		...
		Если ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоДокументДополнительногоСервисаЭДО(СсылкаДляПросмотра) Тогда
			ИмяФормы = "Обработка.ОбменСКонтрагентами.Форма.ок_ФормаПросмотраИсходящегоДокумента";
		КонецЕсли;	
		//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103		
	...
	ИначеЕсли ТипСсылкиДляПросмотра = Тип("ДокументСсылка.ЭлектронныйДокументВходящий") Тогда
		...
		//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103
		Если ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоДокументДополнительногоСервисаЭДО(СсылкаДляПросмотра) Тогда
			ИмяФормы = "Обработка.ОбменСКонтрагентами.Форма.ок_ФормаПросмотраВходящегоДокумента";
		//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-06-17 (#3786)
		ИначеЕсли ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоБумажныйДокумент(СсылкаДляПросмотра) Тогда
			ИмяФормы = "Обработка.ОбменСКонтрагентами.Форма.ок_ФормаПросмотраВходящегоДокумента";
...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйКлиентПовтИсп.ИспользоватьОповещенияОтСервисаЭДО() Тогда
...
Процедура ОбработатьПакетыЭДЦиклНачало(Контекст)
	...
	//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-07-10 (#3796)
	//падает по превышению глубины стека на больших пакетах
	//отключим расшифровку файлов, она не используется пока
	ОбработатьПакетыЭДПослеЦикла(Контекст);
	Возврат;
	//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Конец 2020-07-10 (#3796)
...
Процедура ОбработатьСозданиеЗаявкиПриПолученииНовогоИдентификатораТакском(Результат, ДополнительныеПараметры) Экспорт
	...
	ОтветДействие.Вставить("Действие"     , "ОтправитьЗаявку");
