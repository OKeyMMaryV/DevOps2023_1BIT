...
Процедура УправлениеФормой(Форма) Экспорт
	...
	Элементы.ВидОперации.Доступность = Не (ЭтоФормаДокументаИнтеграцияСБанком И ЭтоУплатаНалога И Не ЭтоНовый);
...
Функция ВидимостьСтатьиДДСПоВидуОперации(ВидОперации)
	...
	Результат = ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога")
		...
		Или ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеСотрудникуПоДоговоруПодряда")
...
Функция ТекущиеДанныеРасшифровкаПлатежа(Форма, ИдСтроки = -1) Экспорт
	...
	Если ФормаДокументаОднострочная(Форма) Тогда
		...
		Результат.Вставить("бит_КлючСтроки", Форма.РасшифровкаПлатежабит_КлючСтроки);
...
Процедура ЗаполнитьЗначенияТекущиеДанныеРасшифровкаПлатежа(Форма, НовыеДанныеСтроки, ИдСтроки = -1, ЗаполнятьСуммуДокумента = Истина) Экспорт
	...
	Если ФормаДокументаОднострочная(Форма) Тогда
		...
		// бит_Финанс изменения кода. Начало.
		Форма.РасшифровкаПлатежабит_КлючСтроки = НовыеДанныеСтроки.бит_КлючСтроки;
		// бит_Финанс изменения кода. Конец. 
	Иначе
...
Процедура ОбновитьИтоги(Форма) Экспорт
	...
	// Установка текста в подвал колонки приводит к тому, что при интерактивном копировании строки "расшифровки"
	// таблица формы не переходит в режим редактирования.
	// Нам нужено событие "ПриНачалеРедактирования" для заполнения "бит_КлюСтроки" в новой строке.
	// Реквизиты формы "бит_ИтогоСуммаПлатежа", "бит_ИтогоВсегоНДС" добавляются в 
	// бит_Казначейство.ПодготовитьФормуНаСервере().
	Форма.бит_ИтогоСуммаПлатежа = Формат(ИтогоСуммаПлатежа, "ЧЦ=15; ЧДЦ=2");
	Форма.бит_ИтогоВсегоНДС = Формат(ИтогоВсегоНДС,     "ЧЦ=15; ЧДЦ=2");
	// бит_Финанс изменения кода. Конец. 
