...
//
// Для подключения документа-основания документа ИС к этому механизму нужно в модуле формы документа-основания добавить вызовы
...
// Возвращает массив/список значений пустых значений составного типа, включая Неопредело
// 
// Параметры:
...
// 
// Параметры:
//  ВидыПродукции - Массив Из ПеречислениеСсылка.ВидыПродукцииИС - виды продукции, которые поддерживает документ.
// Возвращаемое значение:
//  Структура - 
Функция ПараметрыЗаполненияТабличнойЧастиТовары(ВидыПродукции) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП") Тогда
		...
		Если ВидыПродукции.Найти(Перечисления.ВидыПродукцииИС.Обувная) <> Неопределено
			Или ВидыПродукции.Найти(Перечисления.ВидыПродукцииИС.Табачная) <> Неопределено Тогда
...
Функция ИспользуетсяМаркируемаяПродукция() Экспорт
	...
	Возврат ПолучитьФункциональнуюОпцию("ВестиСведенияДляДекларацийПоАлкогольнойПродукции")
		Или ПолучитьФункциональнуюОпцию("ВестиУчетОбувнойПродукцииИСМП")
		Или ПолучитьФункциональнуюОпцию("ВестиУчетТабачнойПродукцииМОТП");
...
Функция СвойстваМаркируемойПродукции(Номенклатура, Характеристика) Экспорт
	...
	Иначе
		Для Каждого Колонка Из ДанныеТаблица.Колонки Цикл
			СвойстваМаркируемойПродукции = Новый Структура;
			СвойстваМаркируемойПродукции.Вставить(Колонка.Имя);
		КонецЦикла;
	КонецЕсли;
...
Функция ДатаИзСтроки(Значение, Делитель = 1000) Экспорт
	
	Если СтрНайти(Значение, "T") > 0 Тогда
		Возврат ДатаИзСтрокиISO(Значение);
	Иначе
		Возврат ДатаИзСтрокиUNIX(Значение, Делитель);
	КонецЕсли;
...
//
// Параметры:
//  НоменклатураУпаковки	 - Массив - ссылки на элементы справочника номенклатуры
//
// ВозвращаемоеЗначение:
//  Строка - строка, состоящая из различных наименований переданной номенклатуры
//
Функция ПредставлениеСоставаУпаковки(НоменклатураУпаковки) Экспорт
	
	НаименованияНоменклатурыУпаковки = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(НоменклатураУпаковки, "Наименование");
	ДеревоЧастейНаименований = Новый ДеревоЗначений();
	
	Для Каждого НаименованиеНоменклатуры Из НаименованияНоменклатурыУпаковки Цикл
		ЧастиНаименования = СтрРазделить(НаименованиеНоменклатуры.Значение, " ");
...
Процедура СоставитьСтрокуСоставаУпаковки(СтрокаСостава, СтрокиТекущейЧасти, Знач ОграничительСостава, Уровень = 1)
	...
	Для Каждого СтрокаЧастиНаименования Из СтрокиТекущейЧасти Цикл
		...
		СтрокаСостава = СтрокаСостава + СтрокаЧастиНаименования["ЧастьНаименования" + Уровень];
