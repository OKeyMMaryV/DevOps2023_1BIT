...
//
Процедура НастроитьПоказателиРазниц(Отчет, ЭлементыПоказателей) Экспорт
	
	ОтображатьПользователю = БухгалтерскийУчетВызовСервераПовтИсп.ПользователюДоступныСуммыПостоянныхВременныхРазниц();
	
	Если Не ОтображатьПользователю Тогда
		Использовать = Ложь;
	Иначе
		Использовать = БухгалтерскиеОтчетыВызовСервера.ИспользоватьСуммыРазниц(
			Отчет.Организация,
			Отчет.НачалоПериода,
			Отчет.КонецПериода);
	КонецЕсли;
...
// При формировании отчета отключает использование показателей с разницами по ПБУ 18, если они не применяются в контексте отчета.
// См. комментарий к НастроитьПоказателиРазниц.
//
// Параметры:
...
//
Процедура ИсправитьПоказателиРазниц(ПараметрыОтчета) Экспорт
...
Функция ОтчетПоддерживаетУниверсальныеНастройки(ИмяОтчета) Экспорт
	...
	бит_БухгалтерскиеОтчетыКлиентСервер.ОтчетПоддерживаетУниверсальныеНастройки(СписокОтчетов);
