...
Процедура ПодстановкаИзвестныхСкалярныхАргументов(Контекст, Условие, РазрядностьОкругления)
	...
	Для Каждого Арг Из ВсеАргументы Цикл
		...
		Если Контекст.ОТЧ.ПривестиЗначение(СекцияАрг) = 0
			Или Контекст.ОТЧ.ПривестиЗначение(СтрокаАрг) = 0
			Или Контекст.ОТЧ.ПривестиЗначение(КолонкаАрг) = 0 Тогда 
			
			Продолжить;
		КонецЕсли;
		
		Отбор = Новый Структура("row, section", Прав("0000" + СтрокаАрг, 4), Прав("000" + СекцияАрг, 3));
		...
		СтрокаСДанными = Контекст[Таблица].НайтиСтроки(Отбор);
...
Функция ВычислитьУсловиеПоСпецификам(Условие, Контекст, РазрядностьОкругления = 2, Специфики = Неопределено)
	...
	Для Каждого Стр Из Специфики Цикл
		...
		Если Не ВычислитьСкалярноеУсловие(ПреобразованноеУсловие, Контекст, РазрядностьОкругления) Тогда 
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
	Возврат Истина;
...
Функция ПроверитьКонтрольноеСоотношение(СтрокаТаблицы, Контекст) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.condition) Тогда
		...
		ИначеЕсли ЭтоВариацияПоСпецификам(Контекст, СтрокаТаблицы.condition, СтрокаТаблицы.rule, Строки, Специфики) Тогда
			Возврат ВычислитьУсловиеПоСпецификам(СтрокаТаблицы.rule, Контекст, СтрокаТаблицы.precision, Специфики);
