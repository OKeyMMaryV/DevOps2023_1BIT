...
// декларации по НДС. Предназначена для редакций декларации по НДС, соответствующих формам "ФормаОтчета2017Кв1"
// и "ФормаОтчета2019Кв1" отчета "РегламентированныйОтчетНДС".
// Параметры:
...
Процедура ПроверитьЗаполнение_Раздел1(КонтекстВыполнения)
	...
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-09-14 (#4354)
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2021Кв3")
	...
	КонецЕсли;
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-09-14 (#4354)
...
Процедура ПроверитьЗаполнение_Раздел2(КонтекстВыполнения)
	...
	//ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2020-12-23 (#3946)
	//Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2019Кв1")
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела Цикл
		...
		//ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2020-12-23 (#3946)
		//Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2019Кв1") Тогда
		Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2020Кв4")
			// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-09-14 (#4354)
			ИЛИ ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2021Кв3")
			// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-09-14 (#4354)
...
Процедура ПроверитьЗаполнение_Раздел8(КонтекстВыполнения, ДанныеРаздела)
	...
	//ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2020-12-23 (#3946)
	//КодыПоНепродавцам      = СтрРазделить(КодыОперацийПоНепродавцам(), ",", Ложь);
...
Процедура ПроверитьЗаполнениеСтрокиДанных_Раздел8(СтрокаДанных, НомерСтроки, КонтекстВыполнения)
	...
	//КонецЕсли;
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2021Кв3") Тогда
...
Функция КодыОперацийПоНепродавцам(КонтекстВыполнения) //ОКЕЙ Сафронов А.А. (СофтЛаб) Начало 2020-12-23 (#3946) КонтекстВыполнения
