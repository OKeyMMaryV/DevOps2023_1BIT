...
Функция ПолучитьМассивВидовДоговоров(СПоставщиком = Ложь, СПокупателем = Ложь, 
	...
	СФакторинговойКомпанией = Ложь) Экспорт
...
Функция ПолучитьСведенияОСпискеНоменклатуры(Знач СписокНоменклатуры, ПараметрыОбъекта, ОпределятьСчета = Истина, ОпределятьЦены = Истина) Экспорт
	...
	Если Реализация И 
		...
		((ПрименяетсяЕНВД ИЛИ ДеятельностьНаПатенте) ИЛИ
	...
	ИменаРеквизитов =
		...
		|ЕдиницаИзмерения, ВидСтавкиНДС, КодТНВЭД,
		...
		|ПродукцияМаркируемаяДляГИСМ, ТабачнаяПродукция, ПериодичностьУслуги";
	...
	Для Каждого Номенклатура Из СписокНоменклатуры Цикл
		...
		ДатаОпределенияСтавкиНДС = ?(ЗначениеЗаполнено(Дата), Дата, ОбщегоНазначения.ТекущаяДатаПользователя());

		СведенияОНоменклатуре.Вставить("СтавкаНДС", 
			Перечисления.СтавкиНДС.СтавкаНДС(СведенияОНоменклатуре.ВидСтавкиНДС, ДатаОпределенияСтавкиНДС));
		...
		Если СчетаУчета <> Неопределено Тогда
			...
			ДоходЕНВД = БухгалтерскийУчетПовтИсп.СчетОтноситсяКДеятельностиЕНВД(СчетаУчета.СчетДоходов);
		...
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-15 (#4214)
		Если НЕ ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров")
			ИЛИ СведенияОНоменклатуре.ПрослеживаемыйТовар = Неопределено Тогда
			СведенияОНоменклатуре.Вставить("ПрослеживаемыйТовар", Ложь);
		КонецЕсли;
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-15 (#4214)
...
Функция ПолучитьЗначениеПоУмолчанию(Настройка, ИмяПользователяИБ = Неопределено) Экспорт
	...
	//1c-izhtc spawn (	
	ИначеЕсли НастройкаВРег = ВРег("ОсновнойОтветственный") Тогда	
