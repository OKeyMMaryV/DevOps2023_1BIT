...
#Область ПолучениеСпискаОСиВерсий
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		ДобавитьПараметрKeyValue(ДополнительныеПараметры, "configVersion", бит_ОбщегоНазначения.МетаданныеВерсия());
...
Процедура ВсеОбновленияОблачногоАрхива() Экспорт
	...
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Метаданные_Версия%", бит_ОбщегоНазначения.МетаданныеВерсия());
