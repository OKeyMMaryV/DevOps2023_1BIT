...
Процедура ПриПолученииПредопределенныхНаборовСвойств(Наборы) Экспорт
	...
	УправлениеСвойствамиБЗК.ЗарегистрироватьНаборСвойств(Наборы, "b080f311-2352-47c8-8abf-a4affae1a6de", Метаданные.Документы.ЗаявлениеОПредоставленииСведенийОТрудовойДеятельности);
...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Списки.Вставить(Метаданные.Документы.ЗаявлениеОПредоставленииСведенийОТрудовойДеятельности, Истина);
	Списки.Вставить(Метаданные.Справочники.ЗаявлениеОПредоставленииСведенийОТрудовойДеятельностиПрисоединенныеФайлы, Истина);
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Если ДанныеОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
			...
			ИначеЕсли Не ЗначениеЗаполнено(ДанныеОрганизации.КПП) Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'У организации не заполнен код по КПП'"), Организация, "КПП", ,Отказ);
			ИначеЕсли Не РегламентированныеДанныеКлиентСервер.КППСоответствуетТребованиям(ДанныеОрганизации.КПП, "") Или СтрЗаменить(ДанныеОрганизации.КПП, "0", "") = "" Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'У организации неверно указан код по КПП'"), Организация, "КПП", ,Отказ);
			КонецЕсли;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеОрганизации.ИНН) Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'У организации не заполнен ИНН'"), Организация, "ИНН", ,Отказ);
		ИначеЕсли Не РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(ДанныеОрганизации.ИНН, ЗарплатаКадры.ЭтоЮридическоеЛицо(Организация), "") Или СтрЗаменить(ДанныеОрганизации.ИНН, "0", "") = "" Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'У организации неверно указан ИНН'"), Организация, "ИНН", ,Отказ);
		КонецЕсли;
	КонецЕсли;	
...
Функция ПолучитьДатыПериодаДокумента(ДокументСсылка) Экспорт
	...
	Если ТипДокумента = Тип("ДокументСсылка.ВедомостьУплатыАДВ_11")
		...
		ИначеЕсли ТипДокумента = Тип("ДокументСсылка.СведенияОЗастрахованныхЛицахСЗВ_М")
			Или ТипДокумента = Тип("ДокументСсылка.СведенияОТрудовойДеятельностиРаботниковСЗВ_ТД") Тогда
...
Процедура ПроверитьДанныеСотрудников(Сотрудники, НачалоПериода, ОкончаниеПериода, ЗаписиОСтаже, Отказ) Экспорт 
...
Процедура ПроверитьДанныеСтрокиСотрудника(ДанныеСотрудника, УникальныеИНН, УникальныеСтраховыеНомера, Отказ, ИмяПоляИНН = "НомерСтроки") Экспорт 
	...
	Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл 
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ПредыдущаяЗапись <> Неопределено И КонецДня(ПредыдущаяЗапись.ДатаОкончанияПериода) + 1 = Выборка.ДатаНачалаПериода
				И ДанныеСтрокСовпадают(ПредыдущаяЗапись, Выборка, РеквизитыСтроки) Тогда 
