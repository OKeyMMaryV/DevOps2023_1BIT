...
Процедура НапечататьЧек(Форма, ОписаниеОповещения) Экспорт
	ОчиститьСообщения();
	...
	ИначеЕсли ОбщегоНазначенияБПКлиент.ПроверитьВозможностьПечатиЧека(Форма) Тогда
...
Процедура НапечататьЧеки(Форма, ОписаниеОповещения) Экспорт
	...
	Если ПечатьФискальныхДокументовВызовСервера.ЧекиПробитыНаФискальномУстройстве(Объект.Ссылка, ПараметрыОбработчика) <> 0 Тогда
		...
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, Результат);
	ИначеЕсли ОбщегоНазначенияБПКлиент.ПроверитьВозможностьПечатиЧеков(Форма, ПараметрыОбработчика) Тогда
...
Процедура ВыборФискальногоУстройстваЗавершение(ИдентификаторУстройства, ПараметрыПечатиЧека) Экспорт
	...
	Результат = ПечатьФискальныхДокументовВызовСервера.ПроверитьНеобходимостьАктуализацииРасчетов(Объект.Ссылка, Форма.УникальныйИдентификатор);
...
Процедура ПолучитьПараметрыЗавершение(ПараметрыФискальногоУстройства, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Если НЕ ПечатьФискальныхДокументовВызовСервера.ОрганизацияККТСоотвествуетДокументу(Форма.Объект.Организация, ПараметрыФискальногоУстройства.ВыходныеПараметры.ОрганизацияИНН) Тогда
			Результат = НовыйРезультатПечатиЧека();
...
Процедура ПроверкаГотовностиККТЗавершение(РезультатПроверкиГотовности, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ДополнительныеПараметры.Вставить("ИдентификаторУстройстваЭТ", ПечатьФискальныхДокументовВызовСервера.ИдентификаторУстройстваЭТ(Объект.Ссылка));
...
Процедура ОперацияПоПлатежнойКартеЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если ДополнительныеПараметры.ИдентификаторУстройства <> Неопределено Тогда
			...
			ОткрытьФорму("Обработка.ПечатьЧека.Форма", ПараметрыФормы, ДополнительныеПараметры.Форма,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
