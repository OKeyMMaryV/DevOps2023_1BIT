...
Процедура СформироватьУдержанныйНалогПоВременнойТаблице(Движения, Отказ, Организация, ДатаОперации, УдержанныйНалог, РегистрацияВНалоговомОргане = Неопределено, Записывать = Ложь, ФормироватьРасчетыНалоговыхАгентовСБюджетом = Истина, Регистратор = Неопределено, ТаблицаМежрасчетныхВыплат = Неопределено) Экспорт
	...
	ПроставитьКрайнийСрокУплаты(Движения.РасчетыНалогоплательщиковСБюджетомПоНДФЛ);
...
Процедура СформироватьНДФЛКПеречислению(Движения, Отказ, КоллекцияДвижений = Неопределено) Экспорт
	...
	Если ЕстьНовыеСтроки Тогда
		ПроставитьКрайнийСрокУплаты(Движения.НДФЛКПеречислению);
...
//
// Параметры:
//	КоллекцияСтрокДвижений - набор записей регистра РасчетыНалогоплательщиковСБюджетомПоНДФЛ, либо иная
//                           коллекция строк регистра, либо таблица значений с соответствующими колонками
//
Процедура ПроставитьКрайнийСрокУплаты(КоллекцияСтрокДвижений) Экспорт
	...
	РабочиеДни = КалендарныеГрафики.ДатыБлижайшихРабочихДней(КалендарныеГрафики.ОсновнойПроизводственныйКалендарь(), ДатыУдержанияНалога, Ложь, Ложь, Истина);
	...
	Для каждого СтрокаТЗ Из КопияТаблицы Цикл
		Если ЕстьДанныеВСтроке(СтрокаТЗ, ИменаКолонокСуммирования) Тогда
			...
			Если СтрокиДанных.Количество() > 1 Тогда
				...
				КонецЦикла; 
				ОтражениеЗарплатыВБухучете.СвернутьРезультатыРаспределения(НоваяСтрока.РаспределениеПоСтатьям);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция РаспределитьПеречисленныйНалогПоФизическимЛицам(Ссылка, Организация, ДатаОперации, Сумма, МесяцНалоговогоПериода, РегистрацияВНалоговомОргане, ФизическиеЛица)
	...
	РабочиеДни = КалендарныеГрафики.ДатыБлижайшихРабочихДней(КалендарныеГрафики.ОсновнойПроизводственныйКалендарь(), ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДатаБлижайшегоПлатежа), Ложь, Ложь, Истина);
	Если РабочиеДни <> Неопределено Тогда 
