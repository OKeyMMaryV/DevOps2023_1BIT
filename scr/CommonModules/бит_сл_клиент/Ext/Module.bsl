...
// Функция - Получить форму клиента лицензирования
// Получает подходящую форму клиента лицензирования.
// Возвращаемое значение:
// ФормаКлиентаЛицензирования  - управляемая форма
//
Функция ПолучитьФормуКлиентаЛицензирования() Экспорт
	...
	Параметры = Новый Структура("ФормаОткрытаКодом", Истина);
...
Процедура ПриОткрытииФормыВыбораВерсииПродукта() Экспорт
	...
	Если бит_ОбщегоНазначения.ЕстьДоступНаИзменениеВерсииПродукта() Тогда
		
		ОткрытьФорму("ОбщаяФорма.бит_КонстантыУправляемая");
		
	Иначе
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Недостаточно прав для изменения версии продукта, обратитесь к администратору базы.");
