...
Процедура ПолучитьСертификатПоОтпечаткуПослеПоиска(Сертификат, Контекст) Экспорт
	...
	ПолучитьСертификатПоОтпечаткуЗавершение(Сертификат, Неопределено, Контекст);
...
//
Процедура ПроверитьСертификат(Оповещение, Сертификат, МенеджерКриптографии = Неопределено, НаДату = Неопределено, ПоказатьОшибку = Истина)
...
Процедура ПроверитьСертификатПослеВыгрузкиСертификата(Сертификат, Контекст) Экспорт
	...
	Если ЭлектроннаяПодписьСлужебныйВызовСервера.ПроверитьСертификат(АдресСертификата,
			Контекст.ОписаниеОшибкиНаСервере, Неопределено) Тогда
...
Процедура ПроверитьСертификатНаКлиентеПослеОшибкиПроверки(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Результат = Новый Структура("ТекстОшибкиПроверкиСертификата", ОписаниеОшибки);
	ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПодключенияРасширения(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		НачатьПомещениеФайла(Новый ОписаниеОповещения(
			...
			ПоместитьВоВременноеХранилище(Неопределено));
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьОшибкуОбращенияКПрограммеПослеПодключенияРасширения(Подключено, Контекст) Экспорт
	...
	Форма = ОткрытьФорму("Справочник.ПрограммыЭлектроннойПодписиИШифрования.Форма.ОшибкаОбращенияКПрограмме",
		Контекст.ПараметрыФормы,,, ВариантОткрытияОкна.ОтдельноеОкно, , Контекст.ОбработкаПродолжения);
	...
	Если Контекст.ПроверитьМенеджерКриптографииНаКлиенте
	   И ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ПоказатьОшибкуОбращенияКПрограмме(
			...
			"", Контекст.Результат, Контекст.ФормаДобавления.МенеджерКриптографииНаСервереОписаниеОшибки);
	Иначе
		...
		Иначе // Путь к файлу
			Попытка
				НачатьПомещениеФайла(Новый ОписаниеОповещения(
					...
					Данные, Ложь, Контекст.Форма.УникальныйИдентификатор);
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекущийРезультат = Новый Структура("ОписаниеОшибки",
					КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
...
// Продолжение процедуры ПолучитьДанныеИзОписанияДанных.
Процедура ПолучитьДанныеИзОписанияДанныхЗавершение(Получен, Адрес, ВыбранноеИмяФайла, Контекст) Экспорт
