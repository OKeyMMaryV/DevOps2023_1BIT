...
Функция ДоговорыФизическихЛиц(ТаблицаФизическихЛиц) Экспорт
	...
	Для каждого СтрокаГоловнойОрганизации Из ТаблицаГоловныхОрганизаций Цикл
		...
		Запрос.Текст =
			...
			|	НАЧАЛОПЕРИОДА(Выразить(СотрудникиОрганизации.ПриказОПриемеДата КАК Дата), ДЕНЬ) КАК Дата,
...
Процедура СоздатьВТПоследниеРаботавшиеСотрудникиФизическихЛиц(МенеджерВременныхТаблиц, ТолькоРазрешенные, ФизическиеЛица, ПараметрыПолученияСотрудников, ИмяВТПоследниеСотрудники) Экспорт
...
Процедура ОпределитьПроверкиВеденияУчета(ГруппыПроверок, Проверки) Экспорт
	...
	ГруппаПроверокКонтрольВеденияКадровогоУчета.Наименование					= НСтр("ru='Проверки ведения кадрового учета'");
	...
	ГруппаПроверок.Наименование					= НСтр("ru='Проверки оформления кадровых приказов'");
	...
	Проверка.Наименование					= НСтр("ru='Проверка корректности видов занятости сотрудников'");
	...
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы			= ГруппаПроверок.Идентификатор;
	Проверка.Наименование					= НСтр("ru='Проверка корректной последовательности кадровых событий'");
	...
	ГруппаПроверок = ГруппыПроверок.Добавить();
	ГруппаПроверок.Наименование					= НСтр("ru='Проверки заполнения личных данных сотрудников'");
	ГруппаПроверок.ИдентификаторГруппы			= ГруппаПроверокКонтрольВеденияКадровогоУчета.Идентификатор;
	...
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы			= ГруппаПроверок.Идентификатор;
	Проверка.Наименование					= НСтр("ru='Проверка заполненности даты рождения, сотрудников учет которых начался с начала прошлого месяца'");
	...
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы			= ГруппаПроверок.Идентификатор;
	Проверка.Наименование					= НСтр("ru='Проверка заполненности ИНН, сотрудников учет которых начался с начала прошлого месяца'");
	...
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы			= ГруппаПроверок.Идентификатор;
	Проверка.Наименование					= НСтр("ru='Проверка заполненности СНИЛС, сотрудников учет которых начался с начала прошлого месяца'");
	...
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы			= ГруппаПроверок.Идентификатор;
	Проверка.Наименование					= НСтр("ru='Проверка того, что на дату начала учета заполнены сведения о гражданстве.'");
	...
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы			= ГруппаПроверок.Идентификатор;
	Проверка.Наименование					= НСтр("ru='Проверка того, что на дату начала учета установлен статус физического лица как налогоплательщика НДФЛ'");
	...
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы			= ГруппаПроверок.Идентификатор;
	Проверка.Наименование					= НСтр("ru='Проверка того, что на дату начала учета установлен статус застрахованного физического лица'");
...
Функция ЗапросВТСобытияТрудовыхКнижек(Организация, ТолькоРазрешенные) Экспорт
	...
	Запрос.Текст =
		...
		|	ПриемНаРаботу.ТрудоваяФункция КАК ТрудоваяФункция,
		|	ЗНАЧЕНИЕ(Перечисление.ВидыМероприятийТрудовойДеятельности.Прием) КАК ВидМероприятия,
		|	NULL КАК СтатьяТКРФ,
		|	ПриемНаРаботу.НаименованиеДокумента КАК НаименованиеДокумента
		...
		|	И КадровыйПеревод.ОбособленноеПодразделение = &Организация
		...
		|	Увольнение.СтатьяТКРФ,
		|	Увольнение.НаименованиеДокумента
		|ИЗ
		|	Документ.Увольнение КАК Увольнение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКадровыеДанныеСотрудников КАК КадровыеДанные
		...
		|	ЕСТЬNULL(СобытияТрудовыхКнижек.Подразделение, СобытияТрудовыхКнижекПередУвольнением.Подразделение) КАК Подразделение,
		|	ЕСТЬNULL(СобытияТрудовыхКнижек.Должность, СобытияТрудовыхКнижекПередУвольнением.Должность) КАК Должность,
		|	ЕСТЬNULL(СобытияТрудовыхКнижек.РазрядКатегория, СобытияТрудовыхКнижекПередУвольнением.РазрядКатегория) КАК РазрядКатегория,
		|	ЕСТЬNULL(СобытияТрудовыхКнижек.ТрудоваяФункция, СобытияТрудовыхКнижекПередУвольнением.ТрудоваяФункция) КАК ТрудоваяФункция,
		|	СобытияТрудовыхКнижек.ВидМероприятия КАК ВидМероприятия,
		|	СобытияТрудовыхКнижек.СтатьяТКРФ КАК СтатьяТКРФ,
		|	СобытияТрудовыхКнижек.НаименованиеДокумента КАК НаименованиеДокумента
