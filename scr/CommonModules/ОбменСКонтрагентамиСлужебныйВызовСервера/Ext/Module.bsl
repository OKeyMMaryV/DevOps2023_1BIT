...
Функция ДанныеПакетовЭД(МассивПакетовЭД, КонтекстОперации) Экспорт
	...
	ИмяРЗ = Метаданные.РегламентныеЗадания.ок_РаспаковкаПакетовЭД.Синоним;
	СтруктураДляРС = ок_ОбменСКонтрагентамиСлужебныйВызовСервера.СформироватьСтруктуруДляИнформацииПоРЗ(КонтекстОперации.ДатаНачалаОперации,
															ИмяРЗ,
															Ложь,
															Ложь,
	...
	СтруктураДляРС = ок_ОбменСКонтрагентамиСлужебныйВызовСервера.СформироватьСтруктуруДляИнформацииПоРЗ(КонтекстОперации.ДатаНачалаОперации,
															...
															Ложь,
															МассивПакетовЭД.Количество());
	ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ЗафиксироватьИнформациюПоРЗ(СтруктураДляРС, Ложь);
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-05-20 (#4175)
...
Функция ТекстСостоянияЭД(СсылкаНаВладельца, Гиперссылка) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаВладельца) Тогда
		...
		Иначе
			...
			Если (Не НастройкаСуществует ИЛИ ПриглашениеОтклонено) Тогда
				Гиперссылка = Истина;
...
Функция ПараметрыЕстьИдея() Экспорт
	...
	Конфигурация = СтрШаблон(ШаблонКонфигурации, Метаданные.Синоним, бит_ОбщегоНазначения.МетаданныеВерсия());
...
Функция ФормаПросмотраЭлектронногоДокумента(Знач СсылкаДляПросмотра) Экспорт
	...
	ИначеЕсли ТипЗнч(ЭлектронныйДокумент) = Тип("ДокументСсылка.ЭлектронныйДокументИсходящий") Тогда
		...
		//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-06-17 (#3786)
		Если ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоДокументДополнительногоСервисаЭДО(ЭлектронныйДокумент) Тогда
			ВыбраннаяФорма = "Обработка.ОбменСКонтрагентами.Форма.ок_ФормаПросмотраИсходящегоДокумента";
		КонецЕсли;	
		//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Конец 2020-06-17 (#3786)
	...
	Иначе
		...
		//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-06-17 (#3786)
		Если ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоДокументДополнительногоСервисаЭДО(ЭлектронныйДокумент) 
			ИЛИ ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоБумажныйДокумент(ЭлектронныйДокумент) Тогда
...
Процедура СоздатьСлужебногоПользователяОповещенийЭДО() Экспорт
	...
	СтрокаСоединенияИнформационнойБазы = ПараметрыКлиентаНаСервере.Получить("СтрокаСоединенияИнформационнойБазы");
	Если СтрНайти(ВРег(СтрокаСоединенияИнформационнойБазы), "WS=") <> 1 Тогда
		АдресДляУведомленийЭДО = Константы.АдресДляУведомленийЭДО.Получить();
		Если НЕ ЗначениеЗаполнено(АдресДляУведомленийЭДО) Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция СостояниеВерсииЭД(СсылкаНаВладельца)
	...
	ок_ЭлектронныйДокумент = Неопределено;
	Если ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоДокументДополнительногоСервисаЭДО(СсылкаНаВладельца, ок_ЭлектронныйДокумент) Тогда
		
		Если ЗначениеЗаполнено(ок_ЭлектронныйДокумент) Тогда
			Возврат ок_ОбменСКонтрагентамиВнутренний.СостояниеВерсииЭД(ок_ЭлектронныйДокумент);
		КонецЕсли;	
		
	//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2019-09-20 (#3414)
	ИначеЕсли ЗначениеЗаполнено(ок_ЭлектронныйДокумент) 
		И ТипЗнч(ок_ЭлектронныйДокумент) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий") 
		И ок_ОбменСКонтрагентамиВнутренний.ЭтоБумажныйДокумент(ок_ЭлектронныйДокумент) Тогда
			//Возврат СтрЗаменить(Строка(ок_ЭлектронныйДокумент), "Входящий", "Бумажный");
			Возврат ок_ОбменСКонтрагентамиВнутренний.СостояниеВерсииЭД(ок_ЭлектронныйДокумент);
