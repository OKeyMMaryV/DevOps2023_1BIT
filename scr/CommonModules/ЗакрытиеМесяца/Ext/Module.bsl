...
Функция ГруппаПоВидуОперации(ВидОперации) Экспорт
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РеформацияБаланса Тогда 
		НомерГруппы = 4;
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетНДФЛДляИП Тогда
		НомерГруппы = 4;
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-10-20 (#4407)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.ПереходНаФСБУ25 Тогда
		НомерГруппы = 5;
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-10-20 (#4407)
	Иначе
		НомерГруппы = 1;
	КонецЕсли;
...
Функция ПериодичностьРегламентнойОперации(ВидОперации) Экспорт
	...
	Если ВидОперации = Перечисления.ВидыРегламентныхОпераций.НачислениеНДС 
		...
		Или ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетНДФЛДляИП Тогда
...
//  ОтборОрганизаций		 - Массив - в детальный статус может потребоваться включить операции не только по организации, переданной в Организация
...
//  ВидыОпераций			 - Массив - виды операций, которые следует включить в детальный статус
...
Функция УсловиеВыполненияРегламентнойОперации(ВидОперации, Организация, Период, ОднаОрганизация, КешНастроек)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.НачислениеПроцентныхРасходов Тогда
		...
		СложноеУсловие.ПараметрыЗапросов.Вставить(
			"СчетаПроцентовПоОбязательствам", ПланыСчетов.Хозрасчетный.СчетаПроцентовПоОбязательствам());
		...
		|ИЗ
		|	(ВЫБРАТЬ
		|		ПроцентыПоОбязательствам.Организация КАК Организация
		|	ИЗ
		|		РегистрБухгалтерии.Хозрасчетный.Остатки(&НачалоПериода, Счет В (&СчетаПроцентовПоОбязательствам), , Организация В (&Организации)) КАК ПроцентыПоОбязательствам
		|	ГДЕ
		|		ПроцентыПоОбязательствам.СуммаОстаток <> 0
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ПроцентыПоОбязательствам.Организация
		|	ИЗ
		|		РегистрБухгалтерии.Хозрасчетный.Остатки(&КонецПериодаВключительно, Счет В (&СчетаПроцентовПоОбязательствам), , Организация В (&Организации)) КАК ПроцентыПоОбязательствам
		|	ГДЕ
		|		ПроцентыПоОбязательствам.СуммаОстаток <> 0) КАК Сводный";
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетЕНВД Тогда
		...
		Возврат Период >= УчетЕНВД.ДатаНачалаПоддержкиЕНВД()
			И ЗначениеНастройки(КешНастроек, "ПлательщикЕНВД", Организация, Период, Периодичность); // Периодичность задана в ПериодичностьРегламентнойОперации()
...
Функция ЗначениеНастройки(КешНастроек, ИмяНастройки, Организация = Неопределено, Период = Неопределено, Периодичность = Неопределено)
	...
	ИначеЕсли ИмяНастройки = "ТребуетсяРасчетСебестоимости" Тогда
		Значение = (ПолучитьФункциональнуюОпцию("ВедетсяПроизводственнаяДеятельность") И УчетнаяПолитика.ВедетсяПроизводственнаяДеятельность(Организация, Период))
...
Функция ПолучитьСписокСправокРасчетовПоВидуОперации(Знач ВидОперации,Знач Организация,Знач ПериодРегистрации = Неопределено) Экспорт
	...
	Если МассивОпераций.Найти(Перечисления.ВидыРегламентныхОпераций.РасчетРасходовУменьшающихОтдельныеНалоги) <> Неопределено Тогда
		...
		Если ПрименяетсяУСНДоходы Тогда
			Список.Добавить("СправкаРасчетРасходовУменьшающихНалогУСН","Расходы, уменьшающие налог УСН");
		КонецЕсли;
		Если ПрименяетсяЕНВД Тогда
			Список.Добавить("СправкаРасчетРасходовУменьшающихЕНВД","Расходы, уменьшающие ЕНВД");
		КонецЕсли;
	КонецЕсли;
...
Функция ПредставлениеОперацииРасчетРасходовУменьшающихОтдельныеНалоги(Организация, Период) Экспорт
	...
	ЗаголовокОперации = Нстр("ru = 'Расчет расходов, уменьшающих %1'");
	
	Если ПрименяетсяУСНДоходы И ПлательщикЕНВД Тогда
		Возврат СтрШаблон(ЗаголовокОперации, НСтр("ru = 'налог УСН и ЕНВД'"));
	ИначеЕсли ПрименяетсяУСНДоходы Тогда
		Возврат СтрШаблон(ЗаголовокОперации, НСтр("ru = 'налог УСН'"));
	ИначеЕсли ПлательщикЕНВД Тогда
		Возврат СтрШаблон(ЗаголовокОперации, НСтр("ru = 'ЕНВД'"));
	Иначе
		Возврат СтрШаблон(ЗаголовокОперации, НСтр("ru = 'налог УСН и ЕНВД'"));
	КонецЕсли;
