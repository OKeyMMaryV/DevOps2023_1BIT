...
Функция ТребуетсяВызовСервераПриИзмененииДатыДокумента(НоваяДата, ПредыдущаяДата,
	...
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2018-10-30 (#3111)
	//Если НЕ Результат И
	//	 НоваяДата>=Дата('20190101') И
	//	 ПредыдущаяДата<Дата('20190101')
	//Тогда 
	//	Результат = Истина;	
	//КонецЕсли;
	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2018-10-30 (#3111)	
...
//	Булево - Признак возможности печати
//
Функция ПроверитьВозможностьПечатиЧеков(Форма, ПараметрыОбработчика = Неопределено) Экспорт
...
КонецФункции // ПроверитьВозможностьПечатиЧеков()
...
Процедура ПредложитьИспользоватьОсновнойИнтерфейс(ВариантПредложения) Экспорт
	// бит_Финанс изменение кода. Начало. 04.06.2014{{
	Возврат;
	// бит_Финанс изменение кода. Конец. 04.06.2014}}
...
Процедура ПредупредитьОНеобходимостиОбновленияПрограммы(ПараметрыРаботыКлиента) Экспорт
	...
	// бит_Финанс начало добавления 24.04.2013
	// не открываем форму, т.к. в ней есть возможность обновить типовую конфигурацию
	Возврат;
	// бит_Финанс конец добавления 24.04.2013
...
Процедура ОткрытьНачинаемРаботатьВ30(ИмяОбработки) Экспорт
	...
	// бит_Финанс изменение кода. Начало. 07.08.2015{{
	Если бит_ПраваДоступа.ЕстьПравоДоступа("Просмотр", "Обработка." + ИмяОбработки) Тогда
		
		ОткрытьФорму("Обработка." + ИмяОбработки + ".Форма");
		
	КонецЕсли;
	// бит_Финанс изменение кода. Конец. 07.08.2015}}
