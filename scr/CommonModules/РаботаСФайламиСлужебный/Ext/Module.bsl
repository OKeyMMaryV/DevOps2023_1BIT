...
Процедура ПометитьНаУдалениеВерсииФайла(Знач ФайлСсылка, Знач ВерсияИсключение) Экспорт
	...
	// бит_Финанс изменения кода. Начало: 20.09.2017 . 
	// Исправлена неточность в определении имени таблицы запроса.
	// Метод Метаданные.НайтиПоТипу(Парам1). Тип значения Парам1 должен быть "ТИП".
	// После исправления со стороны 1С принять правки постащика.
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ВерсииФайлов.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник." + Метаданные.НайтиПоТипу(ТипЗнч(ВерсияИсключение)).Имя + " КАК ВерсииФайлов
	|ГДЕ
	|	ВерсииФайлов.Владелец = &Владелец
	|	И НЕ ВерсииФайлов.ПометкаУдаления
	|	И ВерсииФайлов.Ссылка <> &Исключение";
	// бит_Финанс изменения кода. Конец 20.09.2017 . 	
	...
	Иначе
		...
		Иначе
			...
			Если ок_ТипЗначения = Тип("ДокументСсылка.ПакетЭД") Тогда
				ТекстЗапроса = ок_ОбменСКонтрагентамиВнутренний.ТекстЗапросаДляОчисткиФайловПакетЭД();
			КонецЕсли;	
			//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-12-05 (#3964)
...
Функция ТекстЗапросаДляИзвлеченияТекста(ПолучитьВсеФайлы = Ложь, ДополнительныеПоля = Ложь) Экспорт
	...
	Для Каждого Тип Из ТипыФайлов Цикл
		// *Обновление*, Братухин М.В., 2017.10.19 (
		// Почему-то в состав типов добавлена строка. Обход этой ситуации.
...
Процедура ОбновитьДвоичныеДанныеФайлаНаСервере(Знач ПрисоединенныйФайл,
	...
	Попытка
		...
		//ок_ДатаТранзакции, ок_КодТранзакции
		Если РеквизитыЗначения.Свойство("ДополнительныеСвойства") Тогда
			Для каждого Свойство Из РеквизитыЗначения.ДополнительныеСвойства Цикл
				ФайлОбъект.ДополнительныеСвойства.Вставить(Свойство.Ключ, Свойство.Значение); 
			КонецЦикла;	
		КонецЕсли;
		//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103		
	...
	Исключение
		ОтменитьТранзакцию();
		//ОКЕЙ Бублик А.А.(СофтЛаб) Начало 2020-09-14 (#3816)
		Если ЭтоСсылка Тогда
			РазблокироватьДанныеДляРедактирования(ПрисоединенныйФайл);
		КонецЕсли;
		//ОКЕЙ Бублик А.А.(СофтЛаб) Начало 2020-09-14 (#3816)
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ОбновитьФайл(ИнформацияОФайле, ПрисоединенныйФайл) Экспорт
	...
	//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103
	Если ИнформацияОФайле.Свойство("ДополнительныеСвойства") Тогда
		РеквизитыЗначения.Вставить("ДополнительныеСвойства", ИнформацияОФайле.ДополнительныеСвойства);
	КонецЕсли;
	//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103	
...
Процедура ОчиститьДанныеНенужныхФайлов(НастройкаОчистки, МассивИсключений = Неопределено)
	...
	//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-12-05 (#3964)
	ок_ТипЗначения = ТипЗнч(НастройкаОчистки.ВладелецФайла.ЗначениеПустойСсылки);
	//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-12-05 (#3964)
	...
	Для Каждого Файл Из ДеревоВладельцев.Строки Цикл
		...
		Иначе
			...
			Если ок_ТипЗначения = Тип("ДокументСсылка.ПакетЭД") Тогда
				ПакетЭД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Файл.ФайлСсылка, "ВладелецФайла");
				ПоследнийПакетЭД = Неопределено;
				Если Не ЗначениеЗаполнено(ПакетЭД) Тогда
					Продолжить;
				ИначеЕсли ок_ОбменСКонтрагентамиВнутренний.ЭтоПоследнийПакетЭДПоЭлектронномуДокументу(ПакетЭД, ПоследнийПакетЭД) Тогда
					Продолжить;
				КонецЕсли;	
				Если Не ок_ОбменСКонтрагентамиВнутренний.ЕстьВсеФайлыЭлектронногоДокументаПакетаЭД(ПоследнийПакетЭД) Тогда
					Продолжить;
				КонецЕсли;	
			КонецЕсли;	
			//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-12-05 (#3964)
			...
			//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-12-05 (#3964)
			Если ок_ТипЗначения = Тип("ДокументСсылка.ПакетЭД") Тогда
				Если ЗначениеЗаполнено(ПакетЭД) 
					И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПакетЭД, "СтатусПакета") <> Перечисления.СтатусыПакетовЭД.ок_АрхивУдален Тогда
					
					ПакетЭДОбъект = ПакетЭД.ПолучитьОбъект();
					ПакетЭДОбъект.СтатусПакета = Перечисления.СтатусыПакетовЭД.ок_АрхивУдален;
...
Функция ПолныеИменаСправочниковПрисоединенныхФайлов()
	...
	Для Каждого ТипПрисоединенногоФайла Из Метаданные.ОпределяемыеТипы.ПрисоединенныйФайл.Тип.Типы() Цикл
		// *Обновление*, Братухин М.В., 2017.10.19 (
		// Почему-то в состав типов добавлена строка. Обход этой ситуации.
		Если ТипПрисоединенногоФайла = ТипЗнч("Строка") Тогда
			Продолжить;
		КонецЕсли;
		// *Обновление*, Братухин М.В., 2017.10.19 )		
