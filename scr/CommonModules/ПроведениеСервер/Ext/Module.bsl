...
Процедура ПодготовитьНаборыЗаписейКПроведению(Объект, ВыборочноОчищатьРегистры = Истина) Экспорт
	...
	Если Объект.ДополнительныеСвойства.Свойство("ЭтоНовый")
		И Объект.ДополнительныеСвойства.ЭтоНовый Тогда // очистка регистров в информационной базе не нужна
	...
	СтандартнаяОчисткаРегистров = ВыборочноОчищатьРегистры 
									...
									//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-26 (#4214)
									И Объект.ДополнительныеСвойства.Свойство("ДатаДокументаСдвинутаВперед")
									//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-26 (#4214) 
	...
	Иначе
		...
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-07-26 (#4214) 
			...
			(Объект.ДополнительныеСвойства.Свойство("ДатаДокументаСдвинутаВперед") И Объект.ДополнительныеСвойства.ДатаДокументаСдвинутаВперед));
		//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-07-26 (#4214) 
...
Процедура ПодготовитьНаборыЗаписейКОтменеПроведения(Объект) Экспорт
	...
	РучнаяКорректировка = МетаданныеОбъекта.Реквизиты.Найти("РучнаяКорректировка") <> Неопределено
		И Объект.РучнаяКорректировка;
	// бит_Финанс добавление кода. Начало: 11.10.2016 BF-71 {{
	бит_ОбщегоНазначения.ПодготовитьНаборыЗаписейКОтменеПроведенияРучнаяКорректировкаФакта(Объект, МассивИменРегистров);
	// бит_Финанс добавление кода конец }}  
	// бит_Финанс добавление кода. Начало: 21.11.2016 BF-321 {{
	бит_КонтрольЗакрытогоПериода.ПодготовитьНаборыЗаписейКОтменеПроведенияПереопределяемый(Объект, МассивИменРегистров, Ложь);
	// бит_Финанс добавление кода конец }} 
...
Функция РегистрыТребующиеПринудительнойОчистки(ПроверяемыеРегистры)
	...
	Если ПроверяемыеРегистры.Найти("РасходыПриУСН", "ИмяРегистра") <> Неопределено Тогда
		...
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	// бит_Финанс изменение кода. Начало. 06.09.2017{{
	Если ПроверяемыеРегистры.Найти("бит_КонтрольныеЗначенияБюджетов", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Накопления";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_КонтрольныеЗначенияБюджетов";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Ложь;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_ПараметрыФинИнструментов", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Сведений";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_ПараметрыФинИнструментов";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Ложь;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_СобытияФинИнструментов", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Сведений";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_СобытияФинИнструментов";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_ПараметрыОС", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Сведений";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_ПараметрыОС";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_СобытияОС", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Сведений";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_СобытияОС";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_СостоянияОС", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Сведений";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_СостоянияОС";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_ПараметрыНМА", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Сведений";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_ПараметрыНМА";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_СобытияНМА", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Сведений";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_СобытияНМА";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_СостоянияНМА", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Сведений";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_СостоянияНМА";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Ложь;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_ВыбытиеОС", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Накопления";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_ВыбытиеОС";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_ПартииМПЗ", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Накопления";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_ПартииМПЗ";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_ПартииМПЗСобственныхКонтрагентов", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Накопления";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_ПартииМПЗСобственныхКонтрагентов";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_му_ФинансовыеНачисления", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Накопления";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_му_ФинансовыеНачисления";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_Дополнительный_2", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Бухгалтерии";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_Дополнительный_2";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	Если ПроверяемыеРегистры.Найти("бит_ОборотыПоТипамПроводок", "ИмяРегистра") <> Неопределено Тогда
		НовыйПроверяемыйРегистр = ИменаРегистровПринудительнойОчистки.Добавить();
		НовыйПроверяемыйРегистр.ВидРегистра     = "Накопления";
		НовыйПроверяемыйРегистр.ИмяРегистра     = "бит_ОборотыПоТипамПроводок";
		НовыйПроверяемыйРегистр.ЕстьОрганизация = Истина;
		НовыйПроверяемыйРегистр.ЕстьПериод      = Истина;
	КонецЕсли;
	// бит_Финанс Изменение кода. Конец. 02.08.2013}}
...
Процедура ПодготовитьНаборыЗаписейВОсобойСитуации(Объект, РегистрыТребующиеОчистки, ПараметрыНестандартнойСитуации)
	...
	ИначеЕсли НЕ ПараметрыНестандартнойСитуации.ДатаДокументаСдвинутаВперед Тогда
		...
		Исключение
			...
			"ПроведениеСервер.ПодготовитьНаборыЗаписейВОсобойСитуации", 
			УровеньЖурналаРегистрации.Ошибка,, 
			Объект.Ссылка, 
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) + Символы.ПС 
						   + Нстр("ru = 'Подробности смотри в журнале регистрации'") ;
...
Процедура ПодготовитьНаборыЗаписейКРегистрацииДвижений(Объект, ЭтоНовый = Ложь, ДвиженияМетаданные = НеОпределено) Экспорт
	...
	// Далее вызов УстановитьЗаписьОчищаемыхНаборовЗаписей() не требуется,
