...
Функция ПересчитатьПоКурсу(Сумма, ПараметрыТекущегоКурса, ПараметрыНовогоКурса) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотВызовСервера.ЗначениеРеквизитаОбъекта(ПараметрыТекущегоКурса.Валюта, "бит_КурсыЕвропейскогоБанка") Тогда
		Результат = Окр((Сумма * ПараметрыТекущегоКурса.Курс), 2);
	Иначе
		Результат = Окр((Сумма * ПараметрыТекущегоКурса.Курс * ПараметрыНовогоКурса.Кратность) / (ПараметрыНовогоКурса.Курс * ПараметрыТекущегоКурса.Кратность), 2);
	КонецЕсли;
	Возврат Результат;
	//***БИТ***Теплова***)
