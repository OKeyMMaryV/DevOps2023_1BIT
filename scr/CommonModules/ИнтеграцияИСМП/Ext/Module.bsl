...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	Настройка.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ВестиУчетОбувнойПродукцииИСМП;
	
	Настройка = Настройки.Добавить();
	Настройка.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ОчисткаПулаКодовМаркировкиИСМП;
	Настройка.РаботаетСВнешнимиРесурсами = Ложь;
	Настройка.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ВестиУчетТабачнойПродукцииМОТП;
...
Функция РазрешенаПовторнаяПечатьКодовМаркировки() Экспорт
	...
	Возврат РольДоступна("ПовторнаяПечатьКодовМаркировкиИСМП");
...
//
Функция ВидыПродукцииПоТаблицеНоменклатуры(Номенклатура) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапроса;
	Запрос.УстановитьПараметр("Товары", Номенклатура);
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
					...
					Если Не ЗначениеЗаполнено(СтатусОбработки) Тогда
						...
						ИначеЕсли ПротоколОбмена.Количество() > 0 Тогда
							...
							Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
								...
								Статус = ПоследняяЗаписьПротоколаОбмена.Статус;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
			...
			Иначе
				...
				ПараметрыОбновленияСтатуса.ОперацияКвитанции = Выборка.Операция;
				ПараметрыОбновленияСтатуса.ПротоколОбмена    = Выборка.ПротоколОбмена.Получить();
				ПараметрыОбновленияСтатуса.ПараметрыЗапроса  = Неопределено;
...
Функция ПредставлениеСтатуса(Статус, ВходящиеДальнейшееДействие, ДопустимыеДействия) Экспорт
	...
	Для Каждого ДальнейшееДействие Из ДальнейшиеДействия Цикл
		...
		ИначеЕсли ДальнейшееДействие = Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗапроситеGTINНаОстатки Тогда
			...
			Представление    = "запросите GTIN на остатки"
...
Функция ИспользуетсяИнтеграцияВФормеДокументаОснования(Форма, Объект) Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ВестиУчетОбувнойПродукцииИСМП")
		И Не ПолучитьФункциональнуюОпцию("ВестиУчетТабачнойПродукцииМОТП") Тогда
		Возврат Ложь;
...
//
// Параметры:
//	МетаданныеДокумента - ОбъектМетаданных:Документ - метаданные документа из ОпределяемыйТип.ДокументыИСМППоддерживающиеСтатусыОформления
//
// Возвращаемое значение:
...
Функция ПараметрыОбмена(Организации = Неопределено, УникальныйИдентификатор = Неопределено, ЭтоРегламентноеЗадание = Ложь) Экспорт
	...
	ПараметрыОбмена.Вставить("МаксимальноеКоличествоКодовМаркировкиЗагружаемыхИзПула", 1000);
	...
	ПараметрыОбмена.Вставить("КонтрольАктивныхЗаказовПоСУЗ", Новый Соответствие());
...
Функция НоваяСтруктураАктивныхЗаказовПоСУЗ() Экспорт
	...
	ТаблицаАктивныхЗаказовНаЭмиссию = Новый ТаблицаЗначений;
	ТаблицаАктивныхЗаказовНаЭмиссию.Колонки.Добавить("ИдентификаторЗаказа", Новый ОписаниеТипов("Строка"));
	ТаблицаАктивныхЗаказовНаЭмиссию.Индексы.Добавить("ИдентификаторЗаказа");
	
	СтруктураВозврата = Новый Структура;
	СтруктураВозврата.Вставить("АктивныеЗаказыНаЭмиссию",                ТаблицаАктивныхЗаказовНаЭмиссию);
	СтруктураВозврата.Вставить("ДатаЗапросаАктивныхЗаказовНаЭмиссиюСУЗ", '00010101');
	СтруктураВозврата.Вставить("ДоступноеКоличествоЗаказовНаЭмиссиюСУЗ", 0);
	СтруктураВозврата.Вставить("НастройкаОбменаСУЗ",                     Неопределено);
...
// Формирует массив GTIN для выбора в полях ввода
// 
// Параметры:
// 	ТекущиеДанные - ДанныеФормыЭлементКоллекции - Текущая строка таблицы.
// 	ФормаВладелец - ФормаКлиентскогоПриложения  - Форма-источник события.
// 	ВключатьGTINНаОстатки - Булево - Определяет необходимость включения в состав остаточных GTIN.
// Возвращаемое значение:
// 	Массив из Строка - Значения GTIN.
Функция МассивЗначенийGTINДляВыбора(ТекущиеДанные, ФормаВладелец, ВключатьGTINНаОстатки = Ложь) Экспорт
	...
	Если ВключатьGTINНаОстатки Тогда
		...
		Запрос.УстановитьПараметр("КодТНВЭД",           Лев(ТекущиеДанные.КодТНВЭД, 2));
		Запрос.УстановитьПараметр("ВидОбуви",           ТекущиеДанные.ВидОбуви);
		Запрос.УстановитьПараметр("СпособВводаВОборот", ТекущиеДанные.СпособВводаВОборот);
		Запрос.УстановитьПараметр("Организация",        ФормаВладелец.Объект.Организация);
		Запрос.УстановитьПараметр("НачалоGTIN",         ИнтеграцияИСМПСлужебныйКлиентСервер.НачалоGTINМаркировкиОстатковОбуви());
		
		МассивТекстовЗапросов.Добавить("ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		...
		|	И ПОДСТРОКА(ЗаказНаЭмиссиюКодовМаркировкиСУЗТовары.КодТНВЭД, 1, 2) = &КодТНВЭД
		|	И ЗаказНаЭмиссиюКодовМаркировкиСУЗТовары.ВидОбуви                  = &ВидОбуви
		|	И ЗаказНаЭмиссиюКодовМаркировкиСУЗТовары.СпособВводаВОборот        = &СпособВводаВОборот");
...
Функция ЭтоТестовыйПериод() Экспорт
	...
	ЭтоТестовыйПериод = ПолучитьФункциональнуюОпцию("ВестиУчетОбувнойПродукцииИСМП")
		И ДатаОбязательнойМаркировкиОбуви() > НачалоДня(ТекущаяДатаСеанса());
...
//
Функция ДатаОбязательнойМаркировкиОбуви()
	
	Возврат Константы.ДатаОбязательнойМаркировкиОбувиИСМП.Получить();
