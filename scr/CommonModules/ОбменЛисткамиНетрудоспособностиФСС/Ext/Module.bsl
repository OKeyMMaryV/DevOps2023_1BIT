...
Функция ВыгрузитьЗапросДляОтправкиРеестраЭЛН(Документ, ПомещатьВФайл, ИдентификаторФормы, СлужебныйФормат, ИсключатьПринятые) Экспорт
	...
	ROWSET.version_software = Лев(бит_ОбщегоНазначения.МетаданныеВерсия(), 15);
...
Функция ЗагрузитьОтветСервисаФСС(Документ, Операция, АдресРасшифрованногоОтвета, ЗагрузкаИзФайла = Ложь) Экспорт
	...
	Если Операция = "getPrivateLNData" Тогда
		...
		ДанныеИзСервиса = СтруктураОтветаСервисаПолученияЭЛН(LN_ROW, Результат.Отказ);
		Если Результат.Отказ Тогда
			Возврат Результат;
		КонецЕсли;
		
		Если ТипЗнч(Документ) = Тип("Структура") Тогда
			// Метод вызван только для актуализации Хэша ЭЛН.
			РегистрыСведений.СведенияОбЭЛН.ЗаписатьХэш(
...
Функция СтруктураОтветаСервисаПолученияЭЛН(LN_ROW, Отказ)
	ИменаПолейИзСервиса = 
	"APPROVE1,
	|APPROVE2,
	|BASE_AVG_DAILY_SAL,
	...
	ДанныеИзСервиса.Вставить("ТаблицаПолныхПериодов", СписокXDTO(LN_ROW, "TREAT_PERIODS.TREAT_FULL_PERIOD"));
	...
	Если ТипЗнч(ДанныеИзСервиса.SNILS) = Тип("Строка") И ЗначениеЗаполнено(ДанныеИзСервиса.SNILS) Тогда
		ДанныеИзСервиса.СНИЛС = СтрШаблон(
			"%1-%2-%3 %4",
			Сред(ДанныеИзСервиса.SNILS, 1, 3),
			Сред(ДанныеИзСервиса.SNILS, 4, 3),
			Сред(ДанныеИзСервиса.SNILS, 7, 3),
			Сред(ДанныеИзСервиса.SNILS, 10, 2));
	КонецЕсли;
...
Процедура ЗагрузитьРезультатПолученияЭЛН(Документ, ДанныеИзСервиса, ЗагрузкаИзФайла, Отказ)
	...
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	// Перед записью хэша необходимо определить организацию и физлицо.
...
Процедура ЗаполнитьСотрудникаПоСНИЛС(Документ, ДанныеИзСервиса, Отказ)
	...
	РезультатПоиска = СотрудникОрганизации(Документ.Организация, ДатаНачалаСобытия, ФизическоеЛицо);
...
Функция ФизическоеЛицо(СНИЛС, Фамилия, Имя, Отчество, ТекстОшибки)
...
Функция СотрудникОрганизации(ПредполагаемаяОрганизация, ДатаНачалаСобытия, ФизическоеЛицо)
	...
	ПолныеПрава = (ДоступныеФилиалы = Неопределено);
