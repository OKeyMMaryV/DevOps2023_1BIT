...
Функция ПодготовитьПараметрыВыручкиОтРеализации(ТаблицаТоварыУслуги, ТаблицаВзаиморасчеты, ТаблицаСписанныеТовары, ТаблицаРеквизиты)
	...
	Если ТипЗнч(ТаблицаТоварыУслуги) = Тип("ТаблицаЗначений") И ТаблицаТоварыУслуги.Колонки.Найти("Субконто3") <> Неопределено Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",Субконто3"
	КонецЕсли;
	//izhtc-Kir 18.08.2015 )
	//izhtc alena 18.08.2015 (
	Если ТипЗнч(ТаблицаТоварыУслуги) = Тип("ТаблицаЗначений") И ТаблицаТоварыУслуги.Колонки.Найти("Субконто2") <> Неопределено Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",Субконто2"
	КонецЕсли;
	//izhtc alena 18.08.2015 )	
...
Функция ПодготовитьПараметрыРеализация(ТаблицаСобственныеТоварыУслуги, ТаблицаТоварыУслугиКомитентов, ТаблицаРеализованныеТоварыКомитентов, ТаблицаРеквизиты)
	...
	;	
	//izhtc-Kir 18.08.2015 (
	Если ТипЗнч(ТаблицаСобственныеТоварыУслуги) = Тип("ТаблицаЗначений") И ТаблицаСобственныеТоварыУслуги.Колонки.Найти("Субконто3") <> Неопределено Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",Субконто3"
	КонецЕсли;
	//izhtc-Kir 18.08.2015 )
	//izhtc alena 18.08.2015 (
	Если ТипЗнч(ТаблицаСобственныеТоварыУслуги) = Тип("ТаблицаЗначений") И ТаблицаСобственныеТоварыУслуги.Колонки.Найти("Субконто2") <> Неопределено Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",Субконто2"
	КонецЕсли;
	//izhtc alena 18.08.2015 )	
...
Процедура СформироватьДвиженияРеализацияСобственныхТоваровУслуг(ТаблицаВыручкиОтРеализации, ПараметрыРеализации, ЭтоВозврат, Движения, Отказ)
	...
	Для каждого СтрокаТаблицы Из ТаблицаВыручкиОтРеализации Цикл
		...
		//izhtc alena 18.08.2015(
		Если ТипЗнч(ПараметрыРеализации.Регистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			//ОК Калинин М. 260313
		...
		Если ТипЗнч(ПараметрыРеализации.Регистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда 
			...
			КонецЕсли;
			// БИТ AMerkulov --
			// +СБ. Широков Николай. 2016-05-23. #2466 Не переопределяется выбранным значением субконто 2
			Если ЗначениеЗаполнено(СтрокаТаблицы.Субконто2) Тогда
		...
		Если ТипЗнч(ПараметрыРеализации.Регистратор) = Тип("ДокументСсылка.ПередачаОС")
			ИЛИ ТипЗнч(ПараметрыРеализации.Регистратор) = Тип("ДокументСсылка.СписаниеОС") Тогда
...
Процедура  ЗаполнитьАналитикуСчетаДохода(Проводка, СтрокаТаблицы, ТипАналитики, ОтражатьВНалоговомУчете, СуммаВР = 0)
	...
	//ОК Калинин М. 260313
	Попытка
		БухгалтерскийУчет.УстановитьСубконто(Счет, Проводка["Субконто"+ТипАналитики], 3,                   СтрокаТаблицы.Субконто3);
	Исключение
	КонецПопытки;
	//ОК Калинин М. 
	//izhtc-Kir 18.08.2015 )	
...
Функция ПодготовитьТаблицуРеализованныхТоваровУслугКомитентов(ТаблицаТоварыУслугиКомитентов)
	...
	СписокКолонок = "Покупатель,Комитент,ДоговорКомиссии,Номенклатура,Количество,СуммаРасчетовСКомитентом,"
		+ "СуммаПоступленияОтКомитента,Партия,ДатаРеализации,СуммаНДСРуб,Контрагент";
			...
			Иначе
				...
				Если Не СвойстваСчетаКт.Забалансовый Тогда
					...
					Если СтрокаПереоценка.РасчетыВУсловныхЕдиницах
						...
						//ОК+ Аверьянова 27.05.14 заявка #AT-1188871# от 27.05.14
						...
						Если Дата <= '20140101' Тогда
							Проводка.СубконтоДт.ПрочиеДоходыИРасходы = КлассификаторыДоходовРасходов.ПредопределенныйЭлементГрупповойОперации(
								...
								СтатьиДоходовРасходов);
						Иначе
							Проводка.СубконтоДт.ПрочиеДоходыИРасходы = Справочники.ПрочиеДоходыИРасходы.НайтиПоКоду("00001");  //статья: "Курсовые (суммовые) разницы"
						КонецЕсли;
						//ОК- Аверьянова 27.05.14
					Иначе
						Проводка.СубконтоДт.ПрочиеДоходыИРасходы = КлассификаторыДоходовРасходов.ПредопределенныйЭлементГрупповойОперации(
							"КурсовыеРазницы",
							СтатьиДоходовРасходов);
					...
					Если СтрокаПереоценка.РасчетыВУсловныхЕдиницах И НЕ СтрокаПереоценка.ПереоцениватьПоНУпосле2015 Тогда
						...
						Если Дата <= '20140101' Тогда
							Проводка.СубконтоКт.ПрочиеДоходыИРасходы = КлассификаторыДоходовРасходов.ПредопределенныйЭлементГрупповойОперации(
								...
								СтатьиДоходовРасходов);
						Иначе
							Проводка.СубконтоКт.ПрочиеДоходыИРасходы = Справочники.ПрочиеДоходыИРасходы.НайтиПоКоду("00001");  //статья: "Курсовые (суммовые) разницы"
						КонецЕсли;
						//ОК- Аверьянова 27.05.14							
					Иначе
						Проводка.СубконтоКт.ПрочиеДоходыИРасходы = КлассификаторыДоходовРасходов.ПредопределенныйЭлементГрупповойОперации(
...
Процедура СформироватьПроводкиКорректировкаСтоимостиАрендованногоИмуществаПриПереоценке(Проводки, ТаблицаПереоценка, СчетаПереоценкиКеш, Реквизиты)
	...
	Запрос = Новый Запрос(
		...
		|	ДанныеПредметовАренды.СуммаРуб КАК СуммаПоступления
		|ИЗ
		...
		|				ИЗ
		|					КонтрагентыДоговоры)) КАК ДанныеПредметовАренды");
...
Функция ПодготовитьТаблицуВалютныхОстатковВсе(СтруктураПараметров, Движения)
	...
	// Оборотные субконто замененяются на пустую ссылку или Неопределено.
