...
Функция ПредставлениеШтрихкода(Штрихкод) Экспорт
	...
	Если ЭтоШтрихкодАкцизнойМарки(Штрихкод, ТипШтрихкода) Тогда
	...
	ИначеЕсли ШтрихкодированиеЕГАИСКлиентСервер.ЭтоШтрихкодЛогистическойУпаковки(Штрихкод) Тогда
		ПредставлениеШтрихкода = Штрихкод;
	Иначе
		ПредставлениеШтрихкода = ШтрихкодированиеИСКлиентСервер.ПредставлениеШтрихкода(Штрихкод);
	КонецЕсли;
...
// * ТребуетсяВыборСправки2 - Истина, если необходимо открыть диалоговое окно с возможностью выбора справки 2.
...
//       и (См. ШтрихкодированиеЕГАИС.ДополнитьСвойстваРезультатаОбработкиШтрихкода)
...
Процедура ДополнитьДанныеПоШтрихкодамСпецификой(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Если ЗаполнитьАлкогольнуюПродукциюВСтрокахДерева Тогда
		...
		СоздатьШтрихкодыУпаковок(ДанныеПоШтрихкодам.ВложенныеШтрихкоды.ДеревоУпаковок, Неопределено, Истина);
	КонецЕсли;
...
Функция РезультатОбработкиШтрихкодаМаркируемойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	Если ЭтоШтрихкодАкцизнойМарки(Штрихкод)
...
// Формирует данные для штрихкодов, сведения о которых отсутствуют в справочнике "Штрихкоды упаковок".
//
// Параметры:
//  Штрихкоды
//  ПараметрыСканирования - (См. ШтрихкодированиеИСКлиент.ПараметрыСканирования).
...
//  РезультатОбработкиШтрихкода - (См. ШтрихкодированиеЕГАИС.ИнициализироватьРезультатОбработкиШтрихкода).
//  КэшированныеЗначения - Структура - закэшированные значения формы.
// Возвращаемое значение:
//  (См. ШтрихкодированиеЕГАИС.ИнициализироватьРезультатОбработкиШтрихкода).
...
Функция ОбработатьДанныеШтрихкодаПослеВыбораНоменклатуры(РезультатВыбора, РезультатОбработкиШтрихкода) Экспорт
	...
	Если ДанныеШтрихкода.Номенклатура <> РезультатВыбора.Номенклатура
		...
		Если Не ЗначениеЗаполнено(ДанныеШтрихкода.МаркируемаяПродукция) Тогда
...
// Определяет тип штрихкода для единицы маркируемой продукции по значению штрихкода акцизной марки.
//
// Параметры:
//  Штрихкод
// Возвращаемое значение:
//  ПеречислениеСсылка.ТипыШтрихкодов, Неопределено - Вычисленный тип штрихкода.
...
// Определяет тип штрихкода для логистической упаковки по значению штрихкода.
//
// Параметры:
//  Штрихкод - Строка - штрихкод логистической упаковки.
// Возвращаемое значение:
//  ПеречислениеСсылка.ТипыШтрихкодов - тип штрихкода логистической упаковки.
...
// Формирует данные для штрихкодов, сведения о которых отсутствуют в справочнике "Штрихкоды упаковок". Данные формируются
// для 150 символьных марок с форматом DataMatrix и марок с форматом штрихкода PDF417.
//
// Параметры:
//  Штрихкоды - Массив - Коллекция штрихкодов акцизных марок.
//  ПараметрыСканирования - (См. ШтрихкодированиеИСКлиент.ПараметрыСканирования).
// Возвращаемое значение:
//  Соответствие - Ключ - значение штрихкода, значение - данные штрихкода.
...
// Формирует данные для штрихкодов, сведения о которых отсутствуют в справочнике "Штрихкоды упаковок". Данные формируются
// для акцизных марок, которые включают в себя серию и номер.
//
// Параметры:
//  Штрихкоды - Массив - Коллекция штрихкодов акцизных марок.
// Возвращаемое значение:
...
Функция ДанныеНеизвестныхШтрихкодовСНомеромИСерией(Штрихкоды)
	...
	Для Каждого Штрихкод Из Штрихкоды Цикл
		...
		СтруктураШтрихкода = АкцизныеМаркиКлиентСервер.РазложитьШтрихкодСНомеромИСерией(Штрихкод, ДанныеШтрихкода.ТекстОшибки);
...
//   * Характеристика - ОпределяемыйТип.ХарактеристикиНоменклатуры - Характеристика номенклатуры.
//   * Упаковка - ПеречислениеСсылка.ШтрихкодыУпаковок - Упаковка.
...
// Определяет тип штрихкода акцизной марки по его значению.
//
// Параметры:
//  Штрихкод - Строка - Штрихкод.
...
//  ШтрихкодУпаковки - СправочникСсылка.ШтрихкодУпаковкиТоваров - ссылка на элемент справочника.
...
// Инициализирует структуру "Данные штрихкода" путем получения базовой структуры и дополнения ее специфичными свойствами
// алкогольной продукции.
//
// Параметры:
// Возвращаемое значение:
//  Структура - (См. ШтрихкодированиеИС.ИнициализироватьДанныеШтрихкода)
//          и (См. ШтрихкодированиеЕГАИС.ДополнитьСвойстваДанныхШтрихкода).
...
//            и (См. ШтрихкодированиеЕГАИС.ДобавитьКолонкиДляДереваУпаковок).
...
// Выполняет проверку на принадлежность штрихкода к акцизной марке, длина которой 150 символов.
//
// Параметры:
//  Штрихкод - Строка - Штрихкода акцизной марки.
// Возвращаемое значение:
//  Булево - Истина, если значение штрихкода является акцизной маркой 150 символов.
...
// Дополняет "ОбработанныеШтрихкоды", входящие в состав "ДанныеПоШтрихкодам" спецификой, которая находится в дереве
// упаковок.
//
// Параметры:
//  ДанныеПоШтрихкодам - (см. ШтрихкодированиеИС.ИнициализацияДанныхПоШтрихкодам).
...
// Дополняет базовую структуру "ДанныеПоШтрихкодам" сведениями по алкогольной продукции.
//
// Параметры:
//  Выборка - ВыборкаИзРезультатаЗапроса - Выборка с данными для заполнения.
//  ДанныеШтрихкода - (См. ШтрихкодированиеИС.ИнициализацияДанныхПоШтрихкодам).
//  СтрокаДерева - СтрокаДереваЗначений - строка дерева упаковок.
//  КодыАлкогольнойПродукции - Массив из Число - коллекция кодов алкогольной продукции.
//  ЭлементыДляЗаполненияАлкогольнойПродукции - Массив - коллекция, в которую будут добавляться структуры - "ДанныеПоШтрихкодам",
//     в случае неудачного определения алкогольной продукции.
