...
Функция НовыйКонтекстРасчетаНалога(Организация, ПериодРасчета) Экспорт
	...
	КонтекстРасчета.Вставить("СтавкаНалогаУСН", УчетнаяПолитика.СтавкаНалогаУСН(Организация, КонецОтчетногоПериода));
...
Функция ПотенциальноВозможныйДоход(Организация, ДатаНачала, ДатаОкончания, МассивПатентов = Неопределено, СообщатьОбОшибках = Истина) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ПотенциальноВозможныйГодовойДоход <> 0 Тогда
			ПотенциальноВозможныйДоход = ПотенциальноВозможныйДоход + УчетПСНКлиентСервер.РассчитатьПотенциальноВозможныйДоход(
...
Функция ПодготовитьТаблицуРасходовНаУплатуСтраховыхВзносов(ТаблицаРеквизитов) Экспорт
	...
	Если Не УчетнаяПолитика.ПлательщикЕНВД(Реквизиты.Организация, КонецПериода)
		И Не УчетнаяПолитика.ПлательщикЕНВД(Реквизиты.Организация, НачалоДня(ДатаИзменения) - 1) Тогда
		Возврат ТаблицаРасходов;
	КонецЕсли;
...
Функция СрокУплатыНалогаЗаПериод(Период, Организация) Экспорт
	...
	НачалоОбзора = НачалоКвартала(Период);
	КонецОбзора  = КонецКвартала(Период) + 1; // Для включения в расписание искомый период должен полностью входить в интервал обзора.
	...
	ОтборРасписания = Новый Структура;
	ОтборРасписания.Вставить("ИмяЗадачи", РасчетыСБюджетом.ИмяЗадачиБухгалтера(ВидНалога));
	ОтборРасписания.Вставить("Действие", Перечисления.ВидыДействийКалендаряБухгалтера.УплатаНалога);
	
	Расписание = РегистрыСведений.ЗадачиБухгалтера.РасписаниеПоНалогамОтчетамЗаПериод(
		Организация, НачалоОбзора, КонецОбзора, ОтборРасписания);
