...
//  ПериодРасчета    - Дата - дата из квартала, за который рассчитывается налог.
//
// Возвращаемое значение:
...
Функция НовыйКонтекстРасчетаНалога(Организация, ПериодРасчета) Экспорт
	...
	КонтекстРасчета.Вставить("ПрименяетсяВычетПоОнлайнКассам",
		УчетЕНВД.ПрименяетсяВычетПоОнлайнКассам(Организация, ПериодРасчета));
...
Функция ПодготовитьТаблицыРасчетаЕНВД(КонтекстРасчета, Отказ, Ошибки) Экспорт
	...
	ПредставлениеПериода = ПредставлениеПериода(КонтекстРасчета.НачалоПериода, КонтекстРасчета.КонецПериода, "ФП=Истина");
	СодержаниеПроводки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
