...
Процедура ДобавитьФайл(ПрисоединенныйФайл,
	...
	Попытка
		...
		//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-12-03 (#3948)
...
Процедура УдалитьДанныеФайла(ПрисоединенныйФайлОбъект, ТекстКомментария = "") Экспорт
	...
	ок_СвойстваФайла = СвойстваФайлаВТоме();
	ЗаполнитьЗначенияСвойств(ок_СвойстваФайла, ПрисоединенныйФайлОбъект);
	//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-12-28 (#3964)
	...
	ЗаполнитьЗначенияСвойств(СвойстваФайла, ПрисоединенныйФайлОбъект);
	//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-12-28 (#3964)
	//ошибка в типовом коде, получают файл по уже очищенным реквизитам
	//УдалитьФайл(ПолноеИмяФайлаВТоме(СвойстваФайла));
	УдалитьФайл(ПолноеИмяФайлаВТоме(ок_СвойстваФайла));
	//ОКЕЙ Вдовиченко Г.В(СофтЛаб) Начало 2020-12-28 (#3964)
...
Функция ОбъемТома(Том) Экспорт
	...
	Для Каждого Тип Из ТипыПрисоединенныхФайлов Цикл
		...
		Если Тип = Тип("СправочникСсылка.ВерсииФайлов")
			Или Тип = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
			//ОКЕЙ Бублик А.А.(СофтЛаб) Начало 2020-08-26 (#3816)
			Или Тип = Тип("Строка")
			//ОКЕЙ Бублик А.А.(СофтЛаб) Конец 2020-08-26 (#3816)
		Тогда	
			Продолжить;
		КонецЕсли;
...
Процедура ПроверитьЦелостностьФайлов(ТаблицаФайловНаДиске, Том) Экспорт
	...
	Для Каждого СправочникФайлов Из ТипыФайлов Цикл
		...
		Если СправочникФайлов = ТипЗнч("Строка") Тогда
			Продолжить;
		КонецЕсли;
		// *Обновление*, Братухин М.В., 2017.10.19 )		
