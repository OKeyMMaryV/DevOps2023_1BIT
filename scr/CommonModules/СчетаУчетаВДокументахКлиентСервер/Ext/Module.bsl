...
Процедура НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, ПолноеИмяРеквизита) Экспорт
	...
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Начало 2021-06-15 (#4167)
	ПользовательскаяФункция = ОК_ОбщегоНазначения.ПолучитьЗначениеУниверсальнойНастройки("ЗаполнениеОбъектов", "ФункцияЗаполнятьРеквизитОбъекта");
	Если ЗначениеЗаполнено(ПользовательскаяФункция) Тогда
		
		ЗаполнятьЭтотРеквизит = бит_ОбщегоНазначения.ВыполнитьПользовательскуюФункцию(ПользовательскаяФункция, 
																						ПараметрыЗаполнения.ИмяДокумента, 
																						ИмяРеквизитаДетально.ТабличнаяЧасть, 
																						ИмяРеквизитаДетально.Реквизит);
		
		Если ЗаполнятьЭтотРеквизит = Ложь Тогда
			Возврат;
		КонецЕсли;
		
	КонецЕсли;
	// ОКЕЙ Смирнов М.В. (СофтЛаб) Конец 2021-06-15 (#4167)
