...
Функция ЭлектронноеПредставлениеСообщения(ТипТранзакции, ПараметрыПредставления) Экспорт
	...
	Если ТипТранзакции = Перечисления.ТипыТранзакцийОбменаСБанкамиЗаявкиНаКредит.ЗаявкаНаКредит Тогда
		ВывестиАтрибут(Компоновщик, "ВерсФорм",  "1.01.04");
...
Процедура ЗаполнитьТекстСогласияВФоне(ПараметрыЗаполнения, АдресХранилища) Экспорт
	...
	Если ТипЗнч(ПараметрыЗаполнения.ОбъектыСогласия[0]) = Тип("СправочникСсылка.БанкиУниверсальногоОбмена") Тогда
		
		РеквизитыОбъектов = ПодготовитьИнформациюОБанкахДляСогласий(
...
//
Функция ПодготовитьИнформациюОБанкахДляСогласий(Банки, ПараметрыСогласияЗаемщика = Неопределено) Экспорт
	...
	Если ПараметрыСогласияЗаемщика <> Неопределено Тогда
		...
		ПодобратьТекстыМакетов(РеквизитыБанков, ПараметрыСогласияЗаемщика.ЭтоЮридическоеЛицо, ПараметрыСогласияЗаемщика.ДатаСогласия);
