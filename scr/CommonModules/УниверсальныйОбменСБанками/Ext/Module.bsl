...
Процедура АктуализироватьДанныеСервиса(Сервис) Экспорт
	...
	Если РезультатАвторизациии.Выполнено Тогда
		...
		Если НЕ Результат.Выполнено Тогда
			ВызватьИсключение НСтр("ru='Ошибка получения данных с сервера обмена с банками.'");
		КонецЕсли;
	Иначе
		ВызватьИсключение НСтр("ru='Ошибка авторизации на сервере обмена с банками.'");
	КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"bankflow.1c.ru",
		443,
		НСтр("ru = 'Сервер обмена с банками в рамках технологии 1С:БанкКоннект'"));
...
Функция ПолучитьСписокВходящихТранзакцийНаСервере(Токен, Документооборот, Страница = 0, Данные = Неопределено) Экспорт
	...
	ДанныеДокументооборота = ОбщегоНазначения.
		ЗначенияРеквизитовОбъекта(Документооборот, "Идентификатор, Банк, Сервис");
	...
	Для каждого Контейнер Из Данные Цикл
		// В первую очередь пытаемся определить тип транзакции, если тип ошибочный делаем запись в ЖР и продолжаем загрузку.
		Попытка
...
Функция ВерсияПрограммы()
	...
	Возврат СокрЛП(бит_ОбщегоНазначения.МетаданныеВерсия());
