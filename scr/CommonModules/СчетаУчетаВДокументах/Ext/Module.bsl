...
Функция МетодЗаполнения(Правило)
	...
	ИначеЕсли РесурсНастройкиЗапасы(Роль) <> Неопределено Тогда
		Возврат "Запасы";
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-11-01 (#4405)
	ИначеЕсли Роль = "РасходыНаМалоценку" Тогда
		Возврат "РасходыНаМалоценку";
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-11-01 (#4405)
	ИначеЕсли РесурсНастройкиРасчеты(Роль) <> Неопределено Тогда
		Возврат "Расчеты";
	ИначеЕсли Роль = "АрендныеОбязательства" Или Роль = "АрендныеПлатежи" Тогда
	...
	ИначеЕсли Роль = "Затраты"
		...
		Возврат "Затраты";
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-11-01 (#4405)
	ИначеЕсли Роль = "ДоходыОтСписанияОС" Тогда
		Возврат "ПрочиеДоходы";
	ИначеЕсли Роль = "СубконтоПрочихДоходов1" Или Роль = "СубконтоПрочихДоходов2" Или Роль = "СубконтоПрочихДоходов3" Тогда
		Возврат "СубконтоПрочихДоходов";
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-11-01 (#4405)
...
Функция НастройкиДляЗаполнения(Правило)
	...
	ИначеЕсли МетодЗаполнения = "РасчетыПоАренде" Тогда
...
Функция НастройкиДляПроверкиЗаполнения(Правило)
	...
	Если МетодЗаполнения = "Расчеты"
		Или МетодЗаполнения = "РасчетыПоАренде"
...
Функция ПроверитьУсловиеПоФункциональнымОпциям(Условие) // очередность, сложность - 0
	...
	ИначеЕсли Условие = "УслугиАрендыЛизинга" Тогда
		ИмяФункциональнойОпции = "ВедетсяУчетОсновныхСредств";
...
Процедура ЗаполнитьРеквизит(Параметры)
	...
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-11-01 (#4405)
	ИначеЕсли МетодЗаполнения = "ПрочиеДоходы" Тогда
		
		ЗаполнитьПрочиеДоходы(Параметры);
		
	ИначеЕсли МетодЗаполнения = "СубконтоПрочихДоходов" Тогда
		
		ЗаполнитьСубконтоПрочихДоходов(Параметры);
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-11-01 (#4405)
	ИначеЕсли МетодЗаполнения = "ВидДеятельности" Тогда
	...
	ИначеЕсли МетодЗаполнения = "РасчетыПоАренде" Тогда
		...
		ЗаполнитьРасчетыПоАренде(Параметры);
...
Процедура ЗаполнитьЗатраты(Параметры)
	...
	ИначеЕсли Параметры.Правило.Роль = "РасходыУслугиФакторинга" Тогда
		Параметры.Значение = ПланыСчетов.Хозрасчетный.ОсновнойСчет("ПрочиеРасходы");
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-11-01 (#4405)
	ИначеЕсли Параметры.Правило.Роль = "РасходыОтСписанияОС" Тогда
		Параметры.Значение = ПланыСчетов.Хозрасчетный.ОсновнойСчет("ПрочиеРасходы");
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-11-01 (#4405)
...
Процедура ЗаполнитьСубконтоЗатрат(Параметры)
	...
	Если Не ЗначениеЗаполнено(Параметры.Значение) Тогда
		Если ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы Тогда
			...
			//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-11-01 (#4405)
			ИначеЕсли СвойствоВладельцаПравила(Параметры.Правило, "Роль") = "РасходыОтСписанияОС" Тогда
				Параметры.ЗначениеПоУмолчанию = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.ЛиквидацияОсновныхСредств");
			//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-11-01 (#4405)
...
Процедура ЗаполнитьПрочиеДоходы(Параметры)
	
	Параметры.ЗначениеПоУмолчанию = ПланыСчетов.Хозрасчетный.ОсновнойСчет("ПрочиеДоходы");
	...
	Если Параметры.Правило.Роль = "ДоходыОтСписанияОС" Тогда
		Параметры.Значение = ПланыСчетов.Хозрасчетный.ОсновнойСчет("ПрочиеДоходы");
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьСубконтоПрочихДоходов(Параметры)
	
	КоличествоСубконто = 3;
	
	// Определим НомерСубконто
	Для НомерСубконто = 1 По КоличествоСубконто + 1 Цикл
		Если Параметры.Правило.Роль = "СубконтоПрочихДоходов" + НомерСубконто Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Если Не ЗначениеЗаполнено(Параметры.Значение) Тогда
		Если ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы Тогда
			Если СвойствоВладельцаПравила(Параметры.Правило, "Роль") = "ДоходыОтСписанияОС" Тогда
				Параметры.ЗначениеПоУмолчанию = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.ЛиквидацияОсновныхСредств");
			Иначе
				Параметры.ЗначениеПоУмолчанию = Справочники.ПрочиеДоходыИРасходы.ОсновнаяСтатьяПрочихРасходов();
			КонецЕсли;
...
Функция ПроверятьСоответствиеСчетаВалютеДоговора(Правило, КешОбщихЗначений = Неопределено)
	...
	МетодЗаполнения = МетодЗаполнения(Правило);
	Возврат (МетодЗаполнения = "Расчеты" Или МетодЗаполнения = "РасчетыПоАренде");
...
Функция ДанныеЗаполненияИзДокумента(МенеджерОбъекта, Документ, Правила)
	...
	//	
	//КонецЦикла;
	ДанныеЗаполнения = ДанныеЗаполненияИзРеквизитовДокумента(Документ, Правила);
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-11-01 (#4405)
...
//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-11-01 (#4405)
Функция ДанныеЗаполненияИзРеквизитовДокумента(Документ, Правила)
...
//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-11-01 (#4405)
// Заполняет счета учета в таблице по правилам заполнения.
// Условия заполнения можно определить гибко с помощью параметра Отбор.
//
// Параметры:
//  Правила - Описание правил заполнения. Должны соответствовать требованиям, описанным в ЗаполнениеВнеФормы
//  РеквизитыШапки  - структура с обязательными реквизитами Организация и Дата
//  Таблица         - табличная часть объекта
//  Отбор	 - Структура - см. ПравилоСоответствуетОтбору()
Процедура ЗаполнитьТаблицуПоПравилам(Правила, РеквизитыШапки, Таблица, Отбор = Неопределено) Экспорт
	...
	ДанныеЗаполнения = ДанныеЗаполненияИзРеквизитовДокумента(РеквизитыШапки, Правила);
...
Процедура ЗаполнитьРасчетыПоАренде(Параметры)
