...
Процедура СформироватьДвиженияПрочееСписание(ТаблицаРасшифровкаПлатежа, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если ТаблицаРеквизиты.Количество() > 0 И ТаблицаРеквизиты[0].ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПодотчетномуЛицу И ТипЗнч(ТаблицаРеквизиты[0].Регистратор) = Тип("ДокументСсылка.СписаниеСРасчетногоСчета")
		И НЕ ЗначениеЗаполнено(ТаблицаРеквизиты[0].Регистратор.Контрагент) тогда
			Для каждого ТФизЛицо из ТаблицаРеквизиты[0].Регистратор.ПодотчетныеЛица цикл
				Проводка = Движения.Хозрасчетный.Добавить();
				
				Проводка.Период      = Реквизиты.Период;
				Проводка.Содержание  = Реквизиты.Содержание;
				Проводка.Организация = Реквизиты.Организация;
				Проводка.Сумма       = ТФизЛицо.Сумма;
				
				Проводка.СчетДт = Параметры.РасшифровкаПлатежа[0].СчетДт;
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, ТФизЛицо.ФизЛицо);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2, Параметры.РасшифровкаПлатежа[0].СубконтоДт2);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3, Параметры.РасшифровкаПлатежа[0].СубконтоДт3);
				
				СвойстваСчетаДт = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Проводка.СчетДт);
				Если СвойстваСчетаДт.Валютный Тогда
					Сообщить("Не формируются проводки для данного случая в валюте!");
					//Проводка.ВалютаДт        = Реквизиты.ВалютаДокумента;
					//Проводка.ВалютнаяСуммаДт = СтрокаПлатежа.ВалютнаяСумма;
				КонецЕсли;
				
				Если СвойстваСчетаДт.УчетПоПодразделениям Тогда
					Проводка.ПодразделениеДт = Параметры.РасшифровкаПлатежа[0].ПодразделениеДт;
				КонецЕсли;
				
				Проводка.СчетКт = Параметры.РасшифровкаПлатежа[0].СчетКт;
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 1, Параметры.РасшифровкаПлатежа[0].СубконтоКт1);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 2, Параметры.РасшифровкаПлатежа[0].СубконтоКт2);
				...
				Если СвойстваСчетаКт.УчетПоПодразделениям Тогда
					Проводка.ПодразделениеКт = Параметры.РасшифровкаПлатежа[0].ПодразделениеКт;
				КонецЕсли;
				
				НалоговыйУчет.ЗаполнитьНалоговыеСуммыПроводки(ТФизЛицо.Сумма, ТФизЛицо.Сумма,,,,, Проводка);
			КонецЦикла;
	Иначе
