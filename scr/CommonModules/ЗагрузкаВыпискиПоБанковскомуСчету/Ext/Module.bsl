	...
	Для Каждого КлючИЗначение Из Выписки Цикл
		...
		Для Каждого Операция Из Выписка.Операции Цикл
			...
			Если Операция.ПоступилоСписано = "Поступило" Тогда
				//Определим реквизиты исходя из назначения платежа
				Если ЛЕВ(Операция.НазначениеПлатежа, 2) = "01" Тогда
					РаспознаннаяОперация.ХозяйственнаяОперация.Вставить("ВидОперацииДокумента", Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление);
					РаспознаннаяОперация.ХозяйственнаяОперация.Вставить("СБ_ПрофильРазноски", "57.01");
					РаспознаннаяОперация.ХозяйственнаяОперация.Вставить("СтатьяДвиженияДенежныхСредств", СБ_КазначействоПовтИсп.ПолучитьЗначениеКонстанты("СтатьяДДС_Инкассация"));
				ИначеЕсли ЛЕВ(Операция.НазначениеПлатежа, 2) = "02" Тогда
					РаспознаннаяОперация.ХозяйственнаяОперация.Вставить("ВидОперацииДокумента", Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам);
					РаспознаннаяОперация.ХозяйственнаяОперация.Вставить("СтатьяДвиженияДенежныхСредств", Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам);
				КонецЕсли;
			КонецЕсли;
			// -- 1c-izhtc Клинцов 2016.03.17 )	
...
Функция УточнитьХозяйственнуюОперациюДаннымиВведеннымиПользователем(ДанныеВведенныеПользователем, ИдентификаторВыписки, ИдентификаторОперации, РаспознанныеДанныеИзБанка, Настройки) Экспорт
	...
	Если ЗаменитьХозяйственнуюОперацию Тогда
		...
		// ++ 1c-izhtc Клинцов 2016.03.16 (
		ВидОперацииДокумента = Неопределено;
		Если Не ДанныеВведенныеПользователем.Свойство("ВидОперацииДокумента", ВидОперацииДокумента) Тогда
			ДанныеВведенныеПользователем.Свойство("ВидОперацииДокумента", ВидОперацииДокумента);
		КонецЕсли;
		// -- 1c-izhtc Клинцов 2016.03.16 )		
	...
	Иначе
		...
		// +СБ. Широков Николай. 2016-05-24.
		ЗаполнитьБанковскийСчет = ЗаполнитьБанковскийСчет И Не ДанныеВведенныеПользователем.Свойство("БанковскийСчетКонтрагента");
		// -СБ. Широков Николай
		...
		// +СБ. Широков Николай. 2016-05-24.
		ЗаполнитьДоговор 	= ЗаполнитьДоговор
			И Не ДанныеВведенныеПользователем.Свойство("ДоговорКонтрагента");
		
		ЗаполнитьСтатьюДДС 	= ЗаполнитьСтатьюДДС
			И Не ДанныеВведенныеПользователем.Свойство("СтатьяДвиженияДенежныхСредств");
		// -СБ. Широков Николай		
...
Функция ВыпискиПоБанковскимСчетам(ДанныеИзБанка, УчастникиОпераций, ИдентификаторыВсехДокументов, Протокол = Неопределено)
	...
	Для Каждого Документ Из ДанныеИзБанка.Документы Цикл
		...
		Для Каждого КлючИЗначение Из Стороны Цикл
			...
			Если ПоступилоСписано.Списано И ЭтоОперацияПоКорпоративнойКарте(Операция) Тогда
...
Функция РеквизитыСчетаИдентичны(РеквизитыПротивоположнойСтороны, РеквизитыНашейСтороны)
	...
	Для Каждого ЭлементРеквизитовСчетаУчастника Из ОбменСБанкомВФормате1С.ЭлементыРеквизитовСчетаУчастника() Цикл
		...
		Если РеквизитСчетаПротивоположнойСтороны <> РеквизитСчетаНашейСтороны Тогда
			Результат = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьПредопределенныеПравилаОпределенияХозяйственныхОпераций(Правила, ОтборВидОперации = Неопределено)
	...
	ИменаПравил.Списано.Добавить("ВозвратКредитаБанку");             // Применяется, только если явно указан соответствующий вид операции
...
Функция ОсновнойВидОперацииПредопределенногоПравила(ПоступилоСписано, ИмяПравила)
	...
	Если ПоступилоСписано = "Списано" Тогда
		...
		ИначеЕсли ИмяПравила = "ВозвратКредитаБанку" Тогда
...
Функция ХозяйственнаяОперацияСписано(Параметры)
	...
	ИначеЕсли Параметры.Правило.Имя = "ВозвратЗаймаКонтрагенту" Тогда
		Возврат ХозяйственнаяОперацияВозвратКредитаЗайма(Параметры);
...
Функция ХозяйственнаяОперацияЗаработнаяПлатаПоВедомости(Параметры)
	...
	Если Параметры.ПроверитьПрименимостьПравила Тогда
		...
		ПолучательБанк = ЭтоЮридическоеЛицо(Параметры, "Получатель");
		Если Не ПолучательБанк Тогда
			Возврат НовыйРезультатОпределенияХозяйственнойОперации();
		КонецЕсли;
...
Функция ХозяйственнаяОперацияВозвратКредитаЗайма(Параметры)
...
Функция ХозяйственнаяОперацияПоступлениеНаличныхВБанк(Параметры)
	...
	Если Параметры.ПроверитьПрименимостьПравила Тогда
		...
		Если  Не ЭтоПоступлениеНаличныхВКассуБанка(Параметры.Операция)
...
Функция ВыплатаЗаработнойПлатыПоРеестру(Операция)
	...
	ЭтоСчетНезавершенныхРасчетов30232 = Операция.Очередность = УчетДенежныхСредствКлиентСервер.ОчередностьПлатежаОплатаТруда()
...
Функция ЭтоОперацияПоКорпоративнойКарте(Операция)
	...
	Возврат ЭтоСчетРасчетовСБанком И ЭтоШифрОперацииКорпКарты
		Или ЭтоБалансовыйСчетКорпКарты И (ЭтоШифрОперацииКорпКарты Или ЭтоБанковскийОрдер);
...
Функция СтавкаНДС(ЗначениеСтавкиНДС)
	...
	Возврат ПредопределенноеЗначение("Перечисление.СтавкиНДС." + ТекстСтавки);
...
Процедура ЗаполнитьДаннымиБанкаРаспознаннаяОперация(Документ, ДанныеЗаполнения)
	...
	// ++ 1c-izhtc Клинцов 2016.03.17 (
	ЗаполнитьРеквизит(Документ, "СБ_ПрофильРазноски",       ДанныеЗаполнения.ХозяйственнаяОперация);
	// -- 1c-izhtc Клинцов 2016.03.17 )
...
Функция СоздатьДокументы(Выписка, РаспознаннаяВыписка, Настройки, Отбор = Неопределено) Экспорт
	...
	Для Каждого Операция Из Выписка.Операции Цикл
		...
		Если ЗначениеЗаполнено(РаспознаннаяОперация.Ссылка) Тогда
			...
			Если Операция.ПоступилоСписано = "Списано" И ОбъектДокумента.НеПодтвержденоВыпискойБанка Тогда
...
Функция ВыделитьНДСИзНазначенияПлатежа(ИсходноеНазначениеПлатежа, СуммаПлатежа)
	...
	Если ЗначениеСтавкиНДС = "БезНДС" Или ЗначениеСтавкиНДС = "ПустаяСсылка" Тогда
		СтавкаНДС = СтавкаНДС(ЗначениеСтавкиНДС);
		ЗначениеСуммыНДС = 0;
	...
	Иначе
		...
		Если ЗначениеСуммыНДС <> Неопределено Тогда
			...
			Если СуммаБезНДС > 0 Тогда
				...
				Иначе
					СтавкаНДС = СтавкаНДС("ПустаяСсылка");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ВыделитьИнформациюОНДСВНазначении(ТекстНазначениеПлатежа, ЗначениеСтавкиНДС, СпецСимволыЧисла)
	...
	Пока Не ПустаяСтрока(ТекстНазначениеПлатежа) Цикл
		...
		Если Шаблон.ОблагаетсяНДС И Не ПустаяСтрока(ТекстНазначениеПлатежаПосле) Тогда
...
Функция ВыделитьИскомыйТекст(ОбрабатываемыйТекст, ИскомыйТекст)
	...
	ТекстДо = СокрЛП(Лев(ОбрабатываемыйТекст, ПозицияНачала - 1));
