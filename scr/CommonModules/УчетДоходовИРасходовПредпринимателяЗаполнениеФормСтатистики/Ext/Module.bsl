...
Функция ВыручкаИППоВидамДеятельности(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	Если РеквизитыОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо 
		...
		Если ТипЗнч(ОсновнойВидДеятельности) = Тип("СправочникСсылка.ВидыДеятельностиПредпринимателей")
			...
			НаименованиеОсновногоВидаДеятельности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОсновнойВидДеятельности, "Наименование");
			ОКВЭДОсновногоВидаДеятельности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОсновнойВидДеятельности, "КодОКВЭД");
	...
	Для Каждого Патент Из ПатентыИП Цикл
		...
		ПараметрыОтчета.Вставить("Патент",						Патент);
		...
		Отчеты.КнигаУчетаДоходовПатент.СформироватьОтчет(ПараметрыОтчета, ПоместитьВоВременноеХранилище(Неопределено));
...
#Область СлужебныеПроцедурыИФуенкции
