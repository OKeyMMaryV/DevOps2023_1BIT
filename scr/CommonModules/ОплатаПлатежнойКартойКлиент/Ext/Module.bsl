...
Процедура НомерЧекаНПДПриИзменении(Форма, Элемент) Экспорт
	...
	ДокументОснованиеОбработкаВыбора(Форма);
...
Процедура ТекстНеобходимоЗаполнитьПатентОбработкаНавигационнойСсылки(Форма, Элемент, НавигационнаяСсылка, СтандартнаяОбработка) Экспорт
...
Процедура ДокументОснованиеОбработкаВыбора(Форма, ВыбранноеЗначение = Неопределено) Экспорт
	...
	Если СтруктураОснование.Свойство("РасшифровкаПлатежа") Тогда
		УправлениеЗачетомАвансов = ОплатаПлатежнойКартойВызовСервера.УправлениеЗачетомАвансовПогашениемЗадолженности();
		Для Каждого ТекСтрока Из СтруктураОснование.РасшифровкаПлатежа Цикл
			...
			Если УправлениеЗачетомАвансов Тогда
				СтрокаРасшифровки.СпособПогашенияЗадолженности =
					ПредопределенноеЗначение("Перечисление.СпособыПогашенияЗадолженности.ПоДокументу");
				СтрокаРасшифровки.Сделка = Объект.ДокументОснование;
			Иначе
				СтрокаРасшифровки.СпособПогашенияЗадолженности =
	...
	ОплатаПлатежнойКартойКлиентСервер.ЗаполнитьРеквизитыРасшифровкаПлатежа(Форма, Истина, Объект.РасшифровкаПлатежа[0]);
