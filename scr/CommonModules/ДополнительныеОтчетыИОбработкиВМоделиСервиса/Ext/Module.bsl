...
Процедура ЗаблокироватьДополнительныеОтчетыИОбработкиДляОбновления() Экспорт
	...
	Запрос.УстановитьПараметр("Версия", бит_ОбщегоНазначения.МетаданныеВерсия());
...
Функция ПроверитьСовместимостьПоставляемойОбработки(Знач ТаблицаСовместимости)
	...
	Для Каждого ДекларацияСовместимости Из ТаблицаСовместимости Цикл
		...
		Иначе
			...
			Если ДекларацияСовместимости.ConfigarationName = Метаданные.Имя И ДекларацияСовместимости.VersionNumber = бит_ОбщегоНазначения.МетаданныеВерсия() Тогда
				Возврат Истина;
			КонецЕсли;
