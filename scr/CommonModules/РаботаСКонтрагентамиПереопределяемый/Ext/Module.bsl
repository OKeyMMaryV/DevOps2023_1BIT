...
Процедура ЗаполнитьНаименованияВидовДеятельностиКонтрагента(ВидыДеятельности) Экспорт
	...
	Если ВидыДеятельности.Количество() > 0 Тогда
		КлассификаторОКВЭД  = Неопределено;
		КлассификаторОКВЭД2 = Неопределено;
		Для каждого СтрокаТаблицы Из ВидыДеятельности Цикл
			Если СтрокаТаблицы.ЭтоОКВЭД2 Тогда
				Если КлассификаторОКВЭД2 = Неопределено Тогда
					КлассификаторОКВЭД2 = ОбщегоНазначенияБПВызовСервера.ПолучитьКлассификатор("ОКВЭД2");
				КонецЕсли;
				СтрокаТаблицы.НаименованиеОКВЭД = КлассификаторОКВЭД2.Получить(СтрокаТаблицы.КодОКВЭД);
			Иначе
				Если КлассификаторОКВЭД = Неопределено Тогда
