...
// Устарела.
// Следует использовать НалогНаПрибыльБухгалтерскийУчет.ИспользоватьСуммыРазниц
//
...
Функция ПоддержкаПБУ18(Организация, Период) Экспорт
	...
	Возврат НалогНаПрибыльБухгалтерскийУчет.ИспользоватьСуммыРазниц(Организация, Период);
...
Функция ПлательщикФиксированныхВзносов(Организация, Период, ДатаИзменения = Неопределено) Экспорт
	
	Результат = Не ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация)
		И СистемаНалогообложения(Организация, Период, ДатаИзменения) <> Перечисления.СистемыНалогообложения.НалогНаПрофессиональныйДоход;
...
Функция ПлательщикФиксированныхВзносовЗаПериод(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	Результат = Ложь;
	Для Каждого Значение Из МассивЗначений Цикл
		Результат = Результат Или Значение <> Перечисления.СистемыНалогообложения.НалогНаПрофессиональныйДоход;
	КонецЦикла;
...
// Параметры учетной политики по аренде

Функция ПрименяетсяФСБУ25(Организация, Период) Экспорт
	
	Если Период >= НачалоОбязательногоПримененияФСБУ25() Тогда
		Возврат Истина;
	ИначеЕсли Период < НачалоДосрочногоПримененияФСБУ25() Тогда
		Возврат Ложь;
	Иначе
		ПараметрыОпции = Новый Структура("Организация", Организация);
		Возврат ПолучитьФункциональнуюОпцию("ПрименятьФСБУ25Досрочно", ПараметрыОпции);
	КонецЕсли;
	
КонецФункции

Функция НачалоПримененияФСБУ25(Организация) Экспорт
	
	ПараметрыОпции = Новый Структура("Организация", Организация);
	
	НачалоПрименения = ?(ПолучитьФункциональнуюОпцию("ПрименятьФСБУ25Досрочно", ПараметрыОпции),
		НачалоДосрочногоПримененияФСБУ25(),
		НачалоОбязательногоПримененияФСБУ25());
...
// Параметры учетной политики по основным средствам

Функция ПрименяетсяФСБУ6(Организация, Период) Экспорт
	
	Возврат Период >= НачалоПримененияФСБУ6(Организация);
	
КонецФункции

Функция НачалоПримененияФСБУ6(Организация) Экспорт
	
	ПараметрыОпции = Новый Структура("Организация", Организация);
	
	НачалоПрименения = ?(ПолучитьФункциональнуюОпцию("ПрименятьФСБУ6Досрочно", ПараметрыОпции),
