...
//  СтруктураПараметров - Структура - данные, возвращаемые процедурой.
//
Процедура СформироватьПлатежноеПоручение(СсылкаНаОбъект, НастройкиОбменаЭД, ДеревоДокумента, СтруктураПараметров, Идентификатор = Неопределено) Экспорт
...
Процедура НайтиВыпискиЗаПроизвольныйПериод(Параметры, АдресРезультата) Экспорт
	...
	РеквизитОрганизация = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
		"БанковскийСчетОрганизации.Организация");
	РеквизитБанк = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
		"БанковскийСчетОрганизации.Банк");
	РеквизитНомерСчета = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
		"БанковскийСчетОрганизации.НомерСчета");
	Если Не ЗначениеЗаполнено(РеквизитНомерСчета) Тогда
		РеквизитНомерСчета = "НомерСчета";
	КонецЕсли;
	ЗначенияРеквизитовСчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Параметры.СчетОрганизации, "" + РеквизитОрганизация + "," + РеквизитБанк + "," + РеквизитНомерСчета);
...
Функция НайтиСоздатьСтрокуВДеревеРазбора(СтрокаТипаВДереве, ИдОбъекта, ОписаниеОбъекта, СсылкаНаОбъект, ДопРеквизиты, ДеревоРазбора)
		...
		Если ЭД.СекцияРасчСчет.РасчСчета.Количество()>0 Тогда
			...
			Для Каждого РеквизитыРасчСчета Из ЭД.СекцияРасчСчет.РасчСчета Цикл
				...
				НайденнаяСтрока = НайтиСоздатьСтрокуВДеревеРазбора(НайденныйТипВДереве, РеквизитыРасчСчета.РасчСчет, Неопределено,
					РасчетныйСчет, ПараметрыСчета, ДеревоРазбора);
...
Процедура ПрочитатьВыпискуAsyncXDTO(ЭД, ДеревоРазбора, НовыйЭД)
	...
	НайденнаяСтрока = НайтиСоздатьСтрокуВДеревеРазбора(
		НайденныйТипВДереве, ПараметрыСчета.РасчСчет, Неопределено, РасчетныйСчет, ПараметрыСчета, ДеревоРазбора);
	СписокТЧ.Добавить(НайденнаяСтрока.ИндексСтроки, "БанковскийСчет");
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитыТЧОбъекта(НовыйЭД, "БанковскиеСчетаОрганизаций", СписокТЧ);
	...
	Если Stamp <> Неопределено Тогда
		...
		НайденныйТипВДереве = ЭлектронноеВзаимодействиеСлужебный.НайтиСоздатьТипОбъектаВДеревеРазбора(
			ДеревоРазбора, "Штамп");
		НайденнаяСтрока = НайтиСоздатьСтрокуВДеревеРазбора(
			НайденныйТипВДереве, "Штамп", Неопределено, Неопределено, ПараметрыШтампа, ДеревоРазбора);
...
Процедура ПрочитатьВыпискуБанкаАсинхронныйОбмен(НастройкаОбмена, ПакетОбменСБанками, ДвоичныеДанныеФайла, СоздаватьОперацииВыписки, НовоеСообщениеОбмена)
...
Функция ИзмененыРеквизитыДокумента(СообщениеОбмена)
	...
	СформироватьПлатежноеПоручение(
		СсылкаНаОбъект, НастройкиОбмена, ДеревьяДанных.Получить(0), СтруктураВозврата, Идентификатор);
...
Процедура ПрочитатьВыпискуСбербанкXDTO(ЭД, ДеревоРазбора, НовыйЭД)
	...
	ПлановыйИсходящийОстатокВНациональнойВалюте = ОбменСБанкамиКлиентСервер.ЧислоИзСтроки(
	...
	НайденнаяСтрока = НайтиСоздатьСтрокуВДеревеРазбора(
		НайденныйТипВДереве, ЭД.Acc, Неопределено, РасчетныйСчет, ПараметрыСчета, ДеревоРазбора);
	СписокТЧ.Добавить(НайденнаяСтрока.ИндексСтроки, "БанковскийСчет");
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьРеквизитыТЧОбъекта(НовыйЭД, "БанковскиеСчетаОрганизаций", СписокТЧ);
	...
	PayDocCur = ОбъектТипаCML(ФабрикаXDTO, "PayDocCur", ПространствоИмен);
		...
		СекцияБенефициар = ОбъектТипаCML(ФабрикаXDTO, "Beneficiar_59", ПространствоИмен);
			...
			СекцияСтранаБенефициара = ОбъектТипаCML(ФабрикаXDTO, "Country", ПространствоИмен);
				КодСтраныБенефициара = ЗначениеРеквизитаВДереве(
				...
				ISOКодСтраныБенефициара = ЗначениеРеквизитаВДереве(
