...
Функция УстановкаПараметровСеанса(ИменаПараметровСеанса) Экспорт
	...
	Если ИменаПараметровСеанса = Неопределено Тогда
		...
		ОК_ОбщегоНазначения.УстановитьПараметрыСеанса();
		//ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2021-07-07 (#4242)
		
		//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103
		ок_ОбменСКонтрагентамиВнутренний.УстановитьПараметрыСеанса();
		//ОК(СофтЛаб) Вдовиченко Г.В. 20181101 №3103
...
Процедура ПередЗапускомПрограммы()
	...
	Если ПустаяСтрока(бит_ОбщегоНазначения.МетаданныеВерсия()) Тогда
		ВызватьИсключение НСтр("ru = 'Не заполнено свойство конфигурации Версия.'");
	Иначе
		Попытка
			НулеваяВерсия = ОбщегоНазначенияКлиентСервер.СравнитьВерсии(бит_ОбщегоНазначения.МетаданныеВерсия(), "0.0.0.0") = 0;
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
