...
Процедура ЗаполнитьПризнакиУчета(Форма) Экспорт
	...
	|ИЗ
	|	ПланСчетов.бит_Дополнительный_2 КАК ПланСчетов&УсловиеСоединения
	|ГДЕ
	|	&УсловиеОтбора";
	...
	Для каждого ВидСубконто Из МассивСубконто Цикл
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланСчетов.бит_Дополнительный_2.ВидыСубконто КАК ПланСчетовВидыСубконто" + НомерСубконто + "
...
Функция ИспользуютсяОборотныеСубконто(ИсточникВидовСубконто, Отбор = Неопределено) Экспорт
	...
	Если ТипСпискаСубконто = Тип("ПланСчетовСсылка.бит_Дополнительный_2") Тогда
