...
Функция ИнформацияОПрограммеДляВК() Экспорт
	...
	Информация.Вставить("ВерсияКонфигурации", бит_ОбщегоНазначения.МетаданныеВерсия());
...
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена;
//  СообщенияОбмена - Соответствия - данные сообщений обмена и сертификатов подписи:
...
//
// Параметры:
//  СообщениеОбмена - ДокументСсылка.СообщенияОбменСБанками - сообщение обмена
//  Отпечаток - Строка - отпечаток сертификата
...
Функция ТекстЗапросаНочнойВыписки(Знач НастройкаОбмена, Знач ИдентификаторЗапроса, Знач ИдентификаторОрганизации, Знач ДанныеПодписи = Неопределено) Экспорт
	...
	Incoming = ОбменСБанкамиСлужебный.ОбъектТипаCML(ФабрикаXDTO, "Request.Incoming", "http://bssys.com/upg/request");
...
//    Структура - структура возврата описана в ДлительныеОперации.ВыполнитьВФоне.
//
Функция ЗапускЗаданияСменыПароляСбербанк(Знач НастройкаОбмена, Знач КлючСессии, Знач Логин, Знач Пароль, Знач НовыйПароль, Знач Соль, Знач B) Экспорт
...
Функция МаркерБанкаПоСертификату(АдресСервера, ИдентификаторОрганизации, ДвоичныеДанныеСертификата, ВерсияФормата = Неопределено, ПробнаяОперация = Ложь, НастройкаОбмена = Неопределено) Экспорт
	...
	Заголовки.Вставить("AvailableAPIVersion", ВерсияФормата);
...
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена с банком.
//  ПараметрыПолученияВыписки - Структура - параметры получения выписки. См. ОбменСБанкамиКлиентСервер.ПараметрыПолученияВыпискиБанка()
//  МассивОтпечатковСертификатов - Массив - (возвращаемое значение) дополняет массив строк - отпечатков сертификатов, отпечатками сертификатов на сервере.
//  НастройкиОбмена - Структура - (возвращаемое значение) настройки обмена из ОбменСБанкамиСлужебный.ОпределитьНастройкиОбменаЭДПоИсточнику.
...
//
Функция ЗапросыВыписок(Знач НастройкаОбмена, Знач ПараметрыПолученияВыписки, МассивОтпечатковСертификатов = Неопределено, НастройкиОбмена = Неопределено, АвтоматическоеПолучение = Ложь) Экспорт
...
Процедура СохранитьВходящееПисьмоСбербанка(НастройкаОбмена, ПисьмоXDTO, ДляВалютногоКонтроля, МассивСообщенийОбмена)
	...
	ВходящийНомер = ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(ПисьмоXDTO, "DocData.docNum");
