...
Функция ВыполнитьКомандуПечатиСводныхСправок(ОписаниеКоманды) Экспорт
	...
	Если Тип(СводныеСправки) = Тип("Массив") И СводныеСправки.Количество() = 1 Тогда 
...
Функция ВыполнитьКомандуПечатиКорректировочныхСправок(ОписаниеКоманды) Экспорт
	...
	Если Тип(КорректировочныеСправки) = Тип("Массив") И КорректировочныеСправки.Количество() = 1 Тогда 
		ПараметрыПечати = Новый Структура("ЗаголовокФормы", 
...
Функция СоздатьСчетФактуруВыданный(Форма, СтруктураОтбора = Неопределено) Экспорт
	...
	Результат = УчетНДСВызовСервера.СоздатьСчетФактуруВыданныйНаОсновании(Основание, СтруктураОтбора, Форма.УникальныйИдентификатор);
