...
Процедура ОткрытьДеревоЭД(СсылкаНаОбъект, ПараметрыОткрытия = Неопределено, ЭД = Неопределено) Экспорт
	...
	Если ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоДокументДополнительногоСервисаЭДО(СсылкаНаОбъект, ЭлектронныйДокумент) Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(ЭлектронныйДокумент) 
		И ТипЗнч(ЭлектронныйДокумент) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий") 
		И ок_ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоБумажныйДокумент(ЭлектронныйДокумент) Тогда
			ПоказатьЗначение(, ЭлектронныйДокумент);
			Возврат;
...
Процедура ДекорацияСостояниеЭДОНажатие(Форма, СтандартнаяОбработка = Ложь) Экспорт
	...
	Если Форма.Модифицированность Тогда
		...
		Если НРег(Реквизит.ТекстСостоянияЭДО) = НРег("ЕстьДействующийДокумент") Тогда
			...
			ОбменСКонтрагентамиСлужебныйКлиент.ОткрытьАктуальныйЭД(Форма.Объект.Ссылка);
		Иначе
