...
Процедура УстановитьНастройкиПоУмолчанию(ФормаОтчета) Экспорт
	...
	бит_БухгалтерскиеОтчетыВызовСервера.УстановитьНастройкиПоУмолчанию(ФормаОтчета);
...
Функция ИспользоватьСуммыРазниц(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	Если Не БухгалтерскийУчетВызовСервераПовтИсп.ПользователюДоступныСуммыПостоянныхВременныхРазниц() Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ПараметрыИсполненияОтчета <> Неопределено Тогда
		...
		КонецЕсли;
		Если ПараметрыИсполненияОтчета.Свойство("ПроверкаИтогов") Тогда
			ПроверкаИтогов = ПараметрыИсполненияОтчета.ПроверкаИтогов;
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьПараметрыРасшифровкиОтчета(Адрес, ИдентификаторОбъекта, Расшифровка) Экспорт
	...
	Если ИдентификаторОбъекта = "АнализСубконто"
		...
		// 1С-ИжТиСи Полина 7.10.2015 (
		ИЛИ ИдентификаторОбъекта = "СБ_ОборотыСчетаМеждународный"
		// 1С-ИжТиСи Полина 7.10.2015 )		
		...
		// СофтБаланс Начало Кузнецова С. [27.12.2013] 
		ФормироватьОборотыСчетаМеждународныйПоМесяцам = Ложь;
		ФормироватьОборотыСчетаМеждународныйПоДням    = Ложь;
		ФормироватьОтчетПоПроводкамМеждународный      = Ложь;
		// СофтБаланс Окончание Кузнецова С. [27.12.2013]		
		...
		// СофтБаланс Начало Кузнецова С. [27.12.2013] 
		ПредставлениеДляОборотыСчетаМеждународныйПоМесяцам = "Обороты счета "  + Счет + " (международный) по месяцам";
		ПредставлениеДляОборотыСчетаМеждународныйПоДням    = "Обороты счета "  + Счет + " (международный) по дням";
		ПредставлениеДляОтчетаПоПроводкамМеждународный     = "Отчет по проводкам (международный)";
		// СофтБаланс Окончание Кузнецова С. [27.12.2013]		
			...
			Иначе
				Если ЕстьПоказатель Тогда
					...
					ИначеЕсли ИдентификаторОбъекта = "ОборотыСчета" Тогда				
						СписокПунктовМеню.Добавить("ОтчетПоПроводкам");
					// СофтБаланс Начало Кузнецова С. [27.12.2013] 
					ИначеЕсли ИдентификаторОбъекта = "СБ_ОборотыСчетаМеждународный" Тогда				
						СписокПунктовМеню.Добавить("СБ_ОтчетПоПроводкамМеждународный");
					// СофтБаланс Окончание Кузнецова С. [27.12.2013]						
				...
				Иначе
					...
					Иначе 
						...
						// СофтБаланс Начало Кузнецова С. [27.12.2013] 
						ИначеЕсли ИдентификаторОбъекта = "СБ_ОборотыСчетаМеждународный" Тогда				
							СписокПунктовМеню.Добавить("СБ_ОтчетПоПроводкамМеждународный", ПредставлениеДляОтчетаПоПроводкамМеждународный);
						// СофтБаланс Окончание Кузнецова С. [27.12.2013]						
						КонецЕсли; 
					КонецЕсли;
				КонецЕсли;	
...
Процедура ИнициализацияФормыОтчета(Отчет, РеквизитыФормы, Форма)
	...
	// бит_Финанс изменения кода. Начало.
	бит_БухгалтерскиеОтчетыВызовСервера.ИнициализацияФормыОтчета(Отчет, РеквизитыФормы, Форма);
	// бит_Финанс изменения кода. Конец. 
	...
	Иначе 
		...
		Для каждого Отбор из МассивПолей Цикл
			Если ТипЗнч(Отбор) = Тип("ЗначениеПоляРасшифровкиКомпоновкиДанных") тогда
				...
				ИначеЕсли Отбор.Поле = "Счет" И ЕстьСчет И ИдентификаторОбъекта <> "СБ_ОборотыСчетаМеждународный" Тогда	// Счет задан в дополнительных свойствах, в отбор его добавлять не нужно
				// 1С-ИжТиСи Полина 7.10.2015 )
				...
				ИначеЕсли (ПолеОтбора = "Счет" Или ПолеОтбора = "КорСчет") И Не ЕстьСчет Тогда
					Если ПолеОтбора = "КорСчет" И Не ПустаяСтрока(БухТипРесурса) Тогда      
						// СофтБаланс Начало Кузнецова С. [24.01.2014] 
						...
						Если ИдентификаторОбъекта = "СБ_ОборотыСчетаМеждународный" И ЗначениеОтбора = ПланыСчетов.бит_Дополнительный_2.ПустаяСсылка() Тогда
							БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ПользовательскиеОтборы, "Счет" + ?(БухТипРесурса = "Дт", "Кт", "Дт"), ЗначениеОтбора, ВидСравненияКомпоновкиДанных.Равно);
						Иначе
							// СофтБаланс Окончание Кузнецова С. [24.01.2014]
							БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ПользовательскиеОтборы, "Счет" + ?(БухТипРесурса = "Дт", "Кт", "Дт"), ЗначениеОтбора, ВидСравненияКомпоновкиДанных.ВИерархии);	
							// СофтБаланс Начало Кузнецова С. [24.01.2014] 
						КонецЕсли;
						// СофтБаланс Окончание Кузнецова С. [24.01.2014]			
			...
			ИначеЕсли ТипЗнч(Отбор) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
				...
				Если СписокПолейОтборов.Найти(Строка(Отбор.ЛевоеЗначение)) = Неопределено Тогда
					...
					// 1С-ИжТиСи Полина 7.10.2015 (
					// СофтБаланс Начало Кузнецова С. [24.01.2014]
					ИначеЕсли ИдентификаторОбъекта = "СБ_ОборотыСчетаМеждународный" И ПолеОтбора = Новый ПолеКомпоновкиДанных("Счет") Тогда
						ТипРесурса = Неопределено;
						Для Каждого ЭлементМассива Из МассивПолей Цикл
							Если ТипЗнч(ЭлементМассива) = Тип("ЗначениеПоляРасшифровкиКомпоновкиДанных") И ЭлементМассива.Поле = "БухТипРесурса" Тогда
								ТипРесурса = ЭлементМассива.Значение;
								Прервать;
							КонецЕсли;
						КонецЦикла;
						Если ТипРесурса <> Неопределено Тогда 
							ПолеОтбора = Новый ПолеКомпоновкиДанных("Счет" + ТипРесурса);
						КонецЕсли;
						БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ПользовательскиеОтборы, ПолеОтбора, ЗначениеОтбора, Отбор.ВидСравнения);
						// +СБ. Кузнецова С. 2014-11-10 
					ИначеЕсли ИдентификаторОбъекта = "СБ_ОборотыСчетаМеждународный" И Найти(ПолеОтбора, "Субконто") = 1 Тогда
						
						ГруппаОтбора = ПользовательскиеОтборы.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
						ГруппаОтбора.Использование = Истина;
						ГруппаОтбора.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
						
						БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтбора, "СубконтоДт" + Прав(ПолеОтбора, СтрДлина(ПолеОтбора) - 8), ЗначениеОтбора, Отбор.ВидСравнения);
						БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтбора, "СубконтоКт" + Прав(ПолеОтбора, СтрДлина(ПолеОтбора) - 8), ЗначениеОтбора, Отбор.ВидСравнения);
						
						// -СБ. Кузнецова С.
						// +СБ. Кузнецова С. 2014-11-12 
					ИначеЕсли ИдентификаторОбъекта = "СБ_ОборотыСчетаМеждународный" И Найти(ПолеОтбора, "ПериметрКонсолидации") = 1 Тогда
						
						ГруппаОтбора = ПользовательскиеОтборы.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
						ГруппаОтбора.Использование = Истина;
						ГруппаОтбора.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
						
						БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтбора, "ПериметрКонсолидацииДт" + Прав(ПолеОтбора, СтрДлина(ПолеОтбора) - 20), ЗначениеОтбора, Отбор.ВидСравнения);
						БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтбора, "ПериметрКонсолидацииКт" + Прав(ПолеОтбора, СтрДлина(ПолеОтбора) - 20), ЗначениеОтбора, Отбор.ВидСравнения);
						
					ИначеЕсли ИдентификаторОбъекта = "СБ_ОборотыСчетаМеждународный" И Найти(ПолеОтбора, "КорПериметрКонсолидации") = 1 Тогда
						
						ГруппаОтбора = ПользовательскиеОтборы.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
						ГруппаОтбора.Использование = Истина;
						ГруппаОтбора.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
						
						БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтбора, "ПериметрКонсолидацииДт" + Прав(ПолеОтбора, СтрДлина(ПолеОтбора) - 23), ЗначениеОтбора, Отбор.ВидСравнения);
						БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтбора, "ПериметрКонсолидацииКт" + Прав(ПолеОтбора, СтрДлина(ПолеОтбора) - 23), ЗначениеОтбора, Отбор.ВидСравнения);
...
Функция ОписаниеОтборовОтчета(ПараметрыОтчета, КомпоновщикНастроек)
	...
	Иначе
		...
		ТекстПодразделение = "";
		// СофтБаланс Начало Кузнецова С. [26.12.2013] 
		Если ПараметрыОтчета.Свойство("ИдентификаторОтчета")
			И ПараметрыОтчета.ИдентификаторОтчета <>  "СБ_ОборотыСчетаМеждународный" 
			И ПараметрыОтчета.ИдентификаторОтчета <> "СБ_ОтчетПоПроводкамМеждународный" Тогда
		// СофтБаланс Окончание Кузнецова С. [26.12.2013]		
