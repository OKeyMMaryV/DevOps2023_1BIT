...
Функция СпособВыполненияЗадачи(ПараметрыЗадачи) Экспорт
	...
	ИначеЕсли (ИдентификаторЗадачи = "ТранспортныйНалог"
		...
		// С 2020 года отменены налоговые расчеты по авансам по налогу на имущество (№ 63-ФЗ от 15.04.2019).
		// Поэтому до 2020 года каждому правилу уплаты налога соответствует правило представления отчета,
		// а с 2020 года при уплате авансов по имуществу правило представления отчета отсутствует.
		ИЛИ ИдентификаторЗадачи = "НалогНаИмущество" И НЕ ЗначениеЗаполнено(ПараметрыЗадачи.ПравилоОтчета)) 
		И Правило.Периодичность = Перечисления.Периодичность.Квартал Тогда
		Возврат "УплатаАвансовИмущественныеНалоги"; // по данным государственного учета имущества
...
Функция ОписаниеДействияНалоговыйПлатеж(Событие)
	...
	Если Действие = Перечисления.ВидыДействийКалендаряБухгалтера.Отчет Тогда
		...
		ИначеЕсли ПолноеИмяПравила = "СтраховыеВзносы.2020_СведенияЭТКЕжемесячные" Тогда
...
Процедура ЗарегистрироватьИзменениеСтатусаЗадачиПодготовкиПлатежа(ДокументСсылка) Экспорт
	...
	ОбновитьСтатусЗадачиПодготовкиПлатежа(РеквизитыПлатежа);
	
КонецПроцедуры

Процедура ОбновитьСтатусЗадачиПодготовкиПлатежа(РеквизитыПлатежа)
	
	ТаблицаЗадач = ЗадачиСвязанныеСПлатежом(РеквизитыПлатежа);
	Если ТаблицаЗадач = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Функция ЗадачиСвязанныеСПлатежом(РеквизитыПлатежа)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(
		СвязанныеЗадачиУплатыНалога(РеквизитыПлатежа),
		ТаблицаЗадач);
...
Функция СвязанныеЗадачиУплатыНалога(РеквизитыДокумента)
	...
	НоваяЗадача = НоваяЗадачаНалоговогоПлатежа(РеквизитыДокумента, ТекущаяЗадача);
	...
	ИначеЕсли ТекущаяЗадача <> Неопределено
		И ПлатежиПоНалогуВсегдаОбновляютСвязанныеЗадачи(РеквизитыДокумента.ВидНалога) Тогда
...
Функция СтатусыЗадачПоУплатеНалогов(ТаблицаЗадач)
	...
	|	ЗадачиБухгалтера.ПериодСобытия КАК ПериодСобытия,
	|	ЗадачиБухгалтера.Правило КАК Правило,
	|	ЗадачиБухгалтера.Правило КАК ПравилоУплаты,
	|	ЗадачиБухгалтера.Периодичность КАК Периодичность,
	|	ЗадачиБухгалтера.Срок КАК Срок,
...
//
Функция ИдентификаторыПравилВыполняющихсяНеРеглОтчетами()
...
Процедура ЗаполнитьВыполнениеЗадачНеОбрабатываемыхБРО(ТаблицаЗадач)
	...
	ТаблицаЗадач.Колонки.Добавить("ИсполнительЗадачи", ТаблицаДокументы.Колонки.Ссылка.ТипЗначения);
	...
	|	ИдентификаторыПравил.ИдентификаторЗадачи,
	|	ИдентификаторыПравил.ИдентификаторПравила,
	|	ИдентификаторыПравил.СложноеУсловиеПроверки,
	|	ИдентификаторыПравил.УсловиеПроверки
	|ПОМЕСТИТЬ ИдентификаторыПравилНеОбрабатываемыхБРО
	|ИЗ
	|	&ИдентификаторыПравилВыполняющихсяНеРеглОтчетами КАК ИдентификаторыПравил
	|;
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЖурналОтчетовСтатусы.ДатаОкончания УБЫВ,
	|	ЖурналОтчетовСтатусы.ВариантОтчета УБЫВ
	|
	|ИТОГИ ПО
	|	Индекс";
...
Функция РелевантныйПериодСобытияДляОтчета(СвойстваЗадачи)
	...
	Если ПолноеИмяПравила = "СведенияОСреднесписочнойЧисленности.2016_РегистрацияОрганизации" Тогда
...
Функция НоваяЗадачаНалоговогоПлатежа(РеквизитыДокумента, ПредыдущаяЗадача)
	...
	Если Не ТребуетсяОбновитьЗадачу Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ПриЗаписиПлатежа(ДокументОбъект) Экспорт
	...
	ЗарегистрироватьИсполнениеЗадачиПриЗаписиПлатежа(РеквизитыПлатежа, ДокументОбъект.ДополнительныеСвойства);
...
Процедура ЗарегистрироватьИсполнениеЗадачиПриЗаписиПлатежа(РеквизитыПлатежа, ДополнительныеСвойства)
...
Функция ИзменятьСтатусыЗадачПриЗаписиПлатежа(РеквизитыПлатежа, ДополнительныеСвойства)
	...
	ИначеЕсли РеквизитыПлатежа.ВидДокумента = "ПлатежноеПоручение" Тогда
		...
		ПлатежноеПручениеВведеноНаОснованииСписания = ЭтоПлатежноеПоручениеНаОснованииСписанияСоСчета(РеквизитыПлатежа);
		
		Возврат ОбновлятьСвязанныеЗадачи И ИзменяетсяОплаченныйДокумент И НЕ ПлатежноеПручениеВведеноНаОснованииСписания;
