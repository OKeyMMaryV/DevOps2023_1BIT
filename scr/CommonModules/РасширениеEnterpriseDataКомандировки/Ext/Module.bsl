...
Процедура ПКО_Документ_ОперацияСБилетом_Получение_ПриКонвертацииДанныхXDTO(ДанныеXDTO, ПолученныеДанные, КомпонентыОбмена) Экспорт
	...
	Агент = СсылкаED(ДанныеXDTO.КлючевыеСвойства.Контрагент, "Справочник_Контрагенты_Получение", КомпонентыОбмена);
	...
	Для Каждого ПодтверждающийДокумент Из ПодтверждающиеДокументы Цикл
		...
		Билет = Билет(ПодтверждающийДокумент["content"], ПолученныеДанные.Организация, Агент, КомпонентыОбмена);
...
Функция Билет(СодержимоеБилета, Организация, Агент, КомпонентыОбмена)
	...
	ДанныеЗаполнения.Вставить("Агент",                Агент);
	ДанныеЗаполнения.Вставить("Организация",          Организация);
	ДанныеЗаполнения.Вставить("Сумма",                СодержимоеБилета["amount"]);
	ДанныеЗаполнения.Вставить("СтавкаНДС",            Перечисления.СтавкиНДС.ПустаяСсылка());
	ДанныеЗаполнения.Вставить("СуммаНДС",             0);
	ДанныеЗаполнения.Вставить("СуммаНеОблагаемаяНДС", 0);
	
	// Может быть передано несколько ставок, но сохранить можем лишь одну
	СтавкаНДС0 = Перечисления.СтавкиНДС.ПустаяСсылка(); // В файле может содержаться НДС0 или БезНДС
	БазаНДС    = 0;
	Для Каждого ЗаписьНДС Из СодержимоеБилета["vat_summary"] Цикл
		
		СуммаНДС  = ЗаписьНДС.Значение["amount"];
		СтавкаНДС = ЗаписьНДС.Значение["rate"];
		...
		Если Не ЗначениеЗаполнено(СуммаНДС) Тогда
			...
			Если Не ЗначениеЗаполнено(СтавкаНДС0) Тогда
				...
				Иначе
					СтавкаНДС0 = Перечисления.СтавкиНДС.БезНДС;
				КонецЕсли;
		...
		Иначе
			...
			ДанныеЗаполнения.СуммаНДС = ДанныеЗаполнения.СуммаНДС + СуммаНДС;
			
			БазаНДС = БазаНДС + ЗаписьНДС.Значение["net_sale"];
			
			Если Не ЗначениеЗаполнено(ДанныеЗаполнения.СтавкаНДС) Тогда
				
				Если СтавкаНДС = "18%" Тогда
					ДанныеЗаполнения.СтавкаНДС = Перечисления.СтавкиНДС.НДС18;
				ИначеЕсли СтавкаНДС = "10%" Тогда
					ДанныеЗаполнения.СтавкаНДС = Перечисления.СтавкиНДС.НДС10;
				ИначеЕсли СтавкаНДС = "20%" Тогда
					ДанныеЗаполнения.СтавкаНДС = Перечисления.СтавкиНДС.НДС20;
				Иначе
					ДанныеЗаполнения.СтавкаНДС = УчетНДСКлиентСервер.ОсновнаяСтавкаНДСПоБилетам(Истина);
				КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ДанныеЗаполнения.СтавкаНДС) Тогда
		Если ЗначениеЗаполнено(СтавкаНДС0) Тогда
			ДанныеЗаполнения.СтавкаНДС = СтавкаНДС0;
		Иначе
			ДанныеЗаполнения.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
		КонецЕсли;
	КонецЕсли;
		
	Если ЗначениеЗаполнено(ДанныеЗаполнения.СуммаНДС) Тогда
		ДанныеЗаполнения.СуммаНеОблагаемаяНДС = ДанныеЗаполнения.Сумма - ДанныеЗаполнения.СуммаНДС - БазаНДС;
	КонецЕсли;
