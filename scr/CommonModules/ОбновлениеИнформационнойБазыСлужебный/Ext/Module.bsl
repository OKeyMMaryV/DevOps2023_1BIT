...
Функция НеобходимоОбновлениеНеразделенныхДанныхИнформационнойБазы() Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		ВерсияМетаданных = бит_ОбщегоНазначения.МетаданныеВерсия();
...
Функция НеобходимоВыполнитьОбновление(Знач ВерсияМетаданных, Знач ВерсияДанных) Экспорт
	...
	ВыполнитьОбновление = НЕ ПустаяСтрока(ВерсияМетаданных) И ВерсияДанных <> ВерсияМетаданных;
	Если НЕ ВыполнитьОбновление И ЕстьПраваНаОбновлениеИнформационнойБазы(Истина, Ложь) Тогда
		Библиотека = "БитФинанс";
		ВерсияБиблиотеки = ОбновлениеИнформационнойБазы.ВерсияИБ(Библиотека);
		Если ВерсияБиблиотеки = "0.0.0.0" Тогда
			ВыполнитьОбновление = Истина;
		Иначе	
			ОписанияПодсистем 	= СтандартныеПодсистемыПовтИсп.ОписанияПодсистем();
			НоваяВерсия   		= ОписанияПодсистем.ПоИменам[Библиотека].Версия;
			ВыполнитьОбновление = ОбщегоНазначенияКлиентСервер.СравнитьВерсии(
									НоваяВерсия, ВерсияБиблиотеки) > 0
		КонецЕсли; 
	КонецЕсли; 
...
Функция СтатусНевыполненныхОбработчиков(ПриОбновлении = Ложь) Экспорт
	...
	Если ПриОбновлении Тогда
		...
		ВерсияМетаданныхБезНомераСборки = ОбщегоНазначенияКлиентСервер.ВерсияКонфигурацииБезНомераСборки(бит_ОбщегоНазначения.МетаданныеВерсия());
...
Функция ТребуетсяПроверитьЛегальностьПолученияОбновления() Экспорт
	...
	Если РежимОбновленияДанныхВЛокальномРежимеРаботы() = "НачальноеЗаполнение" Тогда
		ЛегальнаяВерсия = бит_ОбщегоНазначения.МетаданныеВерсия();
	Иначе
		СведенияОбОбновлении = СведенияОбОбновленииИнформационнойБазы();
		ЛегальнаяВерсия = СведенияОбОбновлении.ЛегальнаяВерсия;
	КонецЕсли;
	
	Возврат ЛегальнаяВерсия <> бит_ОбщегоНазначения.МетаданныеВерсия();
...
Процедура ЗаписатьПодтверждениеЛегальностиПолученияОбновлений() Экспорт
	...
	СведенияОбОбновлении = СведенияОбОбновленииИнформационнойБазы();
	СведенияОбОбновлении.ЛегальнаяВерсия = бит_ОбщегоНазначения.МетаданныеВерсия();
	ЗаписатьСведенияОбОбновленииИнформационнойБазы(СведенияОбОбновлении);
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если Пользователи.ЭтоПолноправныйПользователь(, Истина)
		...
		Если Значение = Неопределено
			Или ОбщегоНазначенияКлиентСервер.СравнитьВерсии(бит_ОбщегоНазначения.МетаданныеВерсия(), Значение) > 0 Тогда
			МодульОбновлениеКонфигурации = ОбщегоНазначения.ОбщийМодуль("ОбновлениеКонфигурации");
			Результат = МодульОбновлениеКонфигурации.ИсправленияИзменены();
			ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить("ПроверкаПатчей", "НомерВерсии", бит_ОбщегоНазначения.МетаданныеВерсия());
			Если Результат.ЕстьИзменения Тогда
				Сведения = СведенияОбОбновленииИнформационнойБазы();
				Сведения.ВерсияУдалениеПатчей = бит_ОбщегоНазначения.МетаданныеВерсия();
...
Процедура ПерейтиСДругойПрограммы()
	...
	Параметры.Вставить("ВерсияКонфигурации", бит_ОбщегоНазначения.МетаданныеВерсия());
	...
	Попытка
		...
		ВерсияКонфигурации = бит_ОбщегоНазначения.МетаданныеВерсия(); 
...
Процедура УстановитьФлагОтображенияОписанийПоТекущуюВерсию(Знач ИмяПользователя = Неопределено) Экспорт
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ОбновлениеИБ",
		"ПоследняяВерсияОтображенияИзмененийСистемы", бит_ОбщегоНазначения.МетаданныеВерсия(), , ИмяПользователя);
...
Процедура СформироватьСписокОтложенныхОбработчиковОбновления(ИтерацииОбновления, ПервыйОбменВРИБ = Ложь)
	...
	СведенияОбОбновлении.ВерсияДереваОбработчиков = бит_ОбщегоНазначения.МетаданныеВерсия();
...
Функция РазделыОписанияИзменений() Экспорт
	...
	ВесВерсииМетаданных = ВесВерсии(бит_ОбщегоНазначения.МетаданныеВерсия());
		...
		Для каждого Область Из Макет.Области Цикл
			...
			Если ВесВерсии > ВесВерсииМетаданных Тогда
				...
				ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстИсключения,
					Версия, бит_ОбщегоНазначения.МетаданныеВерсия());
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
