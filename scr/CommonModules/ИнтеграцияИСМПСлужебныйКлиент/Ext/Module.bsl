	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Иначе
			...
			Если РезультатОбмена.ТребуетсяОбновлениеКлючаСессии <> Неопределено Тогда
				...
				Для Каждого КлючИЗначение Из РезультатОбмена.ТребуетсяОбновлениеКлючаСессии Цикл
					
					Организация          = КлючИЗначение.Ключ;
					ПараметрыЗапроса     = КлючИЗначение.Значение.ПараметрыЗапроса;
					ПараметрыАвторизации = КлючИЗначение.Значение.ПараметрыАвторизации;
...
Процедура ОбработатьСообщенияНаПолучениеВходящихДокументов_ПриЗавершенииОперацииПодписи(ПодписанныеСообщенияПоОрганизациям, Контекст) Экспорт
	...
	НаборДанныхДляПодписанияПоОрганизациям = Контекст.НаборДанныхДляПодписанияПоОрганизациям;
	
	Контекст.ПодписанныеСообщенияПоОрганизациям.Вставить(Организация, ПодписанныеСообщенияПоОрганизациям);
	
	НаборДанныхДляПодписанияПоОрганизациям.Удалить(Организация);
