...
Функция РассчитатьСебестоимость(Период, Организация, Ссылка, РегламентнаяОперация) Экспорт
	...
	// ОборотыФикс, ПредварительныеПроводкиВыпуск
	...
	// Выбытия (использует ОборотыФикс, ПредварительныеПроводкиВыпуск)
	...
	ЗаписатьОписаниеГрафаДляОтладки(Затраты, МенеджерВременныхТаблиц, Ссылка);
...
Процедура СоздатьСчетаЗатрат(МенеджерВременныхТаблиц, Период, Организация, Режим)
	...
	//	СоздатьСчетаРасходовДоринда(МенеджерВременныхТаблиц, Режим);
	//Иначе
	//	СоздатьСчетаРасходов(МенеджерВременныхТаблиц, Режим);
	//КонецЕсли;
	////ОК
	//Заменено на:
	СоздатьСчетаРасходов(МенеджерВременныхТаблиц, Режим, Организация);
...
//Заменено на:
Процедура СоздатьСчетаРасходов(МенеджерВременныхТаблиц, Режим = "РасходыНаПроизводство", Организация = Неопределено)
//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-07-03 (#3377)
	...
	Если Режим = "РасходыНаПроизводство" Тогда
		...
		//Заменено на:
		Запрос.УстановитьПараметр("СчетаКосвенныхРасходов",        ?(Организация = бит_БК_Общий.ПолучитьЗначениеНастройкиБК("Соответствие наименования организации","ДОРИНДА"), 
		                                                             ПредопределенныеСчетаКосвенныхРасходовДоринда(),
...
Процедура СоздатьОборотыФикс(МенеджерВременныхТаблиц, Период, Организация)
	...
	// ОКЕЙ Поздняков А.С. (СофтЛаб) Начало 2021-11-12 (#4434)
	ОрганизацияОкей 				= бит_БК_Общий.ПолучитьЗначениеНастройкиМеханизмаИмпортаДанных("Организации", "Организация ОКЕЙ");
	ДатаНачалаТиповогоЗагрытияОкей 	= ок_ОбщегоНазначения.ПолучитьЗначениеУниверсальнойНастройки("ЗакрытиеПериода", "ПериодТиповоеЗакрытиеОкей", Дата(2099,1,1));
	Если Организация = ОрганизацияОкей
		И Период >= ДатаНачалаТиповогоЗагрытияОкей Тогда
		
		Запрос.УстановитьПараметр("ОрганизацияОкей", ОрганизацияОкей);
		ТекстПоДокументуОперацияАксапты = "
			|ОБЪЕДИНИТЬ
			|
			|ВЫБРАТЬ
			|	Движения.Регистратор,
			...
			|	Движения.Активность
			|	И Движения.Период МЕЖДУ &НачалоПериода И &КонецПериода
			|	И Движения.Организация = &ОрганизацияОкей
			|	И Движения.Регистратор ССЫЛКА Документ.бит_ок_ОперацияАксапты
			|	И ЕСТЬNULL(Движения.КоличествоКт, 0) = 0
			|";
		
		ТекстОбъединитьДокументыФикс = ТекстОбъединитьДокументыФикс + ТекстПоДокументуОперацияАксапты;
		
		ТекстПоДокументуОперацияАксапты12 = "
			|ОБЪЕДИНИТЬ
			|
			|ВЫБРАТЬ
			|	Движения.Регистратор,
			...
			|	И Движения.Период МЕЖДУ &НачалоПериода И &КонецПериода
			|	И Движения.Организация = &ОрганизацияОкей
			|	И Движения.Регистратор ССЫЛКА Документ.бит_ок_ОперацияАксапты12
			|	И ЕСТЬNULL(Движения.КоличествоКт, 0) = 0
			|";
		
		ТекстОбъединитьДокументыФикс = ТекстОбъединитьДокументыФикс + ТекстПоДокументуОперацияАксапты12;
	КонецЕсли;
	// ОКЕЙ Поздняков А.С. (СофтЛаб) Конец 2021-11-12 (#4434)
...
//
//
// Среди движений учета затрат есть такие, про которые известна дополнительная информация, не хранящаяся в проводках.
...
// Данные этого регистра поместим во временную таблицу.
// Важно: 
...
//
Процедура СоздатьПредварительныеПроводкиВыпуск(МенеджерВременныхТаблиц, Период, Организация, СИспользованиемСчета40 = Неопределено)
	
	Если СИспользованиемСчета40 = Неопределено Тогда
...
Процедура ЗаполнитьЦелевоеСальдо(Затраты, МенеджерВременныхТаблиц)
	...
	|ИЗ
	|	ВТ_ЦелевоеСальдо КАК ЦелевоеСальдо
	|
	|УПОРЯДОЧИТЬ ПО
	|	Идентификатор";
...
Функция СкорректироватьОценкуРебра(ОписаниеПриемника, Ребро, НоваяОценкаРебра, Затраты, ОписаниеИсточника)
...
Процедура ПодготовитьРасходыНеУдалосьРаспределить(Результат, МенеджерВременныхТаблиц, Протокол)
	...
	// Здесь подразумеваются только счета косвенных расходов, распределяемые по данным "методов распределения косвенных расходов".
...
Процедура ПодготовитьВыпускБезЗатрат(Результат, МенеджерВременныхТаблиц, Протокол)
	...
	|	НезавершенноеПроизводство КАК НезавершенноеПроизводство
	|		ЛЕВОЕ СОЕДИНЕНИЕ СубконтоЗатрат КАК СубконтоЗатрат
	|		ПО НезавершенноеПроизводство.Счет = СубконтоЗатрат.Счет
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КлючиВершин КАК Ключи
	|		ПО НезавершенноеПроизводство.Счет = Ключи.Счет
	|			И НезавершенноеПроизводство.Подразделение = Ключи.Подразделение
	|			И НезавершенноеПроизводство.Субконто1 = Ключи.Субконто1
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НезавершенноеПроизводство.Идентификатор КАК Идентификатор,
	|	НезавершенноеПроизводство.Сумма КАК Сумма
	|ИЗ
	...
	ЗаблокированныеЗатраты.Колонки.Добавить("Сумма",         БухгалтерскийУчетКлиентСервер.ТипСумма());
...
//ОК Довбешка Т.И. 03.02.2016
Функция ПредопределенныеСчетаКосвенныхРасходовДоринда()
...
//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2019-07-03 (#3377)
//Процедура СоздатьСчетаРасходовДоринда(МенеджерВременныхТаблиц, Режим = "РасходыНаПроизводство")
//	
//	Запрос = Новый Запрос;
//	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
//	Если Режим = "РасходыНаПроизводство" Тогда
//		Запрос.УстановитьПараметр("СчетаПрямыхРасходов",     УчетЗатрат.ПредопределенныеСчетаПрямыхРасходов());
//		Запрос.УстановитьПараметр("СчетаКосвенныхРасходов",  ПредопределенныеСчетаКосвенныхРасходовДоринда());
//		Запрос.УстановитьПараметр("ДобавитьСчетВыпуска",     Истина);
//	ИначеЕсли Режим = "РасходыНаПродажу" Тогда
//		Запрос.УстановитьПараметр("СчетаПрямыхРасходов",     Новый Массив);
//		Запрос.УстановитьПараметр("СчетаКосвенныхРасходов",  УчетЗатрат.ПредопределенныеСчетаРасходовНаПродажу());
...
//	|	Счет
//	|;
//	//ОКЕЙ Лобанов В.И.(СофтЛаб) Начало 2019-01-22 (#3194)
//	|
//	|////////////////////////////////////////////////////////////////////////////////
//	|ВЫБРАТЬ
//	|	ХозрасчетныйВидыСубконто.Ссылка КАК Счет,
//	|	ХозрасчетныйВидыСубконто.НомерСтроки КАК НомерСубконто
//	|ПОМЕСТИТЬ СубконтоПродукция
//	|ИЗ
//	|	ПланСчетов.Хозрасчетный.ВидыСубконто КАК ХозрасчетныйВидыСубконто
//	|ГДЕ
...
//	|	Счет
//	|;		
//	//ОКЕЙ Лобанов В.И.(СофтЛаб) Конец 2019-01-22 (#3194) 
//	|
//	|////////////////////////////////////////////////////////////////////////////////
//	|ВЫБРАТЬ
//	|	ХозрасчетныйВидыСубконто.Ссылка КАК Счет,
