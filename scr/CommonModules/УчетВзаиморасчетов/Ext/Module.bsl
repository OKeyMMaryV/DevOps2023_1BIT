...
Процедура ДобавитьПроводкиВспомогательныеРасчетыПоЭквайрингуПогашениеЗадолженности(СтрокаВзаиморасчетов, Реквизиты, ДтКтРасчетов, СвойстваСчетаРасчетов, Движения)
	...
	Если Реквизиты.УчитыватьЗадолженностьУСНПатент 
		...
		Проводка.Период       = Реквизиты.Период;
		Проводка.Организация  = Реквизиты.Организация;
		Если Реквизиты.Период < УчетУСН.ДатаНачалаДействияПатентнойСистемы() Тогда
...
Процедура СформироватьДвиженияПогашениеЗадолженности(ТаблицаВзаиморасчетов, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	ПроверкаПоступленияНаРС = ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.ПоступлениеНаРасчетныйСчет") И Реквизиты.Регистратор.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам;
	СубконтоВидыПереводов 	= ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НайтиПоНаименованию("Виды переводов");
	...
	Для каждого СтрокаВзаиморасчетов из Параметры.Взаиморасчеты Цикл
		...
		Иначе
			...
			//ОКЕЙ Балыков А.Г.(ПервыйБИТ) Начало 2021-04-29 (#ТП_БП11_ФР17)
			Если ПроверкаПоступленияНаРС
				И СтрокаВзаиморасчетов.КорСчет 		= ПланыСчетов.Хозрасчетный.РасчетныеСчета
				И СтрокаВзаиморасчетов.СчетРасчетов = ПланыСчетов.Хозрасчетный.ПереводыВПути Тогда
		
				СубконтоКТ1 = СтрокаВзаиморасчетов.Контрагент;
				СубконтоКТ2 = СтрокаВзаиморасчетов.ДоговорКонтрагента;
				СубконтоКТ3 = СтрокаВзаиморасчетов.ДокументРасчетов;
				
				СтруктураОтбора = Новый Структура;
				СтруктураОтбора.Вставить("ДоговорКонтрагента", 	СтрокаВзаиморасчетов.ДоговорКонтрагента);
				СтруктураОтбора.Вставить("СуммаВзаиморасчетов", СтрокаВзаиморасчетов.СуммаВзаиморасчетов);
				СтруктураОтбора.Вставить("СчетУчетаРасчетовСКонтрагентом", СтрокаВзаиморасчетов.СчетРасчетов);

				НайденныеСтроки = Реквизиты.Регистратор.РасшифровкаПлатежа.НайтиСтроки(СтруктураОтбора);
				Если НайденныеСтроки.Количество() > 0 Тогда
					СубконтоКТ1 = НайденныеСтроки[0].СубконтоКт1;
					СубконтоКТ2 = НайденныеСтроки[0].СтатьяДвиженияДенежныхСредств
				КонецЕсли;
				
				БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + ДтКтРасчетов], Проводка["Субконто" + ДтКтРасчетов],
					СубконтоВидыПереводов, СубконтоКТ1);
				БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + ДтКтРасчетов], Проводка["Субконто" + ДтКтРасчетов],
					"СтатьиДвиженияДенежныхСредств", СубконтоКТ2);
				БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + ДтКтРасчетов], Проводка["Субконто" + ДтКтРасчетов],
					"ДокументыРасчетовСКонтрагентами", СубконтоКТ3);
				
			ИначеЕсли ПроверкаПоступленияНаРС
				И СтрокаВзаиморасчетов.КорСчет 		= ПланыСчетов.Хозрасчетный.ПрочиеРасчетыСРазнымиДебиторамиИКредиторами
				И СтрокаВзаиморасчетов.СчетРасчетов = ПланыСчетов.Хозрасчетный.ПереводыВПути Тогда
				
				БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + ДтКтРасчетов], Проводка["Субконто" + ДтКтРасчетов],
					СубконтоВидыПереводов, СубконтоКТ1);
				БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + ДтКтРасчетов], Проводка["Субконто" + ДтКтРасчетов],
					"СтатьиДвиженияДенежныхСредств", Реквизиты.Регистратор.СтатьяДвиженияДенежныхСредств);
				БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + ДтКтРасчетов], Проводка["Субконто" + ДтКтРасчетов],
					"ДокументыРасчетовСКонтрагентами", СтрокаВзаиморасчетов.ДокументРасчетов);
...
Функция СодержаниеПроводокПоЗачетуАвансов(НаправлениеДвижения, ЭтоВозврат, ВидДоговора, СчетРасчетов) Экспорт
	...
	Иначе //"Выбытие"
		...
		ИначеЕсли БухгалтерскийУчетПереопределяемый.ЭтоВидДоговораСПокупателем(ВидДоговора) Тогда
			//ОКЕЙ Назаренко Д.В. (СофтЛаб) Начало 2021-02-15 (#4033)
			//Если СчетРасчетов = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученнымУЕВСчетОтгрузки Тогда
			Если СчетРасчетов = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученнымВСчетОтгрузкиУЕ Тогда
			//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-02-15 (#4033) 
...
Функция ОсновныеСчетаРасчетов() Экспорт
	...
	КлассСчетовРасчетов.Вставить("АрендныеПлатежи",             ПланыСчетов.Хозрасчетный.АрендныеПлатежи);                              // 76.07.2
	...
	КлассСчетовРасчетов.Вставить("АрендныеПлатежи",             ПланыСчетов.Хозрасчетный.АрендныеПлатежиВал);                           // 76.27.2
	...
	КлассСчетовРасчетов.Вставить("АрендныеПлатежи",             ПланыСчетов.Хозрасчетный.АрендныеПлатежиУЕ);                            // 76.37.2
...
Процедура СформироватьПроводкиПогашениеЗадолженностиПриОплатеПоОтгрузкеВУЕ(СтрокаВзаиморасчетов, Реквизиты, Движения, ОтражатьВНалоговомУчете, ОсновнойВидРазниц)
	...
	Проводка["Счет" + ДтКтРасчетов] = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученнымВСчетОтгрузкиУЕ;
	//ОКЕЙ Назаренко Д.В. (СофтЛаб) Конец 2021-02-15 (#4033) 
