...
Функция ОбработатьРезультатЗапросаИзмененныхВСД(ДанныеДокумента, ХозяйствующийСубъект, Предприятие, ПараметрыОбмена, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ИзмененаДатаСинхронизации И Не ЭтоПопыткаВосстановления Тогда
		УстановитьДатуВыполненияСинхронизации(
			Перечисления.ТипыВЕТИС.ВетеринарноСопроводительныеДокументы,
...
Функция ОбработатьЗаписиСкладскогоЖурнала(ПараметрыОбмена, ХозяйствующийСубъект, Предприятие, ИсключаяПоследнююДату = Ложь, ДокументДляВосстановления = Неопределено)
...
Функция ПараметрыОбновленияСтатуса(ПараметрыОбновленияСтатуса = Неопределено) Экспорт
	...
	КонецЕсли;
	ПараметрыОбновленияСтатуса.Вставить("ЕстьОшибкиВСДДолженБытьВСостоянииОформлен");
...
Процедура ЗаписатьСоответствиеНоменклатуры(Объект, ОписаниеТабличнойЧасти = "Товары") Экспорт
	...
	|	И Т.Номенклатура <> &ПустаяСсылкаНоменклатура
...
Процедура ПодготовитьЗапросыДляПроверкиКорректностиОформления(ПараметрыОбмена, ПовторитьЦиклОбработкиСообщений) Экспорт
	...
	Для Каждого ЭлементОчереди Из ПараметрыОбмена.ОчередьСообщений Цикл
		...
		Иначе
			...
			Если ХСПредприятияСОшибкамиСервиса = Неопределено Тогда
				...
				ХСПредприятияСОшибкамиСервиса.Колонки.Добавить("Сообщения");
			КонецЕсли;
			...
			Если Найденные.Количество() = 0 Тогда
				...
				ЭлементСОшибкой.Сообщения            = Новый Массив;
			Иначе
				ЭлементСОшибкой = Найденные[0];
			КонецЕсли;
			
			ЭлементСОшибкой.Сообщения.Добавить(ЭлементОчереди.Сообщение);
	...
	Если ХСПредприятияСОшибкамиСервиса <> Неопределено Тогда
		...
		Для Каждого СтрокаТЧ Из ХСПредприятияСОшибкамиСервиса Цикл
			...
			|	МАКСИМУМ(Сообщения.ДатаСоздания) КАК ДатаСоздания,
			|	МИНИМУМ(Сообщения.Порядок)       КАК Порядок
			|ИЗ
			|	(ВЫБРАТЬ
			|		ВЕТИСПрисоединенныеФайлы.СообщениеОснование КАК Сообщение,
			|		МАКСИМУМ(ВЕТИСПрисоединенныеФайлы.ДатаСоздания) КАК ДатаСоздания,
			|		1 КАК Порядок
			|	ИЗ
			|		Справочник.ВЕТИСПрисоединенныеФайлы КАК ВЕТИСПрисоединенныеФайлы
			|	ГДЕ
			|		ВЕТИСПрисоединенныеФайлы.СообщениеОснование В(&СообщенияОснования)
			|	
			|	СГРУППИРОВАТЬ ПО
			|		ВЕТИСПрисоединенныеФайлы.СообщениеОснование
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			|		ВЕТИСПрисоединенныеФайлы.Ссылка,
			|		МАКСИМУМ(ВЕТИСПрисоединенныеФайлы.ДатаСоздания),
			|		2
			|	ИЗ
			|		Справочник.ВЕТИСПрисоединенныеФайлы КАК ВЕТИСПрисоединенныеФайлы
			|	ГДЕ
			|		ВЕТИСПрисоединенныеФайлы.Ссылка В(&СообщенияОснования)
			|	
			|	СГРУППИРОВАТЬ ПО
			|		ВЕТИСПрисоединенныеФайлы.Ссылка) КАК Сообщения
			|
			|СГРУППИРОВАТЬ ПО
			|	Сообщения.Сообщение");
			
			Запрос.УстановитьПараметр("СообщенияОснования", СтрокаТЧ.Сообщения);
			...
			Пока ВыборкаСообщений.Следующий() Цикл
				...
				ДополнительныеПараметры.Вставить("НачалоПериода",         ВыборкаСообщений.ДатаСоздания - 60);
				ДополнительныеПараметры.Вставить("КонецПериода",          ВыборкаСообщений.ДатаСоздания + 60);
				...
				Зависимость.Сообщение1 = ВыборкаСообщений.Сообщение;
				...
				ЗаписьОчереди.Отбор.Сообщение.Установить(ВыборкаСообщений.Сообщение);
...
Функция ЗапросДляПроверкиКорректностиОформленияXML(ДокументСсылка) Экспорт
	...
	Если РеквизитыСообщения.Следующий() Тогда
		...
		ДополнительныеПараметры.Вставить("НачалоПериода",         РеквизитыСообщения.ДатаСоздания - 15);
		ДополнительныеПараметры.Вставить("КонецПериода",          РеквизитыСообщения.ДатаСоздания + 15);
...
// Подбирает подходящий документ из таблиц документов для восстановления.
// 
// Параметры:
//  ДатаЭлемента    - Дата - Дата запроса на оформление документа.
//  ДанныеДокумента - СтрокаТабличнойЧасти - строка с данными, подобранными из ВСД и записей складского учета по
//                                           идентификаторам, пришедшим из сервиса.
//  ПараметрыОбмена - Структура - (см. ИнтеграцияВЕТИС.ПараметрыОбмена).
//  ТипДокумента    - Строка - тип подбираемого документа.
// Возвращаемое значение:
//  Неопределено, ДокументСсылка - Документ, который требуется заполнить данными не полученными из-за ошибки сервиса.
Функция ДокументДляВосстановления(ДатаЭлемента, Отбор, ПараметрыОбмена, ТипДокумента) Экспорт
	...
	Для Каждого ЭлементДанных Из НайденныеСтроки Цикл
		...
		Дельта = ЭлементДанных.ДатаСоздания - ДатаЭлемента;
		Если Дельта < 0 Тогда
			Дельта = -Дельта;
		КонецЕсли;
		
		Если Дельта <= 15 Тогда
	...
	ВозвращаемоеЗначение = Неопределено;
	Если ПодходящиеДокументы.Количество() = 1 Тогда
	...
	ИначеЕсли ПодходящиеДокументы.Количество() > 1 Тогда
...
Функция ИнициализироватьДокументыДляВосстановления(ТипДокумента) Экспорт
	...
	ТипыДокументовОбъект = Новый Массив;
	ТипыДокументовОбъект.Добавить(Тип("ДокументОбъект.ВходящаяТранспортнаяОперацияВЕТИС"));
	ТипыДокументовОбъект.Добавить(Тип("ДокументОбъект.ИнвентаризацияПродукцииВЕТИС"));
	ТипыДокументовОбъект.Добавить(Тип("ДокументОбъект.ИсходящаяТранспортнаяОперацияВЕТИС"));
	ТипыДокументовОбъект.Добавить(Тип("ДокументОбъект.ОбъединениеЗаписейСкладскогоЖурналаВЕТИС"));
	ТипыДокументовОбъект.Добавить(Тип("ДокументОбъект.ПроизводственнаяОперацияВЕТИС"));
