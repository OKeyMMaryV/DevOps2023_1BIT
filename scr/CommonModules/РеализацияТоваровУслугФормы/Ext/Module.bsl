...
Процедура ДоговорКонтрагентаОбработатьИзменение(Форма, ПричиныИзмененияСчетовУчета)
	...
	Иначе
		...
		СпособЗачетаАвансовПоДоговору = Справочники.ДоговорыКонтрагентов.ОпределитьСпособЗачетаАвансовПоДоговору(Объект.ДоговорКонтрагента);
		Если СпособЗачетаАвансовПоДоговору <> Неопределено
			И Объект.СпособЗачетаАвансов <> СпособЗачетаАвансовПоДоговору Тогда
			Объект.СпособЗачетаАвансов = СпособЗачетаАвансовПоДоговору;
		КонецЕсли;
